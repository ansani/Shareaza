...
/* List of IP address blocks which are private / reserved and therefore not
 * suitable for public external IP addresses */
...
static const struct {
  ...
  uint32_t rmask;
    ...
    {IP(192, 175, 48, 0),
     MSK(24)},                    /* RFC7534 Direct Delegation AS112 Service */
    ...
    {IP(240, 0, 0, 0),
     MSK(4)}, /* RFC1112 Reserved for Future Use + RFC919 Limited Broadcast */
};
#undef IP
#undef MSK
...
int addr_is_reserved(const char *addr_str) {
  ...
  for (i = 0; i < sizeof(reserved) / sizeof(reserved[0]); ++i) {
    if ((address >> reserved[i].rmask) ==
        (reserved[i].address >> reserved[i].rmask))
      return 1;
  }
