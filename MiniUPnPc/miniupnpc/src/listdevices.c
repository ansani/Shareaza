  ...
  for (elt = *list_head; elt != NULL; elt = elt->next) {
    if (strcmp(elt->descURL, dev->descURL) == 0) {
      for (i = 0; i < elt->count; i++) {
        if (strcmp(elt->array[i]->st, dev->st) == 0 &&
            strcmp(elt->array[i]->usn, dev->usn) == 0) {
      ...
      if (elt->count >= elt->allocated_count) {
        ...
        tmp = realloc(elt->array,
                      elt->allocated_count * sizeof(struct UPNPDev *));
        if (tmp == NULL) {
          fprintf(
              stderr, "Failed to realloc(%p, %lu)\n", elt->array,
              (unsigned long)(elt->allocated_count * sizeof(struct UPNPDev *)));
          return;
        }
        elt->array = tmp;
      }
...
int main(int argc, char **argv) {
  ...
  for (i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-6") == 0)
      ...
      printf("   -l <device1> <device2> ... : search only for theses types of "
             "device\n");
  ...
  if (searched_device) {
    ...
    devlist =
        upnpDiscoverDevice(searched_device, 2000, multicastif, minissdpdpath,
    ...
    for (i = 0; searched_devices[i]; i++)
      printf("\t%s\n", searched_devices[i]);
    devlist =
        upnpDiscoverDevices(searched_devices, 2000, multicastif, minissdpdpath,
                            0 /*localport*/, ipv6, ttl, &error, 1);
  } else {
    printf("searching all UPnP devices\n");
    devlist = upnpDiscoverAll(2000, multicastif, minissdpdpath, 0 /*localport*/,
                              ipv6, ttl, &error);
  }
  if (devlist) {
    ...
    putchar('\n');
    for (dev_array = sorted_list; dev_array != NULL;
         dev_array = dev_array->next) {
      printf("%s :\n", dev_array->descURL);
