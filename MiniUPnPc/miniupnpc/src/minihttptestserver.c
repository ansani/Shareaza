...
static int server(unsigned short port, const char *expected_file_name,
                  int ipv6);
...
const struct {
  ...
  const char *text;
} modes_array[] = {{MODE_CHUNKED, "chunked"},     {MODE_ADDCRAP, "addcrap"},
                   {MODE_NORMAL, "normal"},       {MODE_FAVICON, "favicon.ico"},
                   {MODE_MALFORMED, "malformed"}, {MODE_INVALID, NULL}};
...
void handle_http_connection(int c) {
  ...
  while (request_len < sizeof(request_buffer) && !headers_found) {
    n = read(c, request_buffer + request_len,
  ...
  case MODE_FAVICON:
    ...
    n = snprintf(response_buffer, response_len,
                 ...
                 content_length);
    /* image/x-icon */
    build_favicon_content((unsigned char *)(response_buffer + n),
                          content_length);
...
static int server(unsigned short port, const char *expected_file_name,
  ...
  if (expected_file_name) {
    ...
    if (f) {
      ...
      if (buffer == NULL) {
        ...
        if (i != 16 * 1024) {
          fprintf(stderr,
                  "error writing to file %s : %dbytes written (out of %d)\n",
  ...
  while (!child && !quit) {
    ...
    c = accept(s, (struct sockaddr *)&client_addr, &client_addrlen);
    if (c < 0) {
