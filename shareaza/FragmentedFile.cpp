...
CFragmentedFile::CVirtualFilePart::CVirtualFilePart()
    : m_pFile(NULL), m_nOffset(0), m_bWrite(FALSE),
      m_nPriority(CFragmentedFile::prNormal) {}

CFragmentedFile::CVirtualFilePart::CVirtualFilePart(
    const CVirtualFilePart &pFile)
    : CShareazaFile(pFile), m_pFile(pFile.m_pFile), m_nOffset(pFile.m_nOffset),
      m_bWrite(pFile.m_bWrite), m_nPriority(pFile.m_nPriority) {}

CFragmentedFile::CVirtualFilePart &
CFragmentedFile::CVirtualFilePart::operator=(const CVirtualFilePart &pFile) {
...
CFragmentedFile::CFragmentedFile()
    : m_nUnflushed(0), m_oFList(0), m_nFileError(ERROR_SUCCESS), m_dwRef(1),
      m_pDownload(NULL) {}
...
void CFragmentedFile::AssertValid() const {
  ...
  if (m_oFile.size() != 0) {
    ...
    for (CVirtualFile::const_iterator i = m_oFile.begin(); i != m_oFile.end();
         ++i) {
      if (i != m_oFile.begin())
...
void CFragmentedFile::Dump(CDumpContext &dc) const {
  ...
  for (CVirtualFile::const_iterator i = m_oFile.begin(); i != m_oFile.end();
    ...
    dc << n << _T(". File offset ") << (*i).m_nOffset << _T(", ")
       << (*i).m_nSize << _T(" bytes, ")
       << ((*i).m_bWrite ? _T("RW") : _T("RO")) << _T(" \"") << (*i).m_sPath
       << _T("\"\n");
}
...
BOOL CFragmentedFile::Open(LPCTSTR pszFile, QWORD nOffset, QWORD nLength,
  ...
  CVirtualFile::iterator pItr =
      std::find(m_oFile.begin(), m_oFile.end(), pszFile);
  ...
  if (GetFileAttributes(_T("\\\\?\\") + strPath) == INVALID_FILE_ATTRIBUTES) {
    ...
    const CString strIncompletePath =
        Settings.Downloads.IncompletePath + _T("\\") + strFileName;
    if (GetFileAttributes(_T("\\\\?\\") + strIncompletePath) !=
        INVALID_FILE_ATTRIBUTES) {
      strPath = strIncompletePath;
    } else {
      if (m_pDownload) {
        const CString strSDPath =
            m_pDownload->m_sPath.Left(
                m_pDownload->m_sPath.ReverseFind(_T('\\')) + 1) +
            strFileName;
        if (GetFileAttributes(_T("\\\\?\\") + strSDPath) !=
            INVALID_FILE_ATTRIBUTES) {
          strPath = strSDPath;
        }
      }
  ...
  m_oFList.ensure((nLastBlockLength == SIZE_UNKNOWN)
                      ? SIZE_UNKNOWN
                      : (m_oFile.back().m_nOffset + nLastBlockLength));

  if (!pFile || !pFile->IsExists() ||
      (m_oFList.empty() && nRealLength != nLength))
...
BOOL CFragmentedFile::Open(const CShareazaFile *pSHFile, BOOL bWrite) {
  ...
  if (!m_oFile.empty()) {
    ...
    strSource.Format(_T("%s\\%s.partial"),
                     ...
                     (LPCTSTR)pSHFile->GetFilename());
  } else if (GetFileAttributes(CString(_T("\\\\?\\")) + pSHFile->m_sPath) !=
             INVALID_FILE_ATTRIBUTES) {
    ...
    if (CLibraryFile *pFile =
            LibraryMaps.LookupFileByHash(pSHFile, FALSE, TRUE)) {
      strSource = pFile->GetPath();
    }
  }

  if (!Open(strSource, 0, pSHFile->m_nSize, bWrite, pSHFile->m_sName)) {
    m_sFileError.Format(LoadString(bWrite ? IDS_DOWNLOAD_FILE_CREATE_ERROR
                                          : IDS_DOWNLOAD_FILE_OPEN_ERROR),
                        (LPCTSTR)strSource);
    theApp.Message(
        MSG_ERROR, _T("%s"),
        (LPCTSTR)(m_sFileError + _T(" ") + GetErrorString(m_nFileError)));
...
BOOL CFragmentedFile::Open(const CBTInfo &oInfo, BOOL bWrite) {
  ...
  if (bWrite && nCount == 0 && oInfo.GetCount() == 0 &&
      oInfo.m_nSize != SIZE_UNKNOWN) {
    CString strSource;
    // Generate new filename (inside incomplete folder)
    strSource.Format(_T("%s\\%s.partial"),
                     (LPCTSTR)Settings.Downloads.IncompletePath,
                     (LPCTSTR)oInfo.GetFilename());

    if (!Open(strSource, nOffset, oInfo.m_nSize, bWrite, oInfo.m_sName)) {
      m_sFileError.Format(LoadString(bWrite ? IDS_DOWNLOAD_FILE_CREATE_ERROR
                                            : IDS_BT_SEED_SOURCE_LOST),
                          (LPCTSTR)strSource);
      theApp.Message(MSG_ERROR, _T("%s"),
                     m_sFileError + _T(" ") + GetErrorString(m_nFileError));
  ...
  for (POSITION pos = oInfo.m_pFiles.GetHeadPosition(); pos; ++i) {
    ...
    if (i < nCount) {
      ...
      strSource.Format(_T("%s\\%s_%u.partial"),
                       (LPCTSTR)Settings.Downloads.IncompletePath,
                       (LPCTSTR)oInfo.GetFilename(), (DWORD)i);
    } else {
      // Open existing file from library
    ...
    if (const CLibraryFile *pLibraryFile =
            LibraryMaps.LookupFileByPath(strSource)) {
...
float CFragmentedFile::GetProgress(DWORD nIndex) const {
  ...
  if (nIndex >= m_oFile.size())
    ...
    return ((float)GetCompleted(m_oFile[nIndex].m_nOffset,
                                m_oFile[nIndex].m_nSize) *
            100.f) /
           (float)m_oFile[nIndex].m_nSize;
}
...
Fragments::List CFragmentedFile::GetFullFragmentList() const {
  ...
  for (; pItr != pEnd; ++pItr) {
    if ((*pItr).m_nPriority != prNotWanted)
      oList.insert(Fragments::Fragment((*pItr).m_nOffset,
                                       (*pItr).m_nOffset + (*pItr).m_nSize));
  }
...
Fragments::List CFragmentedFile::GetWantedFragmentList() const {
  ...
  for (CVirtualFile::const_iterator i = m_oFile.begin(); i != m_oFile.end();
    ...
    if ((*i).m_nPriority == prNotWanted)
      oList.erase(
          Fragments::Fragment((*i).m_nOffset, (*i).m_nOffset + (*i).m_nSize));
...
QWORD CFragmentedFile::GetCompleted(DWORD nIndex) const {
  ...
  return (nIndex < m_oFile.size())
             ? GetCompleted(m_oFile[nIndex].m_nOffset, m_oFile[nIndex].m_nSize)
             : 0;
}
...
DWORD CFragmentedFile::Move(DWORD nIndex, LPCTSTR pszDestination,
  ...
  if (bSkip)
    ...
    theApp.Message(MSG_DEBUG, _T("Moving \"%s\" to \"%s\"..."), (LPCTSTR)sPath,
                   (LPCTSTR)strTargetDir);
  ...
  if (bSuccess) {
    if (bSkip || bIsFolder)
      ...
      if (Settings.Downloads.RenameIfExists)
        for (int index = 0;
             GetFileAttributes(strTarget) != INVALID_FILE_ATTRIBUTES; index++) {
          if (nDot > 0)
            strTarget.Format(_T("%s\\%s_%u%s"), pszDestination,
                             sName.Left(nDot), index,
                             sName.Right(sName.GetLength() - nDot));
          else
      ...
      if (GetFileAttributesEx(strTarget, GetFileExInfoStandard, &fileAttr)) {
        // The file exists
        CString fileExtension = strTarget.Right(strTarget.GetLength() -
                                                strTarget.ReverseFind(_T('.')));
        ...
        CString newFileName =
            fileName + _T(" (") + currentTimestamp + _T(")") + fileExtension;
        theApp.Message(MSG_DEBUG,
                       _T("File \"%s\" already exists in the destination ")
                       _T("folder, renaming it to %s"),
                       (LPCTSTR)strTarget, (LPCTSTR)newFileName);
        strTarget = newFileName;
      }

      // Move/copy file using very long filenames
      bSuccess = MoveFileWithProgress(
          CString(_T("\\\\?\\")) + sPath, CString(_T("\\\\?\\")) + strTarget,
          lpProgressRoutine, pTask,
...
void CFragmentedFile::Serialize(CArchive &ar, int nVersion) {
  ...
  if (ar.IsStoring()) {
    ...
    if (nVersion < 40) {
      ...
      CString sPath =
          m_pDownload->m_sPath.Left(m_pDownload->m_sPath.GetLength() - 3);
      ...
      for (DWORD i = 0; i < count; ++i) {
        ...
        if (sPath.IsEmpty() || bWrite < FALSE || bWrite > TRUE ||
            sName.IsEmpty() || nPriority < prNotWanted || nPriority > prHigh)
...
BOOL CFragmentedFile::Write(QWORD nOffset, LPCVOID pData, QWORD nLength,
                            QWORD *pnWritten) {
  if (nLength == 0)
    // No data to write
    return TRUE;
...
BOOL CFragmentedFile::Read(QWORD nOffset, LPVOID pData, QWORD nLength,
                           QWORD *pnRead) {
  if (nLength == 0)
    // No data to read
    return TRUE;
...
BOOL CFragmentedFile::VirtualRead(QWORD nOffset, char *pBuffer, QWORD nBuffer,
                                  QWORD *pnRead) {
  ASSERT(nBuffer != 0 && nBuffer != SIZE_UNKNOWN);
  ASSERT(pBuffer != NULL && AfxIsValidAddress(pBuffer, (UINT_PTR)nBuffer));

  // Find first file
  CVirtualFile::const_iterator i =
      std::find_if(m_oFile.begin(), m_oFile.end(), bind2nd(Greater(), nOffset));
  if (i != m_oFile.begin())
    --i;
...
BOOL CFragmentedFile::VirtualWrite(QWORD nOffset, const char *pBuffer,
...
BOOL CFragmentedFile::EnsureWrite() {
  ...
  return (std::count_if(m_oFile.begin(), m_oFile.end(), EnsureWriter()) ==
          (int)m_oFile.size());
}
