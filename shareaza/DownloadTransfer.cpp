...
CDownloadTransfer::CDownloadTransfer(CDownloadSource *pSource,
    ...
    : CTransfer(nProtocol), m_pDlPrev(NULL), m_pDlNext(NULL), m_nQueuePos(0ul),
      m_nQueueLen(0ul), m_nDownloaded(0ull), m_bWantBackwards(false),
      m_bRecvBackwards(false), m_pDownload(pSource->m_pDownload),
      m_pSource(pSource), m_tSourceRequest(0ul) {
...
void CDownloadTransfer::DrawStateBar(CDC *pDC, CRect *prcBar, COLORREF crFill,
                                     bool bTop) const {
  CFragmentBar::DrawStateBar(pDC, prcBar, m_pDownload->m_nSize, m_nOffset,
                             m_nLength, crFill, bTop);
    ...
    if (pItr != pEnd) {
      ...
      for (++pItr; pItr != pEnd; ++pItr) {
        if (pItr->begin() == nOffset + nLength)
          ...
          CFragmentBar::DrawStateBar(pDC, prcBar, m_pDownload->m_nSize, nOffset,
                                     nLength, crFill, bTop);
...
void CDownloadTransfer::Boost() { m_mInput.pLimit = m_mOutput.pLimit = NULL; }
  ...
  case dtsQueued:
    ...
    if (!bLong) {
      ...
      str.Format((m_nQueueLen ? _T("%s %s %i %s %i") : _T("%s %s #%i")),
                 (LPCTSTR)strQueued, (LPCTSTR)m_sQueueName, m_nQueuePos,
                 (LPCTSTR)strOf, m_nQueueLen);
    } else {
      str.Format((m_nQueueLen ? _T("%s %i %s %i") : _T("%s #%i")),
...
BOOL CDownloadTransfer::OnRun() { return CTransfer::OnRun(); }
...
void CDownloadTransfer::SetState(int nState) {
  ...
  if (m_pDownload && m_pDownload->CheckSource(m_pSource)) {
    if (Settings.Downloads.SortSources) { // Proper sort
      ...
      m_pSource->m_nSortOrder =
          StateSortOrder[max(min(nState, 14), 0)]; // Get state sort order

      if (m_pSource->m_nSortOrder >=
          14) { // Don't bother wasting CPU sorting 'dead' sources- Simply send
                // to bottom.
        ...
        if (nState == dtsTorrent &&
            m_nProtocol == PROTOCOL_BT) // Torrent states
        {
          // Choked torrents after queued, requesting = requesting, uninterested
          // near end
        ...
        if (nState == dtsQueued) // Then queue postion
          ...
          m_pSource->m_nSortOrder +=
              ((m_pSource->m_pAddress.S_un.S_un_b.s_b1 << 8) |
      ...
      if (nState == dtsDownloading &&
          m_nState != dtsDownloading) { // Downloading sources go to the top
        m_pDownload->SortSource(m_pSource, TRUE);
      } else if (nState != dtsDownloading &&
                 m_nState == dtsDownloading) { // Sources that have stopped
                                               // downloading go to the bottom.
        m_pDownload->SortSource(m_pSource, FALSE);
      }
    }
  }
...
bool CDownloadTransfer::ChunkifyRequest(QWORD *pnOffset, QWORD *pnLength,
                                        DWORD nChunk, BOOL bVerifyLock) const {
  ASSUME_LOCK(Transfers.m_pSection);
...
blockPair CDownloadTransfer::SelectBlock(const Fragments::List &oPossible,
                                         const std::vector<bool> &pAvailable,
                                         bool bEndGame) const {
  ASSUME_LOCK(Transfers.m_pSection);
  ...
  for (; pItr != pEnd; ++pItr) {
    ...
    if ((!nPart[1] || nBlockBegin != nBlockEnd) && pItr->end() % nBlockSize &&
    ...
    if (oBiggest.size()) {
      ...
      for (QWORD nCount = 0; nCount < nBlockCount; nCount++) {
        ...
        if (nBlock >= pAvailable.size() || pAvailable[(DWORD)nBlock]) {
          ...
          return std::make_pair(nPartStart,
                                min((nPartEnd - nPartStart), nBlockSize));
        }
      }
    }
...
void CDownloadTransfer::CheckPart(QWORD *nPart, QWORD nPartBlock, QWORD *nRange,
                                  QWORD &nRangeBlock, QWORD *nBestRange) const {
