...
#define ReadValueOrFail(hFile, nID, nRead, nValue, nFile)                      \
  if (!ReadFile(hFile, &nID, 4, &nRead, NULL) || nRead != 4 || nID != nValue)  \
    return false;
...
bool PropGetValue(IPropertyStore *pStore, const PROPERTYKEY &key,
                  CString &val) {
  val.Empty();
...
bool CLibraryBuilderInternals::ExtractMetadata(DWORD nIndex,
                                               const CString &strPath,
                                               HANDLE hFile) {
  CString strType = PathFindExtension(strPath);
  strType.MakeLower();

  if (strType == _T(".mp3") || strType == _T(".mpc") || strType == _T(".mpp") ||
      strType == _T(".mp+")) {
    ...
    if (!ReadFLV(nIndex, hFile))
      return true;
  } else if (strType == _T(".asf") || strType == _T(".wma") ||
             strType == _T(".wmv")) {
    ...
    if (ReadOGG(nIndex, hFile))
      return true;
  } else if (strType == _T(".ape") || strType == _T(".mac") ||
             strType == _T(".apl")) {
    ...
    return ReadPDF(nIndex, hFile, strPath);
             ...
             strType == _T(".emulecollection") || strType == _T(".bz2")) {
  ...
  } else if (strType == _T(".djvu") || strType == _T(".djv")) {
...
bool CLibraryBuilderInternals::ExtractProperties(DWORD nIndex,
                                                 const CString &strPath) {
  bool bSuccess = false;
    ...
    if (theApp.GetPropertyStoreFromParsingName(CT2W(strPath), &pStore)) {
      ...
      if (PropGetValue(pStore, PKEY_PerceivedType, nType)) {
        ...
        case PERCEIVED_TYPE_DOCUMENT:
          ...
          if (pXML) {
            LONG nPages;
            if (PropGetValue(pStore, PKEY_Document_PageCount, nPages) &&
                nPages > 0) {
              CString strItem;
        ...
        case PERCEIVED_TYPE_IMAGE:
          ...
          if (pXML) {
            DWORD nWidth;
            if (PropGetValue(pStore, PKEY_Image_HorizontalSize, nWidth) &&
                nWidth) {
              CString strItem;
            ...
            DWORD nHeight;
            if (PropGetValue(pStore, PKEY_Image_VerticalSize, nHeight) &&
                nHeight) {
              CString strItem;
            ...
            DWORD nBitDepth;
            if (PropGetValue(pStore, PKEY_Image_BitDepth, nBitDepth) &&
                nBitDepth)
        ...
        case PERCEIVED_TYPE_AUDIO:
          ...
          if (pXML) {
            DWORD nSampleRate;
            if (PropGetValue(pStore, PKEY_Audio_SampleRate, nSampleRate) &&
                nSampleRate) {
              CString strItem;
            ...
            DWORD nBitrate;
            if (PropGetValue(pStore, PKEY_Audio_EncodingBitrate, nBitrate) &&
                nBitrate) {
              bool bVariableBitRate = false;
              PropGetValue(pStore, PKEY_Audio_IsVariableBitRate,
                           bVariableBitRate);

              CString strItem;
              strItem.Format(_T("%lu%s"), nBitrate / 1000,
                             (bVariableBitRate ? _T("~") : _T("")));
              pXML->AddAttribute(_T("bitrate"), strItem);
            }

            QWORD nContentLength;
            if (PropGetValue(pStore, PKEY_Media_Duration, nContentLength) &&
                nContentLength) {
            ...
            DWORD nChannelCount;
            if (PropGetValue(pStore, PKEY_Audio_ChannelCount, nChannelCount) &&
                nChannelCount) {
              CString strItem;
        ...
        case PERCEIVED_TYPE_VIDEO:
          ...
          if (pXML) {
            DWORD nVideoWidth;
            if (PropGetValue(pStore, PKEY_Video_FrameWidth, nVideoWidth) &&
                nVideoWidth) {
              CString strItem;
            ...
            DWORD nVideoHeight;
            if (PropGetValue(pStore, PKEY_Video_FrameHeight, nVideoHeight) &&
                nVideoHeight) {
              CString strItem;
            ...
            if (PropGetValue(pStore, PKEY_Video_FourCC, nFourCC)) {
              CString strItem;
              strItem.Format(_T("%c%c%c%c"), (TCHAR)LOBYTE(LOWORD(nFourCC)),
                             (TCHAR)HIBYTE(LOWORD(nFourCC)),
                             (TCHAR)LOBYTE(HIWORD(nFourCC)),
                             (TCHAR)HIBYTE(HIWORD(nFourCC)));
              pXML->AddAttribute(_T("codec"), strItem);
            }

            DWORD nFrameRate;
            if (PropGetValue(pStore, PKEY_Video_FrameRate, nFrameRate) &&
                nFrameRate) {
              CString strItem;
      ...
      if (pXML) {
        ...
        if (PropGetValue(pStore, PKEY_Comment, strDescription))
          pXML->AddAttribute((nType == PERCEIVED_TYPE_DOCUMENT)
                                 ? _T("comments")
                                 : _T("description"),
                             strDescription);
        ...
        if (PropGetValue(pStore, PKEY_Author, strArtist))
          pXML->AddAttribute(
              (nType == PERCEIVED_TYPE_VIDEO || nType == PERCEIVED_TYPE_AUDIO)
                  ? _T("artist")
                  : _T("author"),
              strArtist);
...
bool CLibraryBuilderInternals::CopyID3v1Field(CXMLElement *pXML,
                                              LPCTSTR pszAttribute,
                                              CString strValue) {
  strValue.Trim();
  if (strValue.IsEmpty())
    return false;
...
bool CLibraryBuilderInternals::ReadID3v2(DWORD nIndex, HANDLE hFile) {
  ...
  } else if ((pHeader.nFlags & ID3V2_EXTENDEDHEADER) &&
             pHeader.nMajorVersion == 4) {
  ...
  while (nBuffer) {
    ...
    if (pHeader.nMajorVersion > 2) {
      ...
      nFrameSize =
          (pFrame->nSize[0] << 16) | (pFrame->nSize[1] << 8) | pFrame->nSize[2];
    }
    ...
    } else if (strcmp(szFrameTag, "TPE1") == 0 ||
               strcmp(szFrameTag, "TP1") == 0 ||
               strcmp(szFrameTag, "TPE2") == 0 ||
               strcmp(szFrameTag, "TP2") == 0) {
      CopyID3v2Field(pXML.get(), _T("artist"), pBuffer, nFrameSize);
    } else if (strcmp(szFrameTag, "TOPE") == 0 ||
               strcmp(szFrameTag, "TOA") == 0) {
      CopyID3v2Field(pXML.get(), _T("origArtist"), pBuffer, nFrameSize);
    } else if (strcmp(szFrameTag, "TALB") == 0 ||
               strcmp(szFrameTag, "TAL") == 0) {
      CopyID3v2Field(pXML.get(), _T("album"), pBuffer, nFrameSize);
    } else if (strcmp(szFrameTag, "TOAL") == 0 ||
               strcmp(szFrameTag, "TOT") == 0) {
      CopyID3v2Field(pXML.get(), _T("origAlbum"), pBuffer, nFrameSize);
    } else if (strcmp(szFrameTag, "TRCK") == 0 ||
               strcmp(szFrameTag, "TRK") == 0) {
      CopyID3v2Field(pXML.get(), _T("track"), pBuffer, nFrameSize);
    } else if (pHeader.nMajorVersion < 4 && (strcmp(szFrameTag, "TYER") == 0 ||
                                             strcmp(szFrameTag, "TYE") == 0)) {
      CopyID3v2Field(pXML.get(), _T("year"), pBuffer, nFrameSize);
    } else if (strcmp(szFrameTag, "COMM") == 0 ||
               strcmp(szFrameTag, "COM") == 0) {
      if (CopyID3v2Field(pXML.get(), _T("description"), pBuffer, nFrameSize,
                         true)) {
    ...
    } else if (strcmp(szFrameTag, "TLEN") == 0 ||
               strcmp(szFrameTag, "TLE") == 0) {
      if (CopyID3v2Field(pXML.get(), _T("seconds"), pBuffer, nFrameSize)) {
        ...
        if (_stscanf(strMS, _T("%d"), &nMS) == 1) {
          ...
          pXML->AddAttribute(_T("seconds"), strMS);
               ...
               strcmp(szFrameTag, "TCR") == 0) {
      CopyID3v2Field(pXML.get(), _T("copyright"), pBuffer, nFrameSize);
    } else if (strcmp(szFrameTag, "TCON") == 0 ||
               strcmp(szFrameTag, "TCO") == 0) {
      if (CopyID3v2Field(pXML.get(), _T("genre"), pBuffer, nFrameSize)) {
        ...
        for (;;) {
          ...
          if (strValue.CompareNoCase(_T("RX")) == 0) {
            ...
            strValue = _T("Cover");
          } else if (_stscanf(strValue, _T("%i"), &nGenre) == 1 &&
                     nGenre >= 0 && nGenre < ID3_GENRES) {
    ...
    } else if (strcmp(szFrameTag, "TENC") == 0 ||
               strcmp(szFrameTag, "TEN") == 0) {
      CopyID3v2Field(pXML.get(), _T("encodedby"), pBuffer, nFrameSize);
    } else if (strcmp(szFrameTag, "TSSE") == 0 ||
               strcmp(szFrameTag, "TSS") == 0) {
      CopyID3v2Field(pXML.get(), _T("encodedby"), pBuffer, nFrameSize);
    } else if (strcmp(szFrameTag, "TCOM") == 0 ||
               strcmp(szFrameTag, "TCM") == 0) {
      CopyID3v2Field(pXML.get(), _T("composer"), pBuffer, nFrameSize);
    } else if (strcmp(szFrameTag, "WXXX") == 0 ||
               strcmp(szFrameTag, "WXX") == 0) {
...
bool CLibraryBuilderInternals::CopyID3v2Field(CXMLElement *pXML,
                                              LPCTSTR pszAttribute,
                                              BYTE *pBuffer, DWORD nLength,
                                              bool bSkipLanguage) {
  ...
  while (nOffset < nLength) {
    if (nEncoding == 0) {
    ...
    } else if (nEncoding == 1 && ((nLength - nOffset) & 1) == 0 &&
               nLength - nOffset >= 2) {
      ...
      if (pBuffer[0] == 0xFF && pBuffer[1] == 0xFE) {
        ...
        for (; nChar < nNewLength; nChar++, nOut++) {
          pszOutput[nOut] = (TCHAR)pBuffer[nOffset + nChar * 2 + 0] |
                            ((TCHAR)pBuffer[nOffset + nChar * 2 + 1] << 8);
          if (pszOutput[nOut] == 0) {
            nOffset += (nOut + 1) * 2;
            break;
          }
        ...
        DWORD nOut = 0, nChar = 0;
        for (; nChar < nNewLength; nChar++, nOut++) {
          pszOutput[nOut] = (TCHAR)pBuffer[nOffset + nChar * 2 + 1] |
                            ((TCHAR)pBuffer[nOffset + nChar * 2 + 0] << 8);
          if (pszOutput[nOut] == 0) {
            nOffset += (nOut + 1) * 2;
            break;
          }
      ...
      nOffset +=
          (DWORD)strnlen((LPCSTR)pBuffer + nOffset, nLength - nOffset) + 1;
    }
  ...
  if (_tcsnicmp(strResult, L"musicbrainz ", 12) == 0) {
    ...
    // "Album Artist", "Album Artist Sortname", "Album Release Country",
    // "Non-Album" ToDo: find field names for mbtrmid, mbuniquefileid and cddb
    return true;
...
//
bool CLibraryBuilderInternals::ScanMP3Frame(CXMLElement *pXML, HANDLE hFile,
                                            DWORD nIgnore) {
  // Bitrate index
  static DWORD nBitrateTable[16][5] = {
      ...
      // The first row with zeros is for "free" bitrate (the constant bitrate
  ...
  static DWORD nFrequencyTable[4][4] = {// Row data:
                                        // MPEG1	?	MPEG2  	MPEG2.5
                                        {11025, 0, 22050, 44100},
                                        {12000, 0, 24000, 48000},
                                        {8000, 0, 16000, 32000},
  ...
  static const CString strSoundType[4] = {_T("Stereo"), _T("Joint Stereo"),
                                          _T("Dual Channel"),
                                          _T("Single Channel")};
  ...
  for (DWORD nSeek = 0; bVariable || (nFrameCount < 16 && nSeek < 4096 * 2);
       nSeek++) {
    // "frame sync"
    ...
    if ((nHeader & 0xFFE00000) == 0xFFE00000) {
      ...
      BYTE nBitIndex =
          (BYTE)((nHeader & 0x0000F000) >> 12); //      1111 0000 0000 0000
      BYTE nFreqIndex =
          (BYTE)((nHeader & 0x00000C00) >> 10); //           1100 0000 0000
      bool bPadding =
          (nHeader & 0x00000200) ? true : false; //           0010 0000 0000
      BYTE nChannels =
          (BYTE)((nHeader & 0x000000C0) >> 6); //                1100 0000
      bool bCopyRight =
          (nHeader & 0x00000008) ? true : false; //                     1000
      ...
      if (nLayer) {
        ...
        nFrameCount++;
                 ...
                 0) // reset base values if it was the first frame
      {
        nBaseBitrate = nBaseFrequency = 0;
      }

      if (SetFilePointer(hFile, nFrameSize - 4, NULL, FILE_CURRENT) ==
  ...
  DWORD nTotalTime =
      (DWORD)((__int64)nFrameCount * (__int64)nFrameTime / 100000);
...
bool CLibraryBuilderInternals::ReadVersion(DWORD nIndex, LPCTSTR pszPath) {
  ...
  if (pXML.get()) {
    if (DWORD dwSize = GetFileVersionInfoSize((LPTSTR)pszPath, &dwSize)) {
      ...
      if (pBuffer.get() &&
          GetFileVersionInfo((LPTSTR)pszPath, NULL, dwSize, pBuffer.get())) {
        const DWORD nLangId = GetBestLanguageId(pBuffer.get());

        bool bOur = false;
        bOur |= CopyVersionField(pXML.get(), _T("title"), pBuffer.get(),
                                 _T("ProductName"), nLangId);
        CopyVersionField(pXML.get(), _T("version"), pBuffer.get(),
                         _T("ProductVersion"), nLangId, true);
        bOur |= CopyVersionField(pXML.get(), _T("fileDescription"),
                                 pBuffer.get(), _T("FileDescription"), nLangId);
        CopyVersionField(pXML.get(), _T("fileVersion"), pBuffer.get(),
                         _T("FileVersion"), nLangId, true);
        bOur |=
            CopyVersionField(pXML.get(), _T("originalFileName"), pBuffer.get(),
                             _T("OriginalFilename"), nLangId);
        bOur |= CopyVersionField(pXML.get(), _T("company"), pBuffer.get(),
                                 _T("CompanyName"), nLangId);
        bOur |= CopyVersionField(pXML.get(), _T("copyright"), pBuffer.get(),
                                 _T("LegalCopyright"), nLangId);
        bOur |= CopyVersionField(pXML.get(), _T("comments"), pBuffer.get(),
                                 _T("comments"), nLangId);

        // LPCTSTR pszExt = PathFindExtension( pszPath );
        // if ( bOur && pszExt && _tcscmp( pszExt, L".exe" ) == 0 /*&&
        // ValidateManifest( pszPath )*/ )
        //{
        //	// TODO: mark the file as validated OR otherwise submit
        //corrupted OR delete metadata?
        // }
      }
    }
    LibraryBuilder.SubmitMetadata(nIndex, CSchema::uriApplication,
                                  pXML.release());
  }
  return true;
}
...
bool CLibraryBuilderInternals::CopyVersionField(CXMLElement *pXML,
                                                LPCTSTR pszAttribute,
                                                BYTE *pBuffer, LPCTSTR pszKey,
                                                DWORD nLangId,
                                                bool bCommaToDot) {
  ...
  if (nPos != -1) {
    ...
    if (!bEndOk && strValue.GetLength() > nPos + 8 &&
        strValue.GetAt(nPos + 8) != ' ') {
      // ShareazaPlus etc.
    } else
      bEndOk = true;
...
CString CLibraryBuilderInternals::GetVersionKey(BYTE *pBuffer, LPCTSTR pszKey,
                                                DWORD nLangId) {
  CString strKey;
  strKey.Format(L"\\StringFileInfo\\%04x%04x\\", nLangId & 0x0000FFFF,
                (nLangId & 0xFFFF0000) >> 16);
  strKey += pszKey;

  BYTE *pValue = NULL;
  DWORD dwSize = 0;

  if (!VerQueryValue(pBuffer, (LPTSTR)(LPCTSTR)strKey, (void **)&pValue,
                     (UINT *)&dwSize))
    return CString();
...
DWORD CLibraryBuilderInternals::GetBestLanguageId(LPVOID pBuffer) {
  ...
  VerQueryValue(pBuffer, L"\\VarFileInfo\\Translation", &pTranslation,
                &nLength);
  ...
  if (!GetLanguageId(pTranslation, nLength, GetUserDefaultLangID(), nLangCode,
                     false)) {
    if (!GetLanguageId(pTranslation, nLength, GetSystemDefaultLangID(),
                       nLangCode, true)) {
      if (!GetLanguageId(pTranslation, nLength,
                         MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), nLangCode,
                         true)) {
        if (!GetLanguageId(pTranslation, nLength,
                           MAKELANGID(LANG_ENGLISH, SUBLANG_NEUTRAL), nLangCode,
                           true))
...
bool CLibraryBuilderInternals::GetLanguageId(LPVOID pBuffer, UINT nSize,
                                             WORD nLangId, DWORD &nId,
                                             bool bOnlyPrimary) {
  LPWORD pData = NULL;
  for (pData = (LPWORD)pBuffer; (LPBYTE)pData < ((LPBYTE)pBuffer) + nSize;
       pData += 2) {
    if (*pData == nLangId) {
      nId = *(DWORD *)pData;
      return true;
...
CString CLibraryBuilderInternals::GetSummaryField(MSIHANDLE hSummaryInfo,
                                                  UINT nProperty) {
  CString strValue;
  UINT nPropType = VT_LPSTR;
  DWORD dwSize = 0;

  if (MsiSummaryInfoGetProperty(hSummaryInfo, nProperty, &nPropType, 0, NULL,
                                L"", &dwSize) == ERROR_MORE_DATA) {
    dwSize++;
    MsiSummaryInfoGetProperty(hSummaryInfo, nProperty, &nPropType, 0, NULL,
                              strValue.GetBuffer(dwSize), &dwSize);
...
#define GetUI24(x)                                                             \
  (((DWORD)((x).b1) << 16) & 0x00ff0000 |                                      \
   ((DWORD)((x).b2) << 8) & 0x0000ff00 | (DWORD)((x).b3) & 0x000000ff)
...
typedef struct {
  ...
  UI32 DataOffset; // Offset in bytes from start of file to start of body (that
                   // is, size of header)
} FLV_HEADER;
...
typedef struct {
  UI8 TagType;    // Type of this tag. Values are: 8 - audio, 9 - video, 18 -
                  // script data, all others - reserved
  UI24 DataSize;  // Length of the data in the Data field
  UI24 Timestamp; // Time in milliseconds at which the data in this tag applies.
                  // This value is relative to the first tag in the FLV file,
                  // which always has a timestamp of 0.
  UI8 TimestampExtended; // Extension of the Timestamp field to form a SI32
                         // value. This field represents the upper 8 bits, while
                         // the previous Timestamp field represents the lower 24
                         // bits of the time in milliseconds.
...
bool CLibraryBuilderInternals::ReadFLVVariable(HANDLE hFile, DWORD &nRemaning,
                                               VARIANT &varData,
                                               CXMLElement *pXML) {
  DWORD nRead;
  ...
  if (nRemaning < sizeof(Type))
    return false;
  if (!ReadFile(hFile, &Type, sizeof(Type), &nRead, NULL) ||
      nRead != sizeof(Type))
    return false;
...
bool CLibraryBuilderInternals::ReadFLVDouble(HANDLE hFile, DWORD &nRemaning,
                                             double &dValue) {
  DWORD nRead;
...
bool CLibraryBuilderInternals::ReadFLVBool(HANDLE hFile, DWORD &nRemaning,
                                           bool &bValue) {
  DWORD nRead;
...
bool CLibraryBuilderInternals::ReadFLVString(HANDLE hFile, DWORD &nRemaning,
                                             BOOL bLong, CStringA &strValue) {
  DWORD nRead;
  ...
  if (bLong) {
    ...
    if (nRemaning < sizeof(StringSize))
      return false;
    if (!ReadFile(hFile, &StringSize, sizeof(StringSize), &nRead, NULL) ||
        nRead != sizeof(StringSize))
      return false;
  ...
  if (nRemaning < nStringSize)
    return false;
  BOOL bResult = ReadFile(hFile, strValue.GetBuffer(nStringSize), nStringSize,
                          &nRead, NULL);
...
bool CLibraryBuilderInternals::ReadFLVEMCA(HANDLE hFile, DWORD &nRemaning,
                                           CXMLElement *pXML) {
  DWORD nRead;
  ...
  if (nRemaning < sizeof(Fields))
    return false;
  if (!ReadFile(hFile, &Fields, sizeof(Fields), &nRead, NULL) ||
      nRead != sizeof(Fields))
    return false;
  ...
  for (DWORD i = 0; i < nFields; ++i) {
    ...
    TRACE("FLV EMCA %2u : %s = \"%s\"\n", i + 1, (LPCSTR)strName,
          (LPCSTR)CW2A((LPCWSTR)varDebug.bstrVal));
#endif
...
bool CLibraryBuilderInternals::ReadFLV(DWORD nIndex, HANDLE hFile) {
  ...
  if (!ReadFile(hFile, &header, sizeof(header), &nRead, NULL) ||
      nRead != sizeof(header))
    return false;
  if (header.Signature1 != 'F' || header.Signature2 != 'L' ||
      header.Signature3 != 'V' || header.DataOffset < sizeof(header))
    return false;
  TRACE("FLV version: %d\n", header.Version);
  DWORD nDataOffset = GetUI32(header.DataOffset);
  if (SetFilePointer(hFile, nDataOffset, NULL, FILE_BEGIN) ==
      INVALID_SET_FILE_POINTER)
    return false;
  ...
  CAutoPtr<CXMLElement> pXML(
      new CXMLElement(NULL, bVideo ? _T("video") : _T("audio")));
  if (!pXML)
    // Out of memory
    return false;
  ...
  for (; !bMetadata; ++nTags) {
    ...
    if (!ReadFile(hFile, &PreviousTagSize, sizeof(PreviousTagSize), &nRead,
                  NULL) ||
        nRead != sizeof(PreviousTagSize))
      return false;
    ...
    const DWORD nNextTagPosition =
        SetFilePointer(hFile, 0, NULL, FILE_CURRENT) + nDataSize;
    nLastTagSize = nDataSize + sizeof(tag_header);

    if (tag_header.TagType == FLVTAG_SCRIPT) {
      TRACE("FLV found script tag.\n");

      for (DWORD nRemaning = nDataSize;
           nRemaning > 3;) // Except last SCRIPTDATAOBJECTEND(UI24) == 0x000009
      {
        CComVariant var;
  ...
  if (bMetadata) {
    ...
    LibraryBuilder.SubmitMetadata(
        nIndex, bVideo ? CSchema::uriVideo : CSchema::uriAudio, pXML.Detach());
  }
...
bool CLibraryBuilderInternals::ReadASF(DWORD nIndex, HANDLE hFile) {
  ...
  for (;;) {
    ...
    if (pGUID == asfProperties1) {
      ...
      if (!ReadFile(hFile, &nContentLength, sizeof(nContentLength), &nRead,
                    NULL))
        return false;
  ...
  LibraryBuilder.SubmitMetadata(
      nIndex, bVideo ? CSchema::uriVideo : CSchema::uriAudio, pXML.release());
  return true;
}
...
bool CLibraryBuilderInternals::ReadMPEG(DWORD nIndex, HANDLE hFile) {
  ...
  LPCTSTR pszFPS[] = {_T("23.976"), _T("24"), _T("25"),    _T("29.97"),
                      _T("30"),     _T("50"), _T("59.94"), _T("60")};
...
BYTE *CLibraryBuilderInternals::ReadOGGPage(HANDLE hFile, DWORD &nBuffer,
                                            BYTE nFlags, DWORD nSequence,
                                            DWORD nMinSize) {
...
bool CLibraryBuilderInternals::ReadOGGString(BYTE *&pOGG, DWORD &nOGG,
                                             CString &str) {
  if (nOGG < 4)
    return false;
...
bool CLibraryBuilderInternals::ReadAPE(DWORD nIndex, HANDLE hFile,
                                       bool bPreferFooter) {
    ...
    if (strKey.GetLength() && strValue.GetLength()) {
      ...
      if (strKey == L"title") {
        ...
        if (strValue.Find(L"/") != -1) {
          ...
          strTotalTracksField =
              strValue.Right(strValue.GetLength() - intSlashPosition - 1);
        } else
        ...
        pXML->AddAttribute(L"genre", strValue);
      } else if (strKey.Find(L" url") >
                 0) // are there any tag fields containing that?
      {
        pXML->AddAttribute(L"link", strValue);
      } else if (strKey == L"composer") {
        pXML->AddAttribute(L"composer", strValue);
      ...
      } else if (strKey == L"lyrics" || strKey == L"unsyncedlyrics" ||
                 strKey == L"unsynced lyrics") {
        // pXML->AddAttribute( L"unsyncedlyrics", strValue );
      } else if (strValue != strArtist &&
                 (strKey == L"musicbrainz album artist" ||
                  strKey == L"musicbrainz albumartist" ||
                  strKey == L"album artist" || strKey == L"albumartist")) {
  ...
  if (bHasDiscField) {
    if (bHasTotalDiscsField)
      pXML->AddAttribute(_T("disc"),
                         strDiscField + _T("/") + strTotalDiscsField);
    else
  ...
  if (bHasTrackField == true) {
    if (bHasTotalTracksField)
      pXML->AddAttribute(_T("track"),
                         strTrackField + _T("/") + strTotalTracksField);
    else
  ...
  // We checked if the file size is bigger than the footer, so no check is
  // needed here for the nRead bytes.
  ...
  if (bMAC && pAPE.nVersion >= APE2_VERSION) {
    ...
    ZeroMemory(&pAPE,
               nValidSize); // just in case if someone messes up the code below
  }

  bool bValidSignature = bMAC || bMPC;

  if ((int)nRead != nValidSize || !bValidSignature ||
      pNewAPE.nSampleRate == 0 || bPreferFooter) {
  ...
  if (bNewAPE)
    ...
    nSamplesPerFrame =
        ((pNewAPE.nVersion >= 3900) ||
         (pNewAPE.nVersion >= 3800 && pNewAPE.nCompressionLevel == 4000))
            ? 73728
            : 9216;
  ...
  if (pNewAPE.nTotalFrames != 0)
    nSamples = (pNewAPE.nTotalFrames - 1) * nSamplesPerFrame +
               pNewAPE.nFinalFrameBlocks;
  ...
  if (bNewAPE) {
    ...
    nBitsPerSample = (pNewAPE.nFormatFlags & 1)   ? 8
                     : (pNewAPE.nFormatFlags & 8) ? 24
                                                  : 16;
  }
...
bool CLibraryBuilderInternals::ReadAVI(DWORD nIndex, HANDLE hFile) {
  ...
  ReadValueOrFail(
      hFile, nID, nRead, FCC('RIFF'),
      nIndex) if (!ReadFile(hFile, &nID, 4, &nRead, NULL)) return false;
  if (nRead != 4)
    return false;
  ...
  ReadValueOrFail(hFile, nID, nRead, FCC('LIST'), nIndex)
      ...
      if (!ReadFile(hFile, &nNextOffset, sizeof(DWORD), &nRead,
                    NULL)) return false;
  if (nRead != 4)
    return false;
  ...
  ReadValueOrFail(hFile, nID, nRead, FCC('hdrl'), nIndex)
      ReadValueOrFail(hFile, nID, nRead, FCC('avih'), nIndex)
          ...
          if (!ReadFile(hFile, &nID, 4, &nRead, NULL)) return false;
  if (nRead != 4)
    return false;
  ...
  ReadValueOrFail(hFile, nID, nRead, FCC('vids'),
                  nIndex) if (!ReadFile(hFile,
                                        (BYTE *)strCodec.GetBufferSetLength(4),
                                        4, &nRead, NULL)) return false;
  if (nRead != 4)
    return false;
  ...
  do {
    nPos += nNextOffset;
    if (SetFilePointer(hFile, nPos, NULL, FILE_BEGIN) ==
        INVALID_SET_FILE_POINTER)
      break;
    ReadDwordOrBreak(hFile, nID, nRead) nNextOffset = 0;
    ReadDwordOrBreak(hFile, nNextOffset, nRead) nPos =
        SetFilePointer(hFile, 0, NULL, FILE_CURRENT);
    if (nID == FCC('LIST')) {
      ReadDwordOrBreak(hFile, nID, nRead) if (nID == FCC('movi')) {
        bMoviFound = true;
      }
      else if (nID == FCC('INFO')) {
        bInfoFound = true;
        for (;;) {
          ReadDwordOrBreak(hFile, nID,
                           nRead) if ((CHAR)(nID & 0x000000FF) != 'I') break;

          DWORD nDataLength = 0;
          ReadDwordOrBreak(hFile, nDataLength, nRead)

              auto_array<CHAR>
                  strData(new CHAR[nDataLength]);
          if (!ReadFile(hFile, (BYTE *)strData.get(), nDataLength, &nRead,
                        NULL))
            break;
          ...
          case FCC('IDIT'): // ???
          case FCC(
              'ICRD'): // Creation date. List dates in year-month-day format,
          ...
          if (nDataLength % 2 !=
              0) // Skip padding zero byte if the data size is not even
          {
...
bool CLibraryBuilderInternals::ReadPDF(DWORD nIndex, HANDLE hFile,
                                       LPCTSTR pszPath) {
  DWORD nOffset, nCount, nCountStart, nPages = 0, nOffsetPrev, nFileLength = 0,
                                      nVersion;
  ...
  if (strLine.IsEmpty())
    strLine = ReadPDFLine(hFile, false, true);
  nCount = 0;
  while (!strLine.IsEmpty() && nCount < 9 &&
         nVersion > 1) // read dictionary entries only from 8 lines max
  {
  ...
  if (bLinearized) {
    ...
    // if file length is not the same as in L data, the document is treated as
    // non-linearized
    DWORD nError;
    if (SetFilePointer(hFile, nFileLength, NULL, FILE_BEGIN) ==
            INVALID_SET_FILE_POINTER &&
  ...
  // The main part: an array is filled with the locations of objects from
  // reference tables
  ...
  auto_ptr<CXMLElement> pXML(
      new CXMLElement(NULL, bBook ? _T("book") : _T("wordprocessing")));
  ...
  /*	// Get XMP metadata; Not implemented, we should prefer XMP if the file
     creation time was less
      ...
      if ( nOffsetRoot != 0 )
      {
              ...
              if ( strLine == strSeek )
              {
                      if ( ReadLine( hFile, (LPCTSTR)_T("<") ).IsEmpty() &&
                               ...
                               ReadLine( hFile, (LPCTSTR)_T("/") ).MakeLower()
     == _T("type") && ReadLine( hFile, (LPCTSTR)_T("/") ).MakeLower() ==
     _T("catalog") )
                      {
                              ...
                              while ( !strLine.IsEmpty() )
                              {
                                      ...
                                      {
                                              if ( _stscanf( strLine.Mid( nData
     + 1 ), _T("%lu"), &nOffsetMeta ) != 1 ) return false;
                                      }
                                      strLine = ReadLine( hFile, true );
                              }
                      }
      ...
      if ( nOffsetMeta != 0 )
      {
              ...
              strLine = ReadLine( hFile ); //<</Type /Matadata /Subtype /XML
     /Length xxx strLine = ReadLine( hFile ); //stream strLine = ReadLine( hFile
     ); //XML metadata strLine = ReadLine( hFile ); //endstream strLine =
     ReadLine( hFile ); //endobj
      }*/
    ...
    if (strLine.CompareNoCase(strSeek) == 0) {
      ...
      while (!strLine.IsEmpty()) {
        ...
        if (strEntry.CompareNoCase(_T("title")) == 0)
          ...
          pXML->AddAttribute(bBook ? _T("publisher") : _T("copyright"),
                             strDecodedLine);
...
CString CLibraryBuilderInternals::DecodePDFText(CString strInput) {
  ...
  if (bHex) {
    ...
    if (bWide) {
      for (DWORD nHex = 0; nHex < nByte / 2; nHex++, p += 4) {
        pByte[nHex] = (WCHAR)unhex(p[0]) << 12 | (WCHAR)unhex(p[1]) << 8 |
                      (WCHAR)unhex(p[2]) << 4 | (WCHAR)unhex(p[3]);
      }
      pByte[nByte / 2] = 0;
    } else {
      for (DWORD nHex = 0; nHex < nByte; nHex++, p += 2) {
        pByte[nHex] = (WCHAR)unhex(p[0]) << 4 | (WCHAR)unhex(p[1]);
      }
      pByte[nByte] = 0;
    }
  } else {
      ...
      if (nTemp == '\\' && nChar + 1 < nByte) {
        ...
        if (nTemp == 't')
          ...
          if (nChar + 3 < nByte &&
            ...
            nShift += 2;
            nChar += 2;
          } else if (nChar + 2 < nByte && _stscanf(strInput.Mid(nChar + 1, 2),
                                                   _T("%o"), &nWChar) == 1) {
            pByte[nChar - nShift] = WCHAR(nWChar);
            nShift++;
            nChar++;
          ...
          // backslash with a space is ignored--the backslash at the end just
          // breaks a line (we replaced separators while reading a file)
  ...
  if (bWide || bCharsToMove == 0) {
    CopyMemory(strResult.GetBuffer(nByte), pByte.get() + bCharsToMove,
               (nByte - bCharsToMove) * sizeof(WCHAR));
    ...
    for (DWORD nPos = 0; nPos < nByte; nPos++) {
      pszDest[nPos] = pByte[nPos * 2 + 1 + bCharsToMove] |
  ...
  do {
    ...
    if (nEscapeStart != -1) {
      ...
      if (nEscapeEnd != -1)
        strResult =
            strResult.Left(nEscapeStart - 1) + strResult.Mid(nEscapeEnd + 1);
      else
...
CString CLibraryBuilderInternals::ReadPDFLine(HANDLE hFile, bool bReverse,
                                              bool bComplex, bool bSplitter) {
  bool bGt = false, bLt = false;
  DWORD nRead, nLength;
  TCHAR cChar = 0;
  CString str;

  for (nLength = 0; ReadFile(hFile, &cChar, 1, &nRead, NULL) && nRead == 1 &&
                    nLength++ < 4096;) {
    if (bReverse)
...
bool CLibraryBuilderInternals::ReadCHM(DWORD nIndex, HANDLE hFile,
                                       LPCTSTR pszPath) {
  CHAR szMagic[4];
  ...
  if (nRead != sizeof(nVersion) || nVersion < 3)
    return false; // In Version 2 files, content section data offset is not
                  // there
  ...
  // Read the last qword from the end of header; it contains content section
  // data offset
  ...
  if (!ReadFile(hFile, &nComprVersion, sizeof(nComprVersion), &nRead, NULL))
    return false;
  if (nRead != sizeof(nComprVersion) ||
      nComprVersion != 2) // Note: MS Reader books has version 3
    return false;
  ...
  for (nPos = 0; ReadFile(hFile, &szByte, 1, &nRead, NULL) &&
                 nPos++ < MAX_LENGTH_ALLOWED;) {
    if (nRead != 1) {
      bCorrupted = true;
      break;
    }
  ...
  if (nCwc > 0) {
    ...
    if (TranslateCharsetInfo((LPDWORD)(DWORD_PTR)charSet, &csInfo,
                             TCI_SRCCODEPAGE))
      nCodePage = csInfo.ciACP;
  }
  SetFilePointer(hFile, nPos, NULL, FILE_BEGIN);

  for (int nCount = 1; nCount < 5 && !bCorrupted;
       nCount++) // nCount may be up to 6
  {
    // Unknown data
    ...
    int nWide =
        MultiByteToWideChar(nCodePage, 0, szMetadata.get(), nData, NULL, 0);
    LPWSTR pszOutput = strLine.GetBuffer(nWide + 1);
    MultiByteToWideChar(nCodePage, 0, szMetadata.get(), nData, pszOutput,
                        nWide);
    pszOutput[nWide] = 0;
...
bool CLibraryBuilderInternals::ReadTorrent(DWORD nIndex, HANDLE /*hFile*/,
                                           LPCTSTR pszPath) {
  CBTInfo oTorrent;
...
bool CLibraryBuilderInternals::ReadDJVU(DWORD nIndex, HANDLE hFile) {
  ...
  BYTE pMagic[8];
  if (!ReadFile(hFile, pMagic, sizeof(pMagic), &nRead, NULL) ||
      nRead != sizeof(pMagic))
    return false;
  ...
  // Document size (MSB)
  DWORD nFileSize;
  if (!ReadFile(hFile, &nFileSize, sizeof(nFileSize), &nRead, NULL) ||
      nRead != sizeof(nFileSize))
    return false;
  ...
  BYTE pType[8];
  if (!ReadFile(hFile, pType, sizeof(pType), &nRead, NULL) ||
      nRead != sizeof(pType))
    return false;
  ...
  if (memcmp(pType, "DJVMDIRM", sizeof(pType)) == 0) {
    ...
    BYTE nFlags;
    if (!ReadFile(hFile, &nFlags, sizeof(nFlags), &nRead, NULL) ||
        nRead != sizeof(nFlags))
      return false;
    ...
    // Files count (MSB)
    WORD nFiles;
    if (!ReadFile(hFile, &nFiles, sizeof(nFiles), &nRead, NULL) ||
        nRead != sizeof(nFiles))
      return false;
    ...
    // Image width (MSB)
    if (!ReadFile(hFile, &nWidth, sizeof(nWidth), &nRead, NULL) ||
        nRead != sizeof(nWidth))
      return false;
    nWidth = _byteswap_ushort(nWidth);

    // Image height (MSB)
    if (!ReadFile(hFile, &nHeight, sizeof(nHeight), &nRead, NULL) ||
        nRead != sizeof(nHeight))
      return false;
    nHeight = _byteswap_ushort(nHeight);

    // Version
    if (!ReadFile(hFile, &nVersion, sizeof(nVersion), &nRead, NULL) ||
        nRead != sizeof(nVersion))
      return false;

    // DPI (LSB)
    if (!ReadFile(hFile, &nDPI, sizeof(nDPI), &nRead, NULL) ||
        nRead != sizeof(nDPI))
      return false;
  } else
    return false;
  ...
  CAutoPtr<CXMLElement> pXML(
      new CXMLElement(NULL, bBook ? _T("wordprocessing") : _T("image")));
  if (!pXML)
    return true;

  pXML->AddAttribute(bBook ? _T("format") : _T("description"),
                     _T("Lizardtech DjVu"));
  ...
  LibraryBuilder.SubmitMetadata(
      nIndex, bBook ? CSchema::uriDocument : CSchema::uriImage, pXML.Detach());
  return true;
}
