...
CDownloadWithSources::CDownloadWithSources()
    : m_nG1SourceCount(0), m_nG2SourceCount(0), m_nEdSourceCount(0),
      m_nHTTPSourceCount(0), m_nBTSourceCount(0), m_nFTPSourceCount(0),
      m_nDCSourceCount(0), m_pXML(NULL) {}
...
bool CDownloadWithSources::HasMetadata() const { return (m_pXML != NULL); }
...
DWORD CDownloadWithSources::GetSourceCount(BOOL bNoPush, BOOL bSane) const {
  ...
  for (POSITION posSource = GetIterator(); posSource;) {
    ...
    if (!bNoPush || !pSource->m_bPushOnly) {
      if (!bSane || ((pSource->m_tAttempt < tNow ||
...
DWORD CDownloadWithSources::GetEffectiveSourceCount() const {
  bool bIsG1Allowed = Settings.Gnutella1.EnableToday ||
                      !Settings.Connection.RequireForTransfers;
  bool bIsG2Allowed = Settings.Gnutella2.EnableToday ||
                      !Settings.Connection.RequireForTransfers;
  bool bIsEdAllowed =
      Settings.eDonkey.EnableToday || !Settings.Connection.RequireForTransfers;
  bool bIsBtAllowed = Settings.BitTorrent.EnableToday ||
                      !Settings.Connection.RequireForTransfers;
  bool bIsDCAllowed =
      Settings.DC.EnableToday || !Settings.Connection.RequireForTransfers;
  ...
  for (POSITION posSource = GetIterator(); posSource;) {
    ...
    if ((pSource->m_nProtocol == PROTOCOL_BT) && // Only counting BT sources
        (pSource->m_tAttempt < tNow ||
         pSource->m_tAttempt - tNow <= 900000) && // Don't count dead sources
        (!pSource->m_bPushOnly ||
         !bNoPush)) // Push sources might not be counted
    {
      nCount++;
    }
  }
...
DWORD CDownloadWithSources::GetED2KCompleteSourceCount() const {
  ...
  for (POSITION posSource = GetIterator(); posSource;) {
    ...
    if ((!pSource->m_bPushOnly) && // Push sources shouldn't be counted since
                                   // you often cannot reach them
        (pSource->m_tAttempt < tNow ||
         pSource->m_tAttempt - tNow <=
             900000) && // Only count sources that are probably active
        (pSource->m_nProtocol == PROTOCOL_ED2K) && // Only count ed2k sources
        (pSource->m_oAvailable.empty() &&
         pSource->IsOnline())) // Only count complete sources
...
BOOL CDownloadWithSources::AddSource(const CShareazaFile *pHit, BOOL bForce) {
  ...
  const bool bHitHasSize =
      (pHit->m_nSize != 0 && pHit->m_nSize != SIZE_UNKNOWN);
  bool bHash = FALSE;
  bool bUpdated = FALSE;
  ...
  if (!bHash && !bForce) {
    if (Settings.General.HashIntegrity)
      return FALSE;
    if (m_sName.IsEmpty() || !bHitHasName)
      return FALSE;
    if (m_nSize == SIZE_UNKNOWN || !bHitHasSize)
      return FALSE;
    if (m_nSize != pHit->m_nSize)
      return FALSE;
    if (m_sName.CompareNoCase(pHit->m_sName))
      return FALSE;
  }
...
BOOL CDownloadWithSources::AddSourceHit(const CQueryHit *pHit, BOOL bForce) {
  ...
  if (Settings.Downloads.Metadata && m_pXML == NULL && pHit->m_pXML &&
      pHit->m_pSchema) {
  ...
  if ( pHit->m_nProtocol == PROTOCOL_ED2K )
  {
      Neighbours.FindDonkeySources( pHit->m_oED2K,
              (IN_ADDR*)pHit->m_oClientID.begin(),
...
BOOL CDownloadWithSources::AddSourceHit(const CMatchFile *pMatchFile,
                                        BOOL bForce) {
  CQuickLock oLock(Transfers.m_pSection);
  ...
  for (const CQueryHit *pHit = pMatchFile->GetHits(); pHit;
       pHit = pHit->m_pNext) {
    if (bForce) {
      // Best already added
      if (pHit != pBestHit) {
...
BOOL CDownloadWithSources::AddSourceED2K(DWORD nClientID, WORD nClientPort,
                                         DWORD nServerIP, WORD nServerPort,
                                         const Hashes::Guid &oGUID) {
  return AddSourceInternal(new CDownloadSource((CDownload *)this, nClientID,
                                               nClientPort, nServerIP,
                                               nServerPort, oGUID));
}

BOOL CDownloadWithSources::AddSourceBT(const Hashes::BtGuid &oGUID,
                                       const IN_ADDR *pAddress, WORD nPort,
                                       BOOL bIgnoreLocalIP) {
  ...
  return AddSourceInternal(
      new CDownloadSource((CDownload *)this, oGUID, pAddress, nPort));
}

BOOL CDownloadWithSources::AddSourceBT(const Hashes::BtGuid &oGUID,
                                       const IN6_ADDR *pAddress, WORD nPort,
                                       BOOL bIgnoreLocalIP) {
  // Unreachable (Push) BT sources should never be added.
  if (Network.IsFirewalledAddress(pAddress, Settings.Connection.IgnoreOwnIP,
                                  bIgnoreLocalIP))
...
BOOL CDownloadWithSources::AddSourceURL(LPCTSTR pszURL, FILETIME *pLastSeen,
                                        int nRedirectionCount, BOOL bFailed,
                                        BOOL bForce, BOOL bPartialSame) {
  ...
  if (pURL.m_pAddress.s_addr == 0x0100007f) {
    if (Settings.Connection.IgnoreOwnIP)
      return FALSE;
  } else if (pURL.m_pAddress.s_addr != INADDR_ANY &&
             pURL.m_pAddress.s_addr != INADDR_NONE) {
    if (Network.IsFirewalledAddress(&pURL.m_pAddress,
  ...
  if (CFailedSource *pBadSource = LookupFailedSource(pszURL)) {
    ...
    int nTotal =
        pBadSource->m_nPositiveVotes + pBadSource->m_nNegativeVotes + 1;
    if (bFailed)
  ...
  return AddSourceInternal(
      new CDownloadSource(static_cast<const CDownload *>(this), pszURL,
                          bHashAuth, pLastSeen, nRedirectionCount));
}
...
int CDownloadWithSources::AddSourceURLs(LPCTSTR pszURLs, BOOL bFailed,
  ...
  for (POSITION pos = oUrls.GetStartPosition(); pos;) {
    ...
    if (AddSourceURL(strURL,
                     (tSeen.dwLowDateTime | tSeen.dwHighDateTime) ? &tSeen
                                                                  : NULL,
                     0, bFailed, 0, bPartialSame)) {
      if (bFailed) {
...
BOOL CDownloadWithSources::AddSourceInternal(CDownloadSource *pSource) {
  ...
  if (pSource->m_nRedirectionCount ==
      0) // Don't check for existing sources if source is a redirection
  {
    bool bDeleteSource = false;
    bool bHTTPSource = pSource->IsHTTPSource() || pSource->IsSSLSource();
    bool bNeedHTTPSource = !bHTTPSource && Settings.Gnutella2.EnableToday &&
    ...
    for (POSITION posSource = GetIterator(); posSource;) {
      ...
      {
        bool bExistingHTTPSource =
            pExisting->IsHTTPSource() || pExisting->IsSSLSource();
        ...
        if ((pExisting->m_nProtocol == pSource->m_nProtocol) ||
          ...
          // We are not downloading so we can replace non-HTTP source with a new
          // one
          if (!bExistingHTTPSource && bHTTPSource) {
    ...
    if (bNeedHTTPSource) {
      ...
      if (strURL.GetLength()) {
        if (CDownloadSource *pG2Source =
                new CDownloadSource((CDownload *)this, strURL)) {
          pG2Source->m_sServer = pSource->m_sServer; // Copy user-agent
          pG2Source->m_tAttempt =
              pSource->m_tAttempt; // Set the same connection delay
  ...
  if (nSourceCount >= Settings.Downloads.SourcesWanted) {
    ...
    for (POSITION posSource = m_pSources.GetTailPosition();
         posSource && nSourceCount >= Settings.Downloads.SourcesWanted;) {
...
#ifdef _DEBUG
          theApp.Message(
              MSG_DEBUG, _T( "Removed extra source %s %s (%I64d seconds old)" ),
              (LPCTSTR)pExisting->m_sURL,
              (LPCTSTR)pExisting->m_oGUID.toString<Hashes::base16Encoding>()
                  .MakeUpper(),
              (*((LONGLONG *)&tNow) - *((LONGLONG *)&pExisting->m_tLastSeen)) /
                  10000000);
#endif
    ...
    for (CSortedSources::const_iterator i = oIdleSources.begin();
         i != oIdleSources.end() &&
         nSourceCount >= Settings.Downloads.SourcesWanted;
         ++i) {
...
#ifdef _DEBUG
      theApp.Message(
          MSG_DEBUG, _T( "Ignored extra source %s %s due sources limit %u" ),
          (LPCTSTR)pSource->m_sURL,
          (LPCTSTR)pSource->m_oGUID.toString<Hashes::base16Encoding>()
              .MakeUpper(),
...
CString CDownloadWithSources::GetSourceURLs(CList<CString> *pState,
                                            int nMaximum, PROTOCOLID nProtocol,
                                            CDownloadSource *pExcept) const {
  CQuickLock pLock(Transfers.m_pSection);
  ...
  for (POSITION posSource = GetIterator(); posSource;) {
    ...
    if (pSource != pExcept && pSource->m_bPushOnly == FALSE &&
        ((pSource->m_nFailures == 0 && pSource->m_bReadContent) ||
         nProtocol == PROTOCOL_NULL) &&
        (pSource->m_bSHA1 || pSource->m_bTiger || pSource->m_bED2K ||
         pSource->m_bBTH || pSource->m_bMD5) &&
        (pState == NULL || pState->Find(pSource->m_sURL) == NULL)) {
      // Only return appropriate sources
      if ((nProtocol == PROTOCOL_HTTP) &&
          (pSource->m_nProtocol != PROTOCOL_HTTP))
        continue;
      if ((nProtocol == PROTOCOL_SSL) && (pSource->m_nProtocol != PROTOCOL_SSL))
        continue;
      if ((nProtocol == PROTOCOL_G1) && (pSource->m_nGnutella != 1))
        continue;
...
CString CDownloadWithSources::GetTopFailedSources(int nMaximum,
    ...
    if (pSource->TouchedRange(nOffset, nLength)) {
      if (GetTaskType() == dtaskMergeFile) {
        ...
        theApp.Message(MSG_ERROR, IDS_DOWNLOAD_VERIFY_DROP,
                       ...
                       (LPCTSTR)pSource->m_sServer, (LPCTSTR)m_sName, nOffset,
                       nOffset + nLength - 1);
...
CFailedSource *CDownloadWithSources::LookupFailedSource(LPCTSTR pszUrl,
                                                        bool bReliable) {
  CSingleLock oLock(&Transfers.m_pSection);
  if (!oLock.Lock(100))
    return NULL;
  ...
  for (POSITION pos = m_pFailedSources.GetHeadPosition(); pos;) {
    ...
    if (pResult && pResult->m_sURL.Compare(pszUrl) == 0) {
      ...
      theApp.Message(MSG_DEBUG,
                     _T("Votes for file %s: negative - %i, positive - %i; ")
                     _T("offline status: %i"),
                     pszUrl, pResult->m_nNegativeVotes,
                     pResult->m_nPositiveVotes, pResult->m_bOffline);
#endif
      if (pResult->m_bLocal)
        break;

      if (bReliable) // Not used at the moment anywhere, we check that
                     // explicitly
      {
        INT_PTR nTotalVotes =
            pResult->m_nNegativeVotes + pResult->m_nPositiveVotes;
      ...
      // we should check IPs which add these sources though, since voting takes
      // place
    } else
      pResult = NULL;
  }
...
void CDownloadWithSources::AddFailedSource(const CDownloadSource *pSource,
                                           bool bLocal, bool bOffline) {
  CString strURL;
  if (pSource->m_nProtocol == PROTOCOL_BT && pSource->m_oGUID) {
    strURL.Format(_T("btc://%s/%s/"), (LPCTSTR)pSource->m_oGUID.toString(),
...
void CDownloadWithSources::AddFailedSource(LPCTSTR pszUrl, bool bLocal,
                                           bool bOffline) {
  ASSUME_LOCK(Transfers.m_pSection);

  if (LookupFailedSource(pszUrl) == NULL) {
    if (CFailedSource *pBadSource =
            new CFailedSource(pszUrl, bLocal, bOffline)) {
      m_pFailedSources.AddTail(pBadSource);
      theApp.Message(MSG_DEBUG, L"Bad sources count for \"%s\": %i. URL: %s",
                     (LPCTSTR)m_sName, m_pFailedSources.GetCount(), pszUrl);
    }
  }
}
    ...
    if (oLock.Lock(100)) {
      for (POSITION posSource = GetIterator(); posSource;) {
        ...
        if (pSource->m_crColour) {
          ...
          sg -= sd;
          sb -= sd;
          if (abs(r - d - sr) < 24 && abs(g - d - sg) < 24 &&
              abs(b - d - sb) < 24) {
            // Too similar
            bGood = FALSE;
            break;
...
void CDownloadWithSources::Serialize(CArchive &ar,
                                     int nVersion /* DOWNLOAD_SER_VERSION */) {
  ...
  if (ar.IsStoring()) {
    ...
    DWORD_PTR nSources =
        min((DWORD_PTR)GetCount(), (DWORD_PTR)Settings.Downloads.SourcesWanted);
    ar.WriteCount(nSources);

    for (POSITION posSource = GetIterator(); posSource && nSources;
         nSources--) {
      CDownloadSource *pSource = GetNext(posSource);
    ...
    for (DWORD_PTR nSources = ar.ReadCount(); nSources; nSources--) {
      // Create new source
      CAutoPtr<CDownloadSource> pSource(
          new CDownloadSource(static_cast<CDownload *>(this)));
      if (!pSource)
      ...
      if ((!pSource->m_nPort) &&
          (_tcsnicmp(pSource->m_sURL, _T("ed2kftp://"), 10) == 0)) {
