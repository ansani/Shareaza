...
CNetwork::CNetwork()
    : NodeRoute(new CRouteCache()), QueryRoute(new CRouteCache()),
      QueryKeys(new CQueryKeys()), UPnPFinder(NULL), Firewall(new CFirewall()),
      m_pHost(), m_pHostIPv6(), m_bAutoConnect(FALSE), m_bConnected(false),
      m_tStartedConnecting(0), m_nUPnPTier(0),
      m_bUPnPPortsForwarded(TRI_UNKNOWN), m_tUPnPMap(0),
...
BOOL CNetwork::Init() {
  ...
  if (Firewall->Init() && (Settings.Connection.EnableUPnP ||
      ...
      if (Settings.Connection.EnableUPnP) {
        if (Firewall->SetupService(NET_FW_SERVICE_UPNP))
          theApp.Message(MSG_INFO,
                         _T("Windows Firewall setup for UPnP succeeded."));
        else
          theApp.Message(MSG_ERROR,
                         _T("Windows Firewall setup for UPnP failed."));
      }

      if (Settings.Connection.EnableFirewallException) {
        if (Firewall->SetupProgram(theApp.m_strBinaryPath, CLIENT_NAME_T,
                                   FALSE))
          theApp.Message(
              MSG_INFO,
              _T("Windows Firewall setup for application succeeded."));
        else
          theApp.Message(MSG_ERROR,
                         _T("Windows Firewall setup for application failed."));
      }
    } else
...
HINTERNET CNetwork::InternetOpen() {
  __try {
    return ::InternetOpen(Settings.SmartAgent(), INTERNET_OPEN_TYPE_PRECONFIG,
                          NULL, NULL, 0);
...
bool CNetwork::IsConnected() const { return m_bConnected; }

bool CNetwork::IsListening() const {
  return (IsConnected()) && (m_pHost.sin_addr.S_un.S_addr != 0) &&
         (m_pHost.sin_port != 0) && (Handshakes.IsValid()) &&
         (Datagrams.IsValid());
}
...
bool CNetwork::IsListeningIPv6() const {
  return (IsConnected()) &&
         ...
         (m_pHostIPv6.sin6_port != 0) && (Handshakes.IsValid(true)) &&
...
BOOL CNetwork::IsFirewalled(int nCheck) const {
  ...
  return FALSE;
#else  // LAN_MODE
  if (Settings.Connection.FirewallState ==
      CONNECTION_OPEN) // CHECK_BOTH, CHECK_TCP, CHECK_UDP, CHECK_TCP6,
                       // CHECK_UDP6
    return FALSE;      // We know we are not firewalled on both TCP and UDP
  else if (nCheck == CHECK_IP)
    return IsFirewalledAddress(&Network.m_pHost.sin_addr) ||
           IsReserved(&Network.m_pHost.sin_addr);
  else if (nCheck == CHECK_IP6)
    return IsFirewalledAddress(&Network.m_pHostIPv6.sin6_addr) ||
           IsReserved(&Network.m_pHostIPv6.sin6_addr);
  else if (Settings.Connection.FirewallState == CONNECTION_OPEN_TCPONLY &&
           nCheck == CHECK_TCP)
    return FALSE; // We know we are not firewalled on TCP port
  else if (Settings.Connection.FirewallState == CONNECTION_OPEN_UDPONLY &&
           nCheck == CHECK_UDP)
...
BOOL CNetwork::IsConnectedTo(const IN_ADDR *pAddress) const {
  return IsSelfIP(*pAddress) || Handshakes.IsConnectedTo(pAddress) ||
         Neighbours.Get(*pAddress) || Transfers.IsConnectedTo(pAddress);
}
...
BOOL CNetwork::ReadyToTransfer(DWORD tNow) const {
  ...
  // We should wait a short time after starting the connection sequence before
  // starting downloads
...
BOOL CNetwork::ConnectTo(LPCTSTR pszAddress, int nPort, PROTOCOLID nProtocol,
                         BOOL bNoUltraPeer) {
  if (nPort <= 0 || nPort > USHRT_MAX) {
    nPort =
        protocolPorts[(nProtocol == PROTOCOL_ANY) ? PROTOCOL_NULL : nProtocol];
  }
  ...
  if (IPv6FromString(pszAddress, &saHost)) {
    ...
    // It's IPv6 address
    HostCache.ForProtocol(nProtocol)->AddIPv6(&saHost.sin6_addr,
                                              ntohs(saHost.sin6_port), NULL);

    Neighbours.ConnectTo(saHost.sin6_addr, ntohs(saHost.sin6_port), nProtocol,
                         FALSE, bNoUltraPeer);
    return TRUE;
  } else {
    ...
    if (saHost.sin_addr.s_addr != INADDR_ANY) {
      ...
      HostCache.ForProtocol(nProtocol)->Add(&saHost.sin_addr,
                                            ntohs(saHost.sin_port), NULL);

      Neighbours.ConnectTo(saHost.sin_addr, ntohs(saHost.sin_port), nProtocol,
                           FALSE, bNoUltraPeer);
      return TRUE;
    }
  }

  return AsyncResolve(pszAddress, (WORD)nPort, nProtocol,
                      bNoUltraPeer ? (BYTE)RESOLVE_CONNECT
                                   : (BYTE)RESOLVE_CONNECT_ULTRAPEER);
}
...
BOOL CNetwork::AcquireLocalAddress(SOCKET hSocket, bool bPort,
                                   const IN_ADDR *pFromAddress,
                                   const IN6_ADDR *pFromIPv6Address) {
...
BOOL CNetwork::AcquireLocalAddress(LPCTSTR pszHeader, WORD nPort,
  ...
  if (_stscanf(pszHeader, _T("%i.%i.%i.%i"), &nIPb1, &nIPb2, &nIPb3, &nIPb4) !=
          4 ||
      nIPb1 < 0 || nIPb1 > 255 || nIPb2 < 0 || nIPb2 > 255 || nIPb3 < 0 ||
      nIPb3 > 255 || nIPb4 < 0 || nIPb4 > 255) {
    SOCKADDR_IN6 pHost;
...
BOOL CNetwork::AcquireLocalAddress(const IN_ADDR &pAddress, WORD nPort,
  ...
  if (pAddress.s_addr == INADDR_ANY || pAddress.s_addr == INADDR_NONE)
    return FALSE;
...
BOOL CNetwork::AcquireLocalAddress(const IN6_ADDR &pAddress, WORD nPort,
                                   const IN6_ADDR *pFromAddress) {
  ...
  if (!pFromAddress || (nFromNet < 4 && nFromNet >= nNet)) {
    ...
    if (nMyNet >= nNet) {
      if (m_pHostAddressesIPv6.find(m_pHostIPv6.sin6_addr) ==
          m_pHostAddressesIPv6.end())
...
BOOL CNetwork::Resolve(LPCTSTR pszHost, int nPort, SOCKADDR_IN *pHost,
                       BOOL bNames) {
...
BOOL CNetwork::AsyncResolve(LPCTSTR pszAddress, WORD nPort,
                            PROTOCOLID nProtocol, BYTE nCommand) {
  ...
  if (pResolve.get()) {
    ...
    HANDLE hAsync = WSAAsyncGetHostByName(
        AfxGetMainWnd()->GetSafeHwnd(), WM_WINSOCK, CT2CA(pszAddress),
        pResolve->m_pBuffer, MAXGETHOSTSTRUCT);
    if (hAsync) {
      CQuickLock pLock(m_pLookupsSection);
      m_pLookups.SetAt(hAsync, pResolve.release());
      DEBUG_ONLY(theApp.Message(
          MSG_DEBUG, _T("Network name resolver has %u pending requests."),
          m_pLookups.GetCount()));
      return TRUE;
    }
  }
...
BOOL CNetwork::IsValidAddressFor(const IN_ADDR *pForAddress,
  ...
  if (!IsLocalAreaNetwork(
          pForAddress)) // pForAddress is internet address and pAddress in not
    return FALSE;
  ...
  if (m_bHomeNetworkNAT && IsHomeNetwork(pAddress))
    return FALSE;
...
BOOL CNetwork::IsValidAddressFor(const IN6_ADDR *pForAddress,
...
IN_ADDR CNetwork::GetMyAddressFor(const IN_ADDR *pAddress) const {
  ...
  if (nNet == 2 &&               // For home host
      nMyLanAddress.S_un.S_addr) // It can not be that we do not know our home
                                 // address, but we know the LAN address.
    return nMyLanAddress;

  if (nNet == 1 &&                // For Local Area Network host
      nMyHomeAddress.S_un.S_addr) // Perhaps we are connected to a local network
                                  // without a home router.
...
IN6_ADDR CNetwork::GetMyAddressFor(const IN6_ADDR *pAddress) const {
  ...
  for (ipv6_set::const_iterator pos = m_pHostAddressesIPv6.begin();
       pos != m_pHostAddressesIPv6.end(); pos++) {
  ...
  if (nNet == 2 && // For home host
      !IN6_IS_ADDR_UNSPECIFIED(
          &nMyLanAddress)) // It can not be that we do not know our home
                           // address, but we know the LAN address.
    return nMyLanAddress;

  if (nNet == 1 && // For Local Area Network host
      !IN6_IS_ADDR_UNSPECIFIED(
          &nMyHomeAddress)) // Perhaps we are connected to a local network
                            // without a home router.
    return nMyHomeAddress;
...
BOOL CNetwork::IsHomeNetwork(const IN_ADDR *pAddress) const {
  ...
  if (GetIpAddrTable(ipAddr, &nSize, TRUE) == NO_ERROR) {
    ...
    for (DWORD nIf = 0; nIf < nCount; nIf++) {
      ...
      if ((dwAddr & dwMask) ==
          (pAddress->S_un.S_addr &
           dwMask)) // We check that the address in the same subnet
      {
        MIB_IFROW ifRow = {};
...
BOOL CNetwork::IsFirewalledAddress(const IN6_ADDR *pAddress, BOOL bIncludeSelf,
                                   BOOL bIgnoreLocalIP) const {
  if (!pAddress)
    return TRUE;
...
WORD CNetwork::GetPort() const {
  return m_pHost.sin_port ? ntohs(m_pHost.sin_port)
                          : (WORD)Settings.Connection.InPort;
}

// Returns TRUE if the IP address is reserved.
// Private addresses are treated as reserved when Connection.IgnoreLocalIP =
// TRUE. The code is based on nmap code and updated according to
...
BOOL CNetwork::IsReserved(const IN6_ADDR *pAddress) const {
  if (IN6_IS_ADDR_UNSPECIFIED(pAddress))
    return TRUE;
  if (IN6_IS_ADDR_MULTICAST(pAddress))
    return TRUE;
  if (IN6_IS_ADDR_LOOPBACK(pAddress))
    return TRUE;
  return FALSE;
...
WORD CNetwork::RandomPort() const { return GetRandomNum(10000ui16, 60000ui16); }
...
bool CNetwork::PreRun() {
  ...
  if (Settings.Connection.ForceConnectedState) {
    ...
    InternetSetOption(hInternet, INTERNET_OPTION_CONNECTED_STATE, &ici,
                      sizeof(ici));
  ...
  if (/*IsFirewalled()*/ Settings.Connection.FirewallState ==
      CONNECTION_FIREWALLED) // Temp disable
  ...
  // Map ports using NAT UPnP and Control Point UPnP methods and acquire
  // external IP on success
  MapPorts();
...
void CNetwork::OnWinsock(WPARAM wParam, LPARAM lParam) {
  ...
  if (WSAGETASYNCERROR(lParam) == 0) {
    ...
    case RESOLVE_ONLY:
      HostCache.OnResolve(pResolve->m_nProtocol, pResolve->m_sAddress, pAddress,
                          pResolve->m_nPort);
      break;
    ...
    case RESOLVE_CONNECT:
      HostCache.OnResolve(pResolve->m_nProtocol, pResolve->m_sAddress, pAddress,
                          pResolve->m_nPort);
      Neighbours.ConnectTo(*pAddress, pResolve->m_nPort, pResolve->m_nProtocol,
                           FALSE, (pResolve->m_nCommand == RESOLVE_CONNECT));
      break;

    case RESOLVE_DISCOVERY:
      DiscoveryServices.OnResolve(pResolve->m_nProtocol, pResolve->m_sAddress,
                                  pAddress, pResolve->m_nPort);
      break;

    default:;
    }
  } else {
    theApp.Message(MSG_ERROR, IDS_NETWORK_RESOLVE_FAIL,
                   (LPCTSTR)pResolve->m_sAddress);
...
BOOL CNetwork::GetNodeRoute(const Hashes::Guid &oGUID, CNeighbour **ppNeighbour,
                            SOCKADDR_IN *pEndpoint) {
  if (validAndEqual(oGUID, Hashes::Guid(MyProfile.oGUID)))
    return FALSE;

  if (NodeRoute->Lookup(oGUID, ppNeighbour, pEndpoint))
    return TRUE;
  if (ppNeighbour == NULL)
    return FALSE;
...
BOOL CNetwork::RoutePacket(CG2Packet *pPacket) {
  ...
  if (!pPacket->GetTo(oGUID) ||
      validAndEqual(oGUID, Hashes::Guid(MyProfile.oGUID)))
    return FALSE;
...
BOOL CNetwork::SendPush(const Hashes::Guid &oGUID, DWORD nIndex) {
  ...
  while (GetNodeRoute(oGUID2, &pOrigin, &pEndpoint)) {
    if (pOrigin != NULL && pOrigin->m_nProtocol == PROTOCOL_G1) {
      CG1Packet *pPacket =
          CG1Packet::New(G1_PACKET_PUSH, Settings.Gnutella1.MaximumTTL - 1);
...
BOOL CNetwork::RouteHits(CQueryHit *pHits, CPacket *pPacket) {
  ...
  if (!QueryRoute->Lookup(pHits->m_oSearchID, &pOrigin, &pEndpoint))
    return FALSE;
  ...
  if (pPacket->m_nProtocol == PROTOCOL_G1) {
    ...
    if (pG2->IsType(G2_PACKET_HIT) && pG2->m_nLength > 17) {
      ...
      if (*pHops > Settings.Gnutella1.MaximumTTL)
        return FALSE;
      (*pHops)++;
    } else if (pG2->IsType(G2_PACKET_HIT_WRAP)) {
      if (!pG2->SeekToWrapped())
        return FALSE;
      GNUTELLAPACKET *pG1 =
          (GNUTELLAPACKET *)(pPacket->m_pBuffer + pPacket->m_nPosition);
      if (pG1->m_nTTL == 0)
        return FALSE;
      pG1->m_nTTL--;
      pG1->m_nHops++;
  ...
  if (pOrigin != NULL) {
    ...
    } else if (pOrigin->m_nProtocol == PROTOCOL_G1 &&
               pPacket->m_nProtocol == PROTOCOL_G2) {
      if (!bWrapped)
        return FALSE;
      pPacket = CG1Packet::New(
          (GNUTELLAPACKET *)(pPacket->m_pBuffer + pPacket->m_nPosition));
      pOrigin->Send(pPacket, TRUE, TRUE);
    } else if (pOrigin->m_nProtocol == PROTOCOL_G2 &&
               pPacket->m_nProtocol == PROTOCOL_G1) {
    ...
    if (IsSelfIP(pEndpoint.sin_addr))
      return FALSE;
    Datagrams.Send(&pEndpoint, pPacket, FALSE);
  } else {
    if (IsSelfIP(pEndpoint.sin_addr))
    ...
    // Send searches to monitor window
    {
      ...
      if (oAppLock.Lock(250)) {
        if (CMainWnd *pMainWnd = theApp.SafeMainWnd()) {
          ...
          CChildWnd *pChildWnd = NULL;
          while ((pChildWnd = pMainWnd->m_pWindows.Find(pClass, pChildWnd)) !=
                 NULL) {
    ...
    // Send hits to search windows
    {
      ...
      if (oAppLock.Lock(250)) {
        if (CMainWnd *pMainWnd = theApp.SafeMainWnd()) {
          ...
          CChildWnd *pChildWnd = NULL;
          while ((pChildWnd = pMainWnd->m_pWindows.Find(NULL, pChildWnd)) !=
                 NULL) {
...
SOCKET CNetwork::AcceptSocket(SOCKET hSocket, SOCKADDR_IN *addr,
                              LPCONDITIONPROC lpfnCondition,
                              DWORD_PTR dwCallbackData) {
  __try // Fix against stupid firewalls like (iS3 Anti-Spyware or Norman Virus
        // Control)
  {
    int len = sizeof(SOCKADDR_IN);
    return WSAAccept(hSocket, (SOCKADDR *)addr, &len, lpfnCondition,
                     dwCallbackData);
  } __except (EXCEPTION_EXECUTE_HANDLER) {
...
SOCKET CNetwork::AcceptSocket(SOCKET hSocket, SOCKADDR_IN6 *addr,
                              LPCONDITIONPROC lpfnCondition,
                              DWORD_PTR dwCallbackData) {
  __try // Fix against stupid firewalls like (iS3 Anti-Spyware or Norman Virus
        // Control)
...
int CNetwork::SendTo(SOCKET s, const char *buf, int len,
                     const SOCKADDR_IN *pTo) {
  __try // Fix against stupid firewalls like (iS3 Anti-Spyware or Norman Virus
        // Control)
  {
...
HINTERNET CNetwork::InternetOpenUrl(HINTERNET hInternet, LPCWSTR lpszUrl,
                                    LPCWSTR lpszHeaders, DWORD dwHeadersLength,
                                    DWORD dwFlags) {
  __try // Fix against stupid firewalls like (iS3 Anti-Spyware or Norman Virus
        // Control)
  {
    return ::InternetOpenUrl(hInternet, lpszUrl, lpszHeaders, dwHeadersLength,
                             dwFlags, NULL);
  } __except (EXCEPTION_EXECUTE_HANDLER) {
    return NULL;
  }
...
void CNetwork::MapPorts() {
  ...
  if (UPnPFinder) {
    theApp.Message(MSG_INFO, _T("Trying to setup port mappings (tier %u)..."),
                   m_nUPnPTier);
...
void CNetwork::OnMapSuccess() {
  theApp.Message(MSG_INFO, _T("UPnP port mapping succeeded (tier %u)."),
                 m_nUPnPTier);
...
void CNetwork::OnMapFailed() {
  theApp.Message(MSG_ERROR, _T("UPnP port mapping failed (tier %u)."),
                 m_nUPnPTier);
