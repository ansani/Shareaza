...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS construction

CDownloadTransferHTTPS::CDownloadTransferHTTPS(CDownloadSource *pSource)
    : CDownloadTransfer(pSource, PROTOCOL_SSL), m_nRequests(0), m_tContent(0),
      m_bBadResponse(FALSE), m_bBusyFault(FALSE), m_bRangeFault(FALSE),
      m_bKeepAlive(FALSE), m_bTigerFetch(FALSE), m_bTigerIgnore(FALSE),
      m_bMetaFetch(FALSE), m_bGotRange(FALSE), m_bGotRanges(FALSE),
      m_bQueueFlag(FALSE), m_nContentLength(SIZE_UNKNOWN),
      m_nRetryDelay(Settings.Downloads.RetryDelay), m_nRetryAfter(0),
      m_bRedirect(FALSE), m_bGzip(FALSE), m_bCompress(FALSE), m_bDeflate(FALSE),
      m_bChunked(FALSE), m_ChunkState(Header), m_nChunkLength(SIZE_UNKNOWN) {}

CDownloadTransferHTTPS::~CDownloadTransferHTTPS() {}

//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS initiate connection

BOOL CDownloadTransferHTTPS::Initiate() {
  ASSUME_LOCK(Transfers.m_pSection);

  theApp.Message(MSG_INFO, IDS_DOWNLOAD_CONNECTING,
                 (LPCTSTR)CString(inet_ntoa(m_pSource->m_pAddress)),
                 m_pSource->m_nPort, (LPCTSTR)m_pDownload->GetDisplayName());

  if (m_pDownload->IsCompleted()) {
    m_pDownload->RemoveSource(m_pSource, FALSE);
    return FALSE;
  } else if (m_pSource->m_pAddress.s_addr
                 ? SSLConnectTo(&m_pSource->m_pAddress, m_pSource->m_nPort)
                 : ConnectToIPv6(&m_pSource->m_pAddressIPv6,
                                 m_pSource->m_nPort)) {
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS accept push

void CDownloadTransferHTTPS::AttachTo(CConnection *pConnection) {
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS close

void CDownloadTransferHTTPS::Close(TRISTATE bKeepSource, DWORD nRetryAfter) {
  ASSUME_LOCK(Transfers.m_pSection);
  ...
  if (Settings.Connection.Reuse && m_bKeepAlive &&
      !(m_bBusyFault || m_bBadResponse))
    GiveConnectionToNext();

  CDownloadTransfer::Close(bKeepSource, nRetryAfter);
}

void CDownloadTransferHTTPS::GiveConnectionToNext() {
  ...
  for (POSITION pos = Downloads.GetIterator(); pos != NULL;) {
    ...
    for (POSITION posSource = pDownload->GetIterator(); posSource;) {
      ...
      if (pSource->m_nBusyCount == 0 && pSource->IsSSLSource() &&
          !pSource->IsConnected() && pSource->Equals(m_pSource)) {
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS speed controls

void CDownloadTransferHTTPS::Boost() {
  m_mInput.pLimit = m_mOutput.pLimit = NULL;
}

DWORD CDownloadTransferHTTPS::GetAverageSpeed() {
  ASSUME_LOCK(Transfers.m_pSection);
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS connection handler

BOOL CDownloadTransferHTTPS::OnConnected() {
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS fragment allocation

BOOL CDownloadTransferHTTPS::StartNextFragment() {
  ASSUME_LOCK(Transfers.m_pSection);
  ...
  if (GetInputLength() > 0 && m_nRequests > 0) {
    theApp.Message(MSG_ERROR, IDS_DOWNLOAD_CLOSING_OVERFLOW,
                   (LPCTSTR)m_sAddress);
    Close(TRI_TRUE);
    return FALSE;
  }
  ...
  if (m_pDownload->NeedTigerTree() &&
      (!m_sTigerTree.IsEmpty() ||
       _tcsistr(m_pSource->m_sURL, _T("?urn:")) != NULL)) {
    if (m_sTigerTree.IsEmpty() && m_pDownload->m_oTiger &&
        Settings.Downloads.VerifyTiger && !m_bTigerIgnore) {
      ...
      // or it is a "GIV" (push) connection
      m_sTigerTree.Format(L"/gnutella/thex/v1?%s&depth=%u&ed2k=%d",
                          (LPCTSTR)m_pDownload->m_oTiger.toUrn(),
  ...
  if (m_pDownload->GetFragment(this)) {
    m_bWantBackwards = ChunkifyRequest(&m_nOffset, &m_nLength,
                                       Settings.Downloads.ChunkSize, TRUE);

    theApp.Message(MSG_INFO, IDS_DOWNLOAD_FRAGMENT_REQUEST, m_nOffset,
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS subtract pending requests

BOOL CDownloadTransferHTTPS::SubtractRequested(
    Fragments::List &ppFragments) const {
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS send request

BOOL CDownloadTransferHTTPS::SendRequest() {
  ASSUME_LOCK(Transfers.m_pSection);

  CString strLine;

  CShareazaURL pURL;
  if (!pURL.Parse(m_pSource->m_sURL, FALSE) || pURL.m_nProtocol != PROTOCOL_SSL)
    return FALSE;
  ...
  if (m_bTigerFetch) {
    Write(
        _P("Accept: application/dime, application/tigertree-breadthfirst\r\n"));
  } else if (m_bMetaFetch) {
  ...
  if (m_nOffset != SIZE_UNKNOWN && !m_bTigerFetch && !m_bMetaFetch) {
    if (m_nLength == SIZE_UNKNOWN || m_nLength == 0 ||
        m_nOffset + m_nLength == m_pDownload->m_nSize) {
  ...
  if (m_pSource->m_nPort == INTERNET_DEFAULT_HTTPS_PORT) {
  ...
  if (!m_bTigerFetch && !m_bMetaFetch) {
    m_pSource->m_bPartialSame = pURL.m_oSHA1 && m_pDownload->m_oSHA1 &&
                                pURL.m_oSHA1 != m_pDownload->m_oSHA1;
    ...
    if (!m_pSource->m_bPartialSame) {
      ...
      if (m_pSource->m_bSHA1 && Settings.Library.SourceMesh) {
        strLine = m_pDownload->GetSourceURLs(
            &m_pSourcesSent, 15,
            (m_pSource->m_nGnutella < 2) ? PROTOCOL_G1 : PROTOCOL_SSL,
            m_pSource);
        if (strLine.GetLength()) {
        ...
        if (m_pDownload->IsShared() && m_pDownload->IsStarted() &&
            Network.IsStable()) {
  ...
  CDownloadTransfer::OnSSLWrite();

  return TRUE;
}

//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS run handler

BOOL CDownloadTransferHTTPS::OnRun() {
  ASSUME_LOCK(Transfers.m_pSection);
  ...
  case dtsBusy:
    if (tNow - m_tRequest > 1000) {
      theApp.Message(MSG_ERROR, IDS_DOWNLOAD_BUSY, (LPCTSTR)m_sAddress,
                     Settings.Downloads.RetryDelay / 1000);
      Close(TRI_TRUE);
      return FALSE;
    }
    break;
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS read handler

BOOL CDownloadTransferHTTPS::OnRead() {
  CDownloadTransfer::OnSSLRead();

  switch (m_nState) {
  case dtsRequesting:
    if (!ReadResponseLine())
      return FALSE;
    if (m_nState != dtsHeaders)
      break;
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS read response line

BOOL CDownloadTransferHTTPS::ReadResponseLine() {
  ASSUME_LOCK(Transfers.m_pSection);
  ...
  theApp.Message(MSG_DEBUG | MSG_FACILITY_INCOMING,
                 _T("%s >> DOWNLOAD RESPONSE: %s"), (LPCTSTR)m_sAddress,
                 (LPCTSTR)strLine);
  ...
  if (strLine.GetLength() >= 12 && strLine.Left(9) == _T("HTTP/1.1 ")) {
    ...
    theApp.Message(MSG_DEBUG, _T("HTTP with unknown version: %s"),
                   (LPCTSTR)strLine);
    m_bKeepAlive = FALSE;
  } else {
  ...
  if (strCode == _T("200") || strCode == _T("206")) {
    ...
    // 503 response without an X-Available-Ranges header means the complete file
    // is available
    ...
    strMessage.TrimLeft();
    if (strMessage.GetLength() > 128)
      strMessage = _T("No Message");
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS read header lines

BOOL CDownloadTransferHTTPS::OnHeaderLine(CString &strHeader,
                                          CString &strValue) {
  ASSUME_LOCK(Transfers.m_pSection);
  ...
  if (strHeader.CompareNoCase(_T("Server")) == 0) {
    ...
    if (m_pSource->m_sServer.GetLength() > 64)
      m_pSource->m_sServer = m_pSource->m_sServer.Left(64);

    if (m_bClientExtended)
      m_pSource->SetGnutella(3);
    if (_tcsistr(m_sUserAgent, _T("trustyfiles")) != NULL)
      m_pSource->SetGnutella(3);
    if (_tcsistr(m_sUserAgent, _T("gnucdna")) != NULL)
      m_pSource->SetGnutella(3);
    if (_tcsistr(m_sUserAgent, _T("vagaa")) != NULL)
      m_pSource->SetGnutella(3);
    if (_tcsistr(m_sUserAgent, _T("mxie")) != NULL)
      m_pSource->SetGnutella(3);
    if (_tcsistr(m_sUserAgent, _T("adagio")) != NULL)
    ...
    if ((_stscanf(strValue, _T("bytes %I64u-%I64u/%I64u"), &nFirst, &nLast,
                  &nTotal) == 3 ||
         _stscanf(strValue, _T("bytes=%I64u-%I64u/%I64u"), &nFirst, &nLast,
                  &nTotal) == 3) &&
      ...
      if (nFirst == m_nOffset && nLast == m_nOffset + m_nLength - 1 &&
          bUseful) {
        // Perfect match, good
      } else if (nFirst >= m_nOffset && nFirst < m_nOffset + m_nLength &&
                 bUseful) {
        m_nOffset = nFirst;
    ...
    if (_tcsistr(strValue, _T("chunked"))) {
      ...
      theApp.Message(MSG_DEBUG, _T( "Unknown transfer encoding: %s" ),
                     (LPCTSTR)strValue);
      Close(TRI_FALSE);
      return FALSE;
    }
    ...
    for (int nPos = strURNs.Find(','); nPos >= 0 && !m_pSource->m_bPartialSame;
         nPos = strURNs.Find(',')) {
    ...
    m_pSource->SetGnutella(1);
  } else if (strHeader.CompareNoCase(_T("X-Gnutella-Alternate-Location")) ==
                 0 ||
    ...
    if (m_pSource->m_oAvailable.empty()) {
      theApp.Message(
          MSG_DEBUG,
          _T( "header did not include valid ranges, dropping source..." ));
      Close(TRI_FALSE);
      return FALSE;
    }
    ...
    if (nPos >= 0)
      _stscanf(strValue.Mid(nPos + 9), _T("%lu"), &m_nQueuePos);

    nPos = strValue.Find(_T("length="));
    if (nPos >= 0)
      _stscanf(strValue.Mid(nPos + 7), _T("%lu"), &m_nQueueLen);

    DWORD nLimit = 0;

    nPos = strValue.Find(_T("pollmin="));
    if (nPos >= 0 &&
        _stscanf(strValue.Mid(nPos + 8), _T("%lu"), &nLimit) == 1) {
    ...
    if (_tcsistr(strValue, _T("g2/")) != NULL)
      m_pSource->SetGnutella(2);
    if (_tcsistr(strValue, _T("gnet2/")) != NULL)
      m_pSource->SetGnutella(2);
    if (_tcsistr(strValue, _T("gnutella2/")) != NULL)
      m_pSource->SetGnutella(2);
    m_pSource->SetGnutella(1);
    ...
    if (!m_pDownload->HasStableName()) {
      ...
      if (nPos >= 0) {
        ...
        const CString strFilename =
            URLDecode(strValue.Mid(nPos + 9).Trim(_T("\" \t\r\n")));
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS end of headers

BOOL CDownloadTransferHTTPS::OnHeadersComplete() {
  // Close parameters:
  // TRI_FALSE   - the source will be added to m_pFailedSources in
  // CDownloadWithSources,
  //			    removed from the sources and can be distributed in the
  //Source Mesh as X-Nalt
  ...
  if (m_bBadResponse) {
    ...
    if (Settings.Downloads.QueueLimit > 0 &&
        m_nQueuePos > Settings.Downloads.QueueLimit) {
      theApp.Message(MSG_ERROR, IDS_DOWNLOAD_QUEUE_HUGE, (LPCTSTR)m_sAddress,
                     (LPCTSTR)m_pDownload->GetDisplayName(), m_nQueuePos);
      Close(TRI_FALSE);
      return FALSE;
  ...
  if (m_bRangeFault) {
    ...
    if (m_sContentType.CompareNoCase(
            _T("application/tigertree-breadthfirst")) &&
        m_sContentType.CompareNoCase(_T("application/dime")) &&
        m_sContentType.CompareNoCase(
            _T("application/binary"))) // Content Type used by Phex
    {
      theApp.Message(MSG_INFO, IDS_DOWNLOAD_TIGER_RANGE, (LPCTSTR)m_sAddress);
      Close(TRI_TRUE);
      return FALSE;
    ...
    theApp.Message(MSG_INFO, IDS_DOWNLOAD_METADATA_RECV, (LPCTSTR)m_sAddress,
                   (LPCTSTR)m_pSource->m_sServer);
    ...
    m_nOffset = 0;
    m_nLength = m_nContentLength;
  } else if (CFailedSource *pBadSource =
                 m_pDownload->LookupFailedSource(m_pSource->m_sURL)) {
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS read content

BOOL CDownloadTransferHTTPS::ReadContent() {
  ASSUME_LOCK(Transfers.m_pSection);
  ...
  while (pInput->m_nLength > 0) {
    ...
    if (m_bChunked) {
      ...
      case Header:
        if (pInput->m_nLength >= 3) {
          ...
          if (i < pInput->m_nLength - 1) {
            if (sscanf_s((LPCSTR)pInput->m_pBuffer, "%I64x", &m_nChunkLength) ==
                1) {
              if (m_nChunkLength == 0) {
      ...
      case BodyEnd:
        ...
        if (pInput->m_nLength >= 2) {
          if (pInput->m_pBuffer[0] == 0x0d && pInput->m_pBuffer[1] == 0x0a) {
            m_ChunkState = Header;
    ...
    if (m_bRecvBackwards) {
      ...
      bSubmit = m_pDownload->SubmitData(m_nOffset + m_nPosition,
    ...
    if (!bSubmit) {
      BOOL bUseful = m_pDownload->IsRangeUsefulEnough(
          this, m_bRecvBackwards ? m_nOffset : m_nOffset + m_nPosition,
          m_nLength - m_nPosition);
      if (/* m_bInitiated || */ !bUseful) {
        theApp.Message(MSG_INFO, IDS_DOWNLOAD_FRAGMENT_OVERLAP,
                       (LPCTSTR)m_sAddress);
        Close(TRI_TRUE);
        return FALSE;
      }
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS read Metadata

BOOL CDownloadTransferHTTPS::ReadMetadata() {
  CLockedBuffer pInput(GetInput());

  if (pInput->m_nLength < m_nLength)
    return TRUE;
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS read tiger tree

BOOL CDownloadTransferHTTPS::ReadTiger(bool bDropped) {
  // It is a fix for very slow DIME uploads, they get dropped while downloading
  // (e.g. LimeWire).
  m_tContent = m_mInput.tLast = GetTickCount();

  // Fix for PHEX TTH which never tell content length for DIME block to get into
  // DIME decoding until the connection drops, if no content length specified
  // and not keep-alive.
  if (!m_bKeepAlive && m_nContentLength == SIZE_UNKNOWN)
    return TRUE;
  ...
  if (m_sContentType.CompareNoCase(_T("application/tigertree-breadthfirst")) ==
    ...
    while (pInput->ReadDIME(&nFlags, &strID, &strType, &nBody)) {
      theApp.Message(MSG_DEBUG, _T("THEX DIME: %i, '%s', '%s', %i"), nFlags,
                     (LPCTSTR)strID, (LPCTSTR)strType, nBody);

      if ((nFlags & 1) && strType.CompareNoCase(_T("text/xml")) == 0 &&
          nBody < 1024 * 1024) {
        ...
        if (CXMLElement *pXML = CXMLElement::FromString(strXML)) {
          if (pXML->IsNamed(_T("hashtree"))) {
            if (CXMLElement *pxFile = pXML->GetElementByName(_T("file"))) {
              QWORD nSize = 0;
              _stscanf(pxFile->GetAttributeValue(_T("size")), _T("%I64u"),
                       &nSize);
              bSize = (nSize == m_pDownload->m_nSize);
            }
            if (CXMLElement *pxDigest = pXML->GetElementByName(_T("digest"))) {
              if (pxDigest->GetAttributeValue(_T("algorithm"))
                      .CompareNoCase(
                          _T("http://open-content.net/spec/digest/tiger")) ==
                  0) {
                bDigest =
                    (pxDigest->GetAttributeValue(_T("outputsize")) == _T("24"));
              }
            }
            if (CXMLElement *pxTree =
                    pXML->GetElementByName(_T("serializedtree"))) {
              bEncoding =
                  (pxTree->GetAttributeValue(_T("type"))
                       .CompareNoCase(_T("http://open-content.net/spec/thex/")
                                      _T("breadthfirst")) == 0);
        ...
        theApp.Message(MSG_DEBUG,
                       _T("THEX XML: size=%i, digest=%i, encoding=%i"), bSize,
                       bDigest, bEncoding);

        if (!bSize || !bDigest || !bEncoding)
          break;
      } else if ((strID == strUUID || strID.IsEmpty()) &&
                 strType.CompareNoCase(
                     _T("http://open-content.net/spec/thex/breadthfirst")) ==
                     0) {
        if (m_pDownload->SetTigerTree(pInput->m_pBuffer, nBody))
          m_pDownload->FindZerosRangesTreeTiger();
      } else if (strType.CompareNoCase(
                     _T("http://edonkey2000.com/spec/md4-hashset")) == 0) {
  ...
  // m_bKeepAlive == FALSE means that it was not keep-alive, so should just get
  // disconnected. after reading of DIME message This might be better with
  // returning FALSE because it is not keep alive connection need to disconnect
  // after the business
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS read flushing

BOOL CDownloadTransferHTTPS::ReadFlush() {
  CLockedBuffer pInput(GetInput());
  ...
  if (m_nContentLength == 0) {
    if (m_bQueueFlag) {
      ...
      theApp.Message(MSG_INFO, IDS_DOWNLOAD_QUEUED, (LPCTSTR)m_sAddress,
                     m_nQueuePos, m_nQueueLen, (LPCTSTR)m_sQueueName);
      ...
      theApp.Message(MSG_INFO, IDS_DOWNLOAD_416_WITHOUT_RANGE,
                     (LPCTSTR)m_sAddress);
      Close(TRI_TRUE);
      return FALSE;
      ...
      theApp.Message(
          MSG_ERROR,
          _T("BUG: Shareaza requested a fragment from host %s, although it ")
          _T("knew that the host doesn't have that fragment"),
          (LPCTSTR)m_sAddress);
      Close(TRI_TRUE);
      return FALSE;
    } else {
...
//////////////////////////////////////////////////////////////////////
// CDownloadTransferHTTPS dropped connection handler

void CDownloadTransferHTTPS::OnDropped() {
  ASSUME_LOCK(Transfers.m_pSection);
  ...
  if (m_nState == dtsConnecting) {
    ...
    // It was closed connection with no content length, so assume the content
    // length is equal to the size of buffer when the connection gets cut. It is
    // important to set it because the DIME decoding code check if the content
    // length is equals to size of buffer.
