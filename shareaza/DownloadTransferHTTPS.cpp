...
CDownloadTransferHTTPS::CDownloadTransferHTTPS(CDownloadSource* pSource) :
	CDownloadTransfer( pSource, PROTOCOL_SSL ),
	m_nRequests( 0 ),
	m_tContent( 0 ),
...
BOOL CDownloadTransferHTTPS::Initiate()
{
	...
	else if ( m_pSource->m_pAddress.s_addr ? SSLConnectTo( &m_pSource->m_pAddress, m_pSource->m_nPort ) : ConnectToIPv6( &m_pSource->m_pAddressIPv6, m_pSource->m_nPort ) )
	{
...
BOOL CDownloadTransferHTTPS::SubtractRequested(Fragments::List& ppFragments) const
{
...
BOOL CDownloadTransferHTTPS::SendRequest()
{
	...
	CShareazaURL pURL;
	if ( ! pURL.Parse( m_pSource->m_sURL, FALSE ) || pURL.m_nProtocol != PROTOCOL_SSL ) return FALSE;
		...
		if ( ! m_pSource->m_bPartialSame ){
			...
			if ( m_pSource->m_bSHA1 && Settings.Library.SourceMesh )
			{
				strLine = m_pDownload->GetSourceURLs( &m_pSourcesSent, 15,
					( m_pSource->m_nGnutella < 2 ) ? PROTOCOL_G1 : PROTOCOL_SSL, m_pSource );
				if ( strLine.GetLength() )
				{
...
BOOL CDownloadTransferHTTPS::OnHeaderLine(CString& strHeader, CString& strValue)
{
	ASSUME_LOCK( Transfers.m_pSection );
