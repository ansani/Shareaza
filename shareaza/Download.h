...
// 34 - added m_bSeeding and m_sServingFileName to CDownloadWithTorrent
// (Rolandas) 35 - added m_sCountry to CDownloadSource (dcat) 36 - nothing
// (Rolandas) 37 - added m_oBTH to CDownloadBase, m_bBTH and m_bMD5 to
// CDownloadSource (Ryo-oh-ki) 38 - added m_sCountryName to CDownloadSource
// (dcat) 39 - added m_bClientExtended to CDownloadSource (Ryo-oh-ki) 40 - added
// virtual fragmented file (Ryo-oh-ki) 41 - added m_sName to CFragmentedFile
// (Ryo-oh-ki) 42 - added m_bMetaIgnore to CDownloadSource (Ryo-oh-ki) 43 -
// added m_pAddressIPv6 and m_pIPv6ServerAddress to CDownloadSource (ivan386) 44
// - added m_tAdded to CDownload (ansani)
...
private:
  ...
  DWORD m_tSaved;
  DWORD
      m_tBegan; // The time when this download began trying to download (Started
  // searching, etc). 0 means has not tried this session.
  DWORD m_tStartFromSet;
  QWORD m_nCompletedAtBegan;
  bool m_bDownloading; // This is used to store if a download is downloading.
                       // (Performance tweak)
...
public:
  ...
  void MergeFile(CList<CString> *pFiles, BOOL bValidation = TRUE,
                 const Fragments::List *pGaps = NULL);
