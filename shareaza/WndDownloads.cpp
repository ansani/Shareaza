...
ON_UPDATE_COMMAND_UI(ID_DOWNLOADS_REMOTE_PREVIEW,
                     OnUpdateDownloadsRemotePreview)
...
ON_UPDATE_COMMAND_UI(ID_DOWNLOADS_FILTER_SOURCES,
                     OnUpdateDownloadsFilterSources)
...
ON_UPDATE_COMMAND_UI(ID_DOWNLOADS_LAUNCH_COMPLETE,
                     OnUpdateDownloadsLaunchComplete)
...
ON_UPDATE_COMMAND_UI(ID_DOWNLOADS_CLEAR_INCOMPLETE,
                     OnUpdateDownloadsClearIncomplete)
ON_COMMAND(ID_DOWNLOADS_CLEAR_INCOMPLETE, OnDownloadsClearIncomplete)
ON_UPDATE_COMMAND_UI(ID_DOWNLOADS_CLEAR_COMPLETE,
                     OnUpdateDownloadsClearComplete)
...
CDownloadsWnd::CDownloadsWnd()
    : CPanelWnd(TRUE, TRUE), m_bMouseCaptured(false) {
...
int CDownloadsWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  if (!m_wndTabBar.Create(this, WS_CHILD | CBRS_TOP, AFX_IDW_TOOLBAR + 1))
    return -1;
  m_wndTabBar.SetBarStyle(m_wndTabBar.GetBarStyle() | CBRS_TOOLTIPS |
                          CBRS_BORDER_BOTTOM);
  if (!m_wndToolBar.Create(this, WS_CHILD | WS_VISIBLE | CBRS_NOALIGN,
                           AFX_IDW_TOOLBAR))
    return -1;
  ...
  m_pDragList = NULL;
  m_pDragImage = NULL;
  m_hCursMove = AfxGetApp()->LoadCursor(
      theApp.m_bIsVistaOrNewer ? IDC_MOVE_VISTA : IDC_MOVE);
  m_hCursCopy = AfxGetApp()->LoadCursor(
      theApp.m_bIsVistaOrNewer ? IDC_COPY_VISTA : IDC_COPY);
  m_tSel = 0;
...
BOOL CDownloadsWnd::OnCmdMsg(UINT nID, int nCode, void *pExtra,
...
void CDownloadsWnd::OnSize(UINT nType, int cx, int cy) {
  ...
  BOOL bTabs =
      (Settings.General.GUIMode != GUI_BASIC) && Settings.Downloads.ShowGroups;
  ...
  DeferWindowPos(hPos, m_wndDownloads, NULL, rc.left, rc.top, rc.Width(),
                 rc.Height(), SWP_NOZORDER | SWP_SHOWWINDOW);
  if (bTabs)
    DeferWindowPos(hPos, m_wndTabBar, NULL, rc.left, 0, rc.Width(), 24,
                   SWP_NOZORDER | SWP_SHOWWINDOW);
  DeferWindowPos(hPos, m_wndToolBar, NULL, rc.left, rc.bottom, rc.Width(), 28,
                 SWP_NOZORDER);
...
void CDownloadsWnd::OnTimer(UINT_PTR nIDEvent) {
          ...
          if (pDownload->IsTorrent() == TRUE) // If it's a torrent
          {
            ...
            if (Settings.BitTorrent.AutoClear) {
              ...
              if (pDownload->IsSeeding() &&
                  (Settings.BitTorrent.ClearRatio < pDownload->GetRatio())) {
                pDownload->Remove();
              }
            }
  ...
  if ((nIDEvent == 2) && (m_pDragList == NULL)) {
    ...
    if ((IsWindowVisible() && IsActive(FALSE)) ||
        ((GetTickCount() - m_tLastUpdate) > 10 * 1000)) {
      // Update the window
      Update();
    }
  }
...
void CDownloadsWnd::OnMDIActivate(BOOL bActivate, CWnd *pActivateWnd,
                                  CWnd *pDeactivateWnd) {
...
void CDownloadsWnd::OnMeasureItem(int nIDCtl,
  ...
  AfxGetMainWnd()->SendMessage(WM_MEASUREITEM, nIDCtl,
...
void CDownloadsWnd::OnContextMenu(CWnd * /*pWnd*/, CPoint point) {
  ...
  CStringList pList;
  {
    ...
    if ((point.x < 0 && point.y < 0)
            ? !bControl && m_wndDownloads.GetAt(m_wndDownloads.m_nFocus,
                                                &pDownload, &pSource)
            : m_wndDownloads.HitTest(ptLocal, &pDownload, &pSource, NULL,
                                     NULL)) {
  ...
  Skin.TrackPopupMenu(ContextMenus[nMenu].szMenu, point,
                      ContextMenus[nMenu].nDefaultID, pList);
}
...
void CDownloadsWnd::Prepare() {
  ...
  m_bSelAny = m_bSelDownload = m_bSelSource = m_bSelTrying = m_bSelPaused =
      FALSE;
  m_bSelNotPausedOrMoving = m_bSelNoPreview = m_bSelNotCompleteAndNoPreview =
      FALSE;
  m_bSelCompletedAndNoPreview = m_bSelStartedAndNotMoving = m_bSelCompleted =
      FALSE;
  ...
  m_bSelSourceAcceptConnections = m_bSelSourceExtended = m_bSelHasReviews =
      FALSE;
  ...
  for (POSITION pos = Downloads.GetIterator(); pos;) {
    ...
    for (POSITION posSource = pDownload->GetIterator(); posSource;) {
      ...
      // Check if we could get remote previews (only from the connected sources
      // for the efficiency)
      if (!bPreviewDone && pDownload->m_bSelected &&
    ...
    if (pDownload->m_bSelected) {
      m_bSelRemotePreviewCapable =
          pDownload->m_bRemotePreviewCapable || pDownload->GotPreview();
      bPreviewDone = TRUE;
    }
  }
...
void CDownloadsWnd::OnUpdateDownloadsResume(CCmdUI *pCmdUI) {
  ...
  if (CCoolBarItem *pcCmdUI = CCoolBarItem::FromCmdUI(pCmdUI))
    pcCmdUI->Show(
        m_bSelPaused || (m_bSelDownload && !m_bSelTrying && !m_bSelCompleted) ||
...
void CDownloadsWnd::OnDownloadsResume() {
  ...
  for (POSITION pos = Downloads.GetIterator(); pos;) {
    ...
    // We must checkexplicitily if the download is Trying (as we ignore the
    // setting in group selection)
    if (pDownload->m_bSelected && !pDownload->IsTrying()) {
      pDownload->Resume();
    } else if (pDownload->IsTrying()) {
      theApp.Message(MSG_INFO,
                     _T("Cannot resume an already searching download"));
    }
  }
...
void CDownloadsWnd::OnUpdateDownloadsPause(CCmdUI *pCmdUI) {
  ...
  if (CCoolBarItem *pcCmdUI = CCoolBarItem::FromCmdUI(pCmdUI))
    pcCmdUI->Show(!m_bSelCompletedAndNoPreview &&
                  (m_bSelNotPausedOrMoving || !m_bSelDownload));
...
void CDownloadsWnd::OnDownloadsClear() {
  ...
  while (!pList.IsEmpty()) {
...

    {
      ...
      if (!Downloads.Check(pDownload1) || pDownload1->IsTasking() ||
          pDownload1->IsPreviewVisible()) {
        pList.RemoveHead();
        continue;
    ...
    for (INT_PTR nProcess = dlg.m_bAll ? pList.GetCount() : 1;
      ...
      CQuickLock pTransfersLock(
          Transfers.m_pSection); // Can clear uploads and downloads
      {
        if (Downloads.Check(pDownload2) && !pDownload2->IsTasking() &&
            !pDownload2->IsPreviewVisible()) {
          dlgProgress.Progress(pDownload2->m_sName, nTotal - pList.GetCount(),
                               nTotal);
...
void CDownloadsWnd::OnDownloadsClearIncomplete() {
  ...
  while (!pList.IsEmpty()) {
...

    {
      ...
      if (!Downloads.Check(pDownload1) || pDownload1->IsTasking() ||
          pDownload1->IsPreviewVisible() || pDownload1->IsCompleted()) {
        pList.RemoveHead();
        continue;
    ...
    CProgressDialog dlgProgress(LoadString(ID_DOWNLOADS_CLEAR_INCOMPLETE) +
                                _T(".."));
    ...
    for (INT_PTR nProcess = dlg.m_bAll ? pList.GetCount() : 1;
      ...
      {
        if (Downloads.Check(pDownload2) && !pDownload2->IsTasking() &&
            !pDownload2->IsPreviewVisible() && !pDownload2->IsCompleted()) {
          dlgProgress.Progress(pDownload2->m_sName, nTotal - pList.GetCount(),
                               nTotal);
      ...
      if (!pSource->m_bPreviewRequestSent && pSource->IsOnline()) {
        if (pSource->m_nProtocol == PROTOCOL_ED2K) {
          ...
          const CDownloadTransferED2K *pEDTransfer =
              static_cast<const CDownloadTransferED2K *>(
                  pSource->GetTransfer());
          if (pEDTransfer->m_pClient->m_bEmPreview) {
            ...
            break;
                   ...
                   pSource->m_bPreview && pDownload->m_oSHA1) {
          if (pSource->m_sPreview.IsEmpty()) {
            pSource->m_sPreview.Format(
                _T("http://%s:%u/gnutella/preview/v1?%s"),
                (LPCTSTR)CString(inet_ntoa(pSource->m_pAddress)),
                pSource->m_nPort, (LPCTSTR)pDownload->m_oSHA1.toUrn());
          }
...
void CDownloadsWnd::OnUpdateDownloadsLaunch(CCmdUI *pCmdUI) {
  ...
  if (CCoolBarItem *pcCmdUI = CCoolBarItem::FromCmdUI(pCmdUI))
    pcCmdUI->Show(!m_bSelCompletedAndNoPreview &&
                  (m_bSelStartedAndNotMoving || !m_bSelCompleted));
  pCmdUI->Enable(!m_bSelCompletedAndNoPreview &&
                 (m_bSelCompleted || m_bSelStartedAndNotMoving));
}
...
void CDownloadsWnd::OnDownloadsLaunchComplete() { OnDownloadsLaunchCopy(); }
    ...
    if (pDownload->m_bSelected) {
      if (!pDownload->IsMoving()) {
        ...
        if (m_nMoreSourcesLimiter >= 0) {
          ...
          // Warn user
          theApp.Message(MSG_DEBUG, _T("Find more sources unable to start due ")
                                    _T("to excessive network traffic"));
          // Prevent ed2k bans, client drops, etc.
          m_tMoreSourcesTimer = GetTickCount();
          if (m_nMoreSourcesLimiter < -30)
            m_nMoreSourcesLimiter = -30;
        }
      }
    }
...
void CDownloadsWnd::OnUpdateDownloadsCopy(CCmdUI *pCmdUI) {
  ...
  pCmdUI->SetText(
      LoadString(((m_nSelectedSources + m_nSelectedDownloads) == 1 && !bShift)
                     ? IDS_LIBRARY_COPYURI
                     : IDS_LIBRARY_EXPORTURIS));
}
...

  {
    ...
    for (POSITION pos = Downloads.GetIterator(); pos;) {
      ...
      if (pDownload->m_bSelected && Downloads.Check(pDownload) &&
          (pDownload->m_oSHA1 || pDownload->m_oTiger || pDownload->m_oED2K ||
           pDownload->m_oBTH || pDownload->m_oMD5 ||
           pDownload->m_sName.GetLength())) {
        pList.AddTail(*pDownload);
      }
...
void CDownloadsWnd::OnUpdateDownloadsEdit(CCmdUI *pCmdUI) {
  Prepare();
  pCmdUI->Enable((!m_bSelCompleted || m_bSelTorrent) && m_bSelNotMoving &&
                 m_nSelectedDownloads == 1);
}
...
void CDownloadsWnd::OnDownloadsMoveUp() { m_wndDownloads.MoveSelected(-1); }
...
void CDownloadsWnd::OnDownloadsMoveDown() { m_wndDownloads.MoveSelected(1); }
    ...
    for (POSITION posSource = pDownload->GetIterator(); posSource;) {
      ...
      if (pSource->m_bSelected) {
        if (pSource->m_nProtocol == PROTOCOL_HTTP) // HTTP chat
          ChatWindows.OpenPrivate(
              Hashes::Guid(), &pSource->m_pAddress, pSource->m_nPort,
              pSource->m_bPushOnly, pSource->m_nProtocol,
              &pSource->m_pServerAddress, pSource->m_nServerPort);
        else if (pSource->m_bClientExtended &&
                 !pSource->m_bPushOnly) // Client accepts G2 chat connections
          ChatWindows.OpenPrivate(Hashes::Guid(), &pSource->m_pAddress,
                                  pSource->m_nPort, FALSE, PROTOCOL_G2);
    ...
    for (POSITION posSource = pDownload->GetIterator(); posSource;) {
      ...
      if (pSource->m_bSelected) {
        ...
        if (pSource->m_nProtocol == PROTOCOL_HTTP ||
            pSource->m_nProtocol ==
                PROTOCOL_ED2K) // Many HTTP clients support this
          new CBrowseHostWnd(pSource->m_nProtocol, &pAddress,
                             pSource->m_bPushOnly, pSource->m_oGUID);
        else if (pSource->m_bClientExtended) // Over other protocols, you can
                                             // only contact non-push G2 clients
          new CBrowseHostWnd(pSource->m_nProtocol, &pAddress, FALSE,
                             Hashes::Guid());
      }
    }
  }
...
void CDownloadsWnd::OnDownloadsClearPaused() {
  ...
  if (AfxMessageBox(strMessage, MB_ICONQUESTION | MB_YESNO | MB_DEFBUTTON2) ==
      IDYES) {
...
void CDownloadsWnd::DragDownloads(CList<CDownload *> *pList, CImageList *pImage,
