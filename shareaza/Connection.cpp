...
// CConnection holds a socket used to communicate with a remote computer, and is
// the root of a big inheritance tree
...
CConnection::CConnection(PROTOCOLID nProtocol)
    : m_bInitiated(FALSE), m_bConnected(FALSE), m_tConnected(0),
      m_hSocket(INVALID_SOCKET), m_pInput(NULL), m_pOutput(NULL),
      m_bClientExtended(FALSE), m_nQueuedRun(0), m_nProtocol(nProtocol),
      m_nDelayCloseReason(0), m_bAutoDelete(FALSE) {
...
void CConnection::LogOutgoing() {
  if (!theApp.IsLogDisabled(MSG_DEBUG | MSG_FACILITY_OUTGOING)) {
    ...
    if (pOutput->m_nLength) {
      ...
      theApp.Message(MSG_DEBUG | MSG_FACILITY_OUTGOING, _T("%s << %s"),
                     (LPCTSTR)m_sAddress, (LPCTSTR)CString(msg));
    }
  }
}
...
// Takes pHost, a pointer to a SOCKADDR_IN structure, which is MFC's way of
// holding an IP address and port number Returns true if connected
...
// Connect this CConnection object to a remote computer on the Internet
// Takes pAddress, a Windows Sockets structure that holds an IP address, and
// takes the port number seprately Returns true if connected
BOOL CConnection::ConnectTo(const IN_ADDR *pAddress, WORD nPort) {
  ...
  // The IP address is in the security list of government and corporate
  ...
  if (Security.IsDenied(pAddress)) {
    ...
    theApp.Message(MSG_ERROR, IDS_SECURITY_OUTGOING,
                   (LPCTSTR)CString(inet_ntoa(*pAddress)));
    return FALSE;
  }

  // The IN_ADDR structure we just got passed isn't the same as the one already
  // stored in this object
  if (pAddress != &m_pHost.sin_addr) {
    // Zero the memory of the entire SOCKADDR_IN structure m_pHost, and then
    // copy in the sin_addr part
    ZeroMemory(&m_pHost, sizeof(m_pHost));
  ...
  m_pHost.sin_port =
      htons(nPort); // Copy the port number into the m_pHost structure
  m_sAddress =
      inet_ntoa(m_pHost.sin_addr); // Save the IP address as a string of text
  UpdateCountry();
  ...
  // If the OutHost string in connection settings has an IP address written in
  // it
  if (Settings.Connection.OutHost.GetLength()) {
    // Read the text and copy the IP address and port into a new local MFC
    // SOCKADDR_IN structure called pOutgoing
    ...
    if (pOutgoing.sin_addr.S_un.S_addr) {
      // Call bind in Windows Sockets to associate the local address with the
      // socket
      bind(m_hSocket,              // Our socket
           (SOCKADDR *)&pOutgoing, // The IP address this computer appears to
                                   // have on the Internet (do)
           sizeof(SOCKADDR_IN)); // Tell bind how many bytes it can read at the
                                 // pointer
    }
  }
  ...
  if (WSAConnect(m_hSocket,            // Our socket
  ...
  {
    // If no error occurs, WSAConnect returns 0, so if we're here an error
    // happened
    int nError = WSAGetLastError(); // Get the last Windows Sockets error number

    // An error of "would block" is normal because connections can't be made
    // instantly and this is a non-blocking socket
...
BOOL CConnection::ConnectTo(const IN6_ADDR *pAddress, WORD nPort) {
  ...
  m_pHostIPv6.sin6_port =
      htons(nPort); // Copy the port number into the m_pHost structure
  m_sAddress = IPv6ToString(
      &m_pHostIPv6.sin6_addr); // Save the IP address as a string of text
...
// When WSAAccept accepted the connection, it created a new socket hSocket for
// it and wrote the remote IP in pHost
...
void CConnection::AcceptFrom(SOCKET hSocket, SOCKADDR_IN6 *pHost) {
  ...
  m_sAddress =
      IPv6ToString(&m_pHostIPv6.sin6_addr); // Store it as a string also
  UpdateCountry();
...
void CConnection::Close(UINT nError) {
  // Make sure this object exists, and is located entirely within the program's
  // memory space
  ASSERT(this != NULL);
  ...
  if (nError) {
    if (nError == IDS_SECURITY_BANNED_USERAGENT) {
      CString sComment;
      sComment.Format(IDS_SECURITY_BANNED_USERAGENT, (LPCTSTR)m_sAddress,
                      (LPCTSTR)m_sUserAgent);
      Security.Ban(&m_pHost.sin_addr, ban2Hours, FALSE, sComment);

      theApp.Message(MSG_ERROR, IDS_SECURITY_BANNED_USERAGENT,
                     (LPCTSTR)m_sAddress, (LPCTSTR)m_sUserAgent);
    } else if (nError == IDS_HANDSHAKE_REJECTED)
      theApp.Message(MSG_ERROR, IDS_HANDSHAKE_REJECTED, (LPCTSTR)m_sAddress,
                     (LPCTSTR)m_sUserAgent);
    else {
      BOOL bInfo = (nError == IDS_CONNECTION_CLOSED ||
                    nError == IDS_CONNECTION_PEERPRUNE);
...
// Close the connection, but not until we've written the buffered outgoing data
// first Takes the reason we're closing the connection, or 0 by default
...
// Talk to the other computer, write the output buffer to the socket and read
// from the socket to the input buffer Return true if this worked, false if
...
BOOL CConnection::DoRun() {
  ...
  // Setup pEvents to store the socket's internal information about network
  // events
  ...
  if (pEvents.lNetworkEvents & FD_CONNECT) {
    ...
    m_bConnected = TRUE;
    m_tConnected = m_mInput.tLast = m_mOutput.tLast =
        GetTickCount(); // Store the time 3 places
  ...
  // If the FD_CLOSE network event has occurred, set bClosed to true, otherwise
  // set it to false
  BOOL bClosed = (pEvents.lNetworkEvents & FD_CLOSE) ? TRUE : FALSE;

  // If the close event happened, null a pointer within the TCP bandwidth meter
  // for input (do)
  if (bClosed)
  ...
  // Write the contents of the output buffer to the remote computer, and read in
  // data it sent us
  if (!OnWrite())
    return FALSE;
  if (!OnRead())
  ...
  if (bClosed) {
    // theApp.Message( MSG_DEBUG, _T("socket close() error %i"),
    // pEvents.iErrorCode[ FD_CLOSE_BIT ] ); Call OnDropped, telling it true if
    // there is a close error
  ...
  // If the queued run state is 2 and OnWrite returns false, leave here with
  // false also
...
void CConnection::QueueRun() {
  // If the queued run state is 1 or 2, make it 2, if it's 0, call OnWrite now
  // (do)
  if (m_nQueuedRun)
    m_nQueuedRun =
        2; // The queued run state is not 0, make it 2 and do nothing else here
  else
...
// Objects that inherit from CConnection have OnConnected methods that do
// things, unlike this one
BOOL CConnection::OnConnected() {
  // Just return true
  return TRUE;
}

// Objects that inherit from CConnection have OnDropped methods that do things,
// unlike this one
void CConnection::OnDropped() {
  // Do nothing
}

// Objects that inherit from CConnection have OnRun methods that do things,
// unlike this one
...
BOOL CConnection::OnRead() {
  ...
  // If we need to worry about throttling bandwidth, calculate nLimit, the
  // number of bytes we are allowed to read now
  if (m_mInput.pLimit     // If there is a limit
      && *m_mInput.pLimit // And that limit isn't 0
      && Settings.Live.BandwidthScale <=
             100) // And the bandwidth scale isn't at MAX
  {
  ...
  while (nLimit) {
    ...
    if (nLength)
      ...
      nLength =
          min(nLimit, 16384ul); // Receive up to 16KB blocks from the socket
    ...
    int nRead =
        CNetwork::Recv(m_hSocket, (char *)m_pInput->GetDataEnd(), (int)nLength);
...
BOOL CConnection::OnSSLRead() {
  ...
  while (nLimit) {
    ...
    // Read the bytes from the socket and record how many are actually read
    int nRead = CNetwork::SSLRecv(sslConnection, (char *)m_pInput->GetDataEnd(),
                                  (int)nLength);
...
BOOL CConnection::OnWrite() {
  ...
  // If we need to worry about throttling bandwidth, calculate nLimit, the
  // number of bytes we are allowed to write now
...
BOOL CConnection::OnSSLWrite() {
  ...
  while (nLimit) {
    // Send the bytes to the socket and record how many are actually sent
    int nSend =
        CNetwork::SSLSend(sslConnection, (const char *)pData, (int)nLimit);
...
void CConnection::Measure() {
  ...
  m_mInput.nMeasure =
      m_mInput.CalculateUsage(tCutoff) / (METER_PERIOD / METER_SECOND);
  m_mOutput.nMeasure =
      m_mOutput.CalculateUsage(tCutoff) / (METER_PERIOD / METER_SECOND);
}
...
BOOL CConnection::ReadHeaders() {
  CString strLine;
  while (
      Read(strLine)) // ReadLine will return false when there are no more lines
  {
    // If the line is more than 256 KB, change it to the line too long error
    // code
    if (strLine.GetLength() > 256 * 1024)
    ...
    if (strLine.IsEmpty()) {
      ...
      // Call the OnHeadersComplete method for the most advanced class that
      // inherits from CConnection
      return OnHeadersComplete();
    }                                     // The line starts with a space
    else if (_istspace(strLine.GetAt(0))) // Get the first character in the
                                          // string, and see if its a space
    {
      // The last header has length
      if (m_sLastHeader.GetLength()) {
        // Trim the spaces from both ends of the line, and see if it still has
        // length
        strLine.TrimLeft();
        strLine.TrimRight();
        if (strLine.GetLength() > 0) {
          ...
          if (!OnHeaderLine(m_sLastHeader, strLine))
            return FALSE;
             ...
             nPos <
                 64) // ":a" is 0 and "a:a" is 1, but "aa:a" is greater than 1
    {
...
BOOL CConnection::OnHeaderLine(CString &strHeader, CString &strValue) {
  theApp.Message(MSG_DEBUG | MSG_FACILITY_INCOMING, _T("%s >> %s: %s"),
  ...
  if (strHeader.CompareNoCase(_T("User-Agent")) == 0) {
    ...
    m_sUserAgent =
        strValue; // This tells what software the remote computer is running
    ...
    Network.AcquireLocalAddress(strValue, 0, &m_pHost.sin_addr,
                                &m_pHostIPv6.sin6_addr);

  } // It's the x my address, listen IP, or node header, like
    ...
    // If the remote computer first contacted us and the colon is there but not
    // first
    if (!m_bInitiated && nColon > 0) {
      // Read the number after the colon into nPort
      int nPort = protocolPorts[PROTOCOL_G2];
      if (_stscanf(strValue.Mid(nColon + 1), _T("%d"), &nPort) ==
              1          // Make sure 1 number was found
          && nPort != 0) // Make sure the found number isn't 0
      {
        // Save the found port number in m_pHost
        m_pHost.sin_port =
            htons(u_short(nPort)); // Convert Windows little endian to big for
                                   // the Internet with htons
      }
    }
...
// Classes that inherit from CConnection override this virtual method, adding
// code specific to them Returns true
BOOL CConnection::OnHeadersComplete() {
  // Just return true, it's CShakeNeighbour::OnHeadersComplete() that usually
  // gets called instead of this method
  return TRUE;
}
...
// Returns true if we are listening on a port and the header was sent, false
// otherwise
BOOL CConnection::SendMyAddress() {
  ...
  if (Network.IsListening()) {
    ...
    if (IsIPv6Host()) {
      strHeader.Format(
          _T("Listen-IP: %s\r\n"), // Make it like "Listen-IP:
                                   // [FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF]:6346\r\n"
          (LPCTSTR)HostToString(&Network.m_pHostIPv6));

      // Print the line into the bottom of the output buffer
      Write(strHeader); // It will be sent to the remote computer on the next
                        // write
    } else {
      strHeader.Format(
          _T("Listen-IP: %s:%hu\r\n"), // Make it like
                                       // "Listen-IP: 67.176.34.172:6346\r\n"
          (LPCTSTR)CString(inet_ntoa(Network.GetMyAddressFor(
              &m_pHost
                   .sin_addr))), // Insert the IP address like "67.176.34.172"
...
DWORD CConnection::TCPBandwidthMeter::CalculateLimit(DWORD tNow,
                                                     bool bMaxMode) const {
  ...
  if (Settings.Live.BandwidthScale <
      100) // The scale is turned down and we should use it
  {
  ...
  if (bMaxMode) {
    ...
    // nData = speed limit in bytes per second - bytes we read in the last
    // second nLimit = speed limit in bytes per second * elapsed time return the
    // smaller of the two
...
// Count the #bytes used for a given time period ( optimal for time periods more
// than METER_LENGTH / 2 )
...
// Count the #bytes used for a given time period ( optimal for time periods less
// than METER_LENGTH / 2 )
DWORD CConnection::TCPBandwidthMeter::CalculateUsage(
    DWORD tTime, bool /*bShortPeriod*/) const {
  // Exit early if the last slot used is older than the time limit
  if (tLastSlot <= tTime)
    return 0;
  ...
  // need to read 12 ( METER_MINIMUM / METER_SECOND + 2 )records instead of all
  // of them
  DWORD nData = 0;           // #bytes in the time period
  ...
  while (slot--) {
    if (pTimes[slot] > tTime)  // Is this within the time period
      nData += pHistory[slot]; //   It was, add it to #bytes
             ...
             nPosition) // It wasn't, did we start with the latest reading
      slot =
          nPosition + 1; //   We didn't, jump to our latest reading and continue
    else
...
void CConnection::TCPBandwidthMeter::Add(const DWORD nBytes, const DWORD tNow) {
  if (tNow - tLastSlot < METER_MINIMUM) {
    ...
    nPosition = (nPosition + 1) %
                METER_LENGTH;     // Move to the next position in the array
