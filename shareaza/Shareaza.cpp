...
CShareazaCommandLineInfo::CShareazaCommandLineInfo()
    : m_bTray(FALSE), m_bNoSplash(FALSE), m_bNoAlphaWarning(FALSE),
      m_nGUIMode(-1), m_bHelp(FALSE), m_bWait(FALSE) {}

void CShareazaCommandLineInfo::ParseParam(const TCHAR *pszParam, BOOL bFlag,
                                          BOOL bLast) {
  if (bFlag) {
...
CShareazaApp::CShareazaApp()
    : m_pMutex(NULL), m_nFontQuality(DEFAULT_QUALITY), m_pSafeWnd(NULL),
      m_bBusy(0), m_bInteractive(false), m_bLive(false), m_bClosing(false),
      m_bIsServer(false), m_bIsWin2000(false), m_bIsVistaOrNewer(false),
      m_bIs7OrNewer(false), m_bLimitedConnections(false),
      m_bMenuWasVisible(FALSE), m_nLastInput(0ul), m_hHookKbd(NULL),
      m_hHookMouse(NULL), m_pPacketWnd(NULL)
      ...
      ,
      m_hTheme(NULL), m_pfnSetWindowTheme(NULL), m_pfnIsThemeActive(NULL),
      m_pfnOpenThemeData(NULL), m_pfnCloseThemeData(NULL),
      m_pfnDrawThemeBackground(NULL), m_pfnGetThemeSysFont(NULL)

      ,
      m_hShlWapi(NULL), m_pfnAssocIsDangerous(NULL)

      ,
      m_hShell32(NULL), m_pfnSHGetFolderPathW(NULL),
      m_pfnSHGetKnownFolderPath(NULL), m_pfnSHCreateItemFromParsingName(NULL),
      m_pfnSHGetPropertyStoreFromParsingName(NULL),
      m_pfnSetCurrentProcessExplicitAppUserModelID(NULL),
      m_pfnSHGetImageList(NULL)

      ,
      m_hUser32(NULL), m_pfnChangeWindowMessageFilter(NULL),
      m_pfnShutdownBlockReasonCreate(NULL),
      m_pfnShutdownBlockReasonDestroy(NULL)

      ,
      m_hGeoIP(NULL), m_pGeoIP(NULL), m_pfnGeoIP_cleanup(NULL),
      m_pfnGeoIP_delete(NULL), m_pfnGeoIP_country_code_by_ipnum(NULL),
  ...
  const DWORDLONG dwlMajorMinorPack = VerSetConditionMask(
      VerSetConditionMask(
  ...
  m_bIsVistaOrNewer = VerifyVersionInfo(&osvi,
                                        VER_MAJORVERSION | VER_MINORVERSION |
                                            VER_SERVICEPACKMAJOR,
                                        dwlMajorMinorPack) != FALSE;

  // Windows 7
  osvi.dwMajorVersion = 6;
  osvi.dwMinorVersion = 1;
  m_bIs7OrNewer = VerifyVersionInfo(&osvi,
                                    VER_MAJORVERSION | VER_MINORVERSION |
                                        VER_SERVICEPACKMAJOR,
                                    dwlMajorMinorPack) != FALSE;
  ...
  if (!VerifyVersionInfo(&osvi, VER_PRODUCT_TYPE,
  ...
  if (bCanBeRegistryPatched) {
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE,
      ...
      if ((RegQueryValueEx(hKey, L"EnableConnectionRateLimiting", NULL, &nType,
                           (LPBYTE)&nResult, &nSize) == ERROR_SUCCESS) &&
...
BOOL CShareazaApp::InitInstance() {
  ...
  AfxEnableControlContainer(
      m_pFontManager); // Enable support for containment of OLE controls.
  ...
  if (Settings.Live.FirstRun || !pTest) {
    ...
    if (!pTest) {
      Plugins.Register(
          m_strBinaryPath.Left(m_strBinaryPath.ReverseFind(_T('\\'))));
    }
  }

  Skin.CreateDefault();     // Loads colors, fonts and language. Depends on
                            // InitFonts().
  Register();               // Re-register Shareaza Type Library
  LoadStdProfileSettings(); // Load MRU file list and last preview state.
  EnableShellOpen(); // Enable open data files when user double-click the files
  ...
  if (!m_cmdInfo.m_bNoAlphaWarning && (tCompileTime + tTimeOut) < tCurrent) {
    if (MsgBox(_T("This is a pre-release version of ") MOD_CLIENT_NAME_T
               _T(", and the beta testing period has ended.  ")
               _T("Please download the full, official release from ") WEB_SITE_T
               _T("."),
               MB_ICONQUESTION | MB_OK, 0, NULL, 30) != IDOK)
      return FALSE;
  }

  // Alpha warning. Remember to remove this section for final releases and
  // public betas.
  if (!m_cmdInfo.m_bNoAlphaWarning && m_cmdInfo.m_bShowSplash) {
    if (MsgBox(
            _T("WARNING: This is an ALPHA TEST version of ") MOD_CLIENT_NAME_T
            _T(".\n\n")
            _T("It is NOT FOR GENERAL USE, and is only for testing specific ")
            _T("features in a controlled ")
            _T("environment. It will frequently stop running, or display ")
            _T("debug information to assist testing.\n\n")
            _T("If you wish to actually use this software, you should ")
            _T("download ")
            _T("the current stable release from ") WEB_SITE_T _T("\n")
            _T("If you continue past this point, you may experience system ")
            _T("instability, lose downloads, ")
            _T("or corrupt system files. Corrupted downloads/files may not be ")
            _T("recoverable. ")
            _T("Do you wish to continue?"),
            MB_ICONEXCLAMATION | MB_YESNO, 0, NULL, 30) == IDNO)
      return FALSE;
  }
#endif // RELEASE_BUILD
  ...
  SplashStep(L"Network",
             ((m_cmdInfo.m_bNoSplash || !m_cmdInfo.m_bShowSplash) ? 0 : 16),
             false);
  if (!Network.Init()) {
    SplashAbort();
    AfxMessageBox(_T("Failed to initialize Windows Sockets."),
                  MB_ICONHAND | MB_OK);
    return FALSE;
  }
  ...
  if (!Settings.MediaPlayer.FileTypes.size()) {
    CString sTypeFilter;
    static const LPCTSTR szTypes[] = {CSchema::uriAudio, CSchema::uriVideo,
                                      NULL};
  ...
  if (!Settings.Library.SafeExecute.size()) {
    CString sTypeFilter;
    static const LPCTSTR szTypes[] = {CSchema::uriArchive,    CSchema::uriAudio,
                                      CSchema::uriVideo,      CSchema::uriBook,
                                      CSchema::uriCollection, CSchema::uriImage,
                                      CSchema::uriBitTorrent, NULL};
    for (int i = 0; szTypes[i]; ++i)
      if (CSchemaPtr pSchema = SchemaCache.Get(szTypes[i]))
        sTypeFilter += pSchema->GetFilterSet();
  ...
  TRY m_pMainWnd = new CMainWnd();
  if (m_pMainWnd) {
    // Bypass CMDIFrameWnd::LoadFrame
    if (((CMainWnd *)m_pMainWnd)
            ->CFrameWnd::LoadFrame(IDR_MAINFRAME, WS_OVERLAPPEDWINDOW)) {
  ...
  if (!bFirst) {
    if (hwndFirst) {
      if (m_cmdInfo.m_nShellCommand == CCommandLineInfo::FileOpen) {
        ...
        COPYDATASTRUCT cd = {
            COPYDATA_OPEN, m_cmdInfo.m_strFileName.GetLength() * sizeof(TCHAR),
            (PVOID)(LPCTSTR)m_cmdInfo.m_strFileName};
        DWORD_PTR dwResult;
        SendMessageTimeout(hwndFirst, WM_COPYDATA, NULL, (WPARAM)&cd,
                           SMTO_NORMAL, 250, &dwResult);
      } else {
        // Popup first instance
        DWORD_PTR dwResult;
        SendMessageTimeout(hwndFirst, WM_COMMAND, ID_TRAY_OPEN, 0, SMTO_NORMAL,
                           250, &dwResult);
...
BOOL CShareazaApp::Register() {
  ...
  if (theApp.m_bIs7OrNewer) {
    ...
    //	oTasks.AddTask( _T("shareaza:command:search"), _T(""), LoadString(
    //IDS_SEARCH_TASK ) + _T("..."), theApp.m_strBinaryPath, - IDR_SEARCHFRAME
    //); 	oTasks.AddTask( _T("shareaza:command:download"), _T(""), LoadString(
    //IDS_DOWNLOAD_TASK ) + _T("..."), theApp.m_strBinaryPath, -
    //IDR_DOWNLOADSFRAME );
    ...
    if ( SUCCEEDED( pList.CoCreateInstance( CLSID_DestinationList ) ) )
    {
            ...
            VERIFY( SUCCEEDED( pList->BeginList( &nMinSlots, IID_IObjectArray,
    (LPVOID*)&pRemoved ) ) ); VERIFY( SUCCEEDED( pList->AppendKnownCategory(
    KDC_RECENT ) ) );

            CComPtr< IObjectCollection > pTasks;
            if ( SUCCEEDED( pTasks.CoCreateInstance(
    CLSID_EnumerableObjectCollection ) ) )
            {
                    CComPtr< IShellLink > pSearch = CreateShellLink(
    _T("shareaza:command:search"), _T(""), LoadString( IDS_SEARCH_TASK ) +
    _T("..."), theApp.m_strBinaryPath, - IDR_SEARCHFRAME, _T("") ); ASSERT(
    pSearch ); if ( pSearch ) VERIFY( SUCCEEDED( pTasks->AddObject( pSearch ) )
    );

                    CComPtr< IShellLink > pDownload = CreateShellLink(
    _T("shareaza:command:download"), _T(""), LoadString( IDS_DOWNLOAD_TASK ) +
    _T("..."), theApp.m_strBinaryPath, - IDR_DOWNLOADSFRAME, _T("") ); ASSERT(
    pDownload ); if ( pDownload ) VERIFY( SUCCEEDED( pTasks->AddObject(
    pDownload ) ) );
...
void CShareazaApp::AddToRecentFileList(LPCTSTR lpszPathName) {
  ...
  /*/
      if ( Windows.dwMajorVersion > 6 || ( Windows.dwMajorVersion == 6 &&
  Windows.dwMinorVersion >= 1 ) )
      {
      if ( m_pfnSHCreateItemFromParsingName )
      {
              CComPtr< IShellItem > pItem;
              if ( SUCCEEDED( m_pfnSHCreateItemFromParsingName( lpszPathName,
  NULL, IID_IShellItem, (LPVOID*)&pItem ) ) )
              {
...
BOOL CShareazaApp::Open(LPCTSTR lpszFileName, BOOL bDoIt, BOOL bDisplay) {
  ...
  if (nLength > 8 && !_tcsicmp(lpszFileName + nLength - 8, _T(".torrent")))
    ...
    return OpenCollection(lpszFileName, bDoIt);
           ...
           !_tcsicmp(lpszFileName + nLength - 11, _T(".collection")))
    return OpenCollection(lpszFileName, bDoIt);
  else if (nLength > 16 &&
           !_tcsicmp(lpszFileName + nLength - 16, _T(".emulecollection")))
    return OpenCollection(lpszFileName, bDoIt);
  else if (nLength > 15 &&
           !_tcsicmp(lpszFileName + nLength - 15, _T("hublist.xml.bz2")))
...
BOOL CShareazaApp::OpenInternetShortcut(LPCTSTR lpszFileName, BOOL bDoIt) {
  ...
  BOOL bResult = (GetPrivateProfileString(_T("InternetShortcut"), _T("URL"),
                                          _T(""), sURL.GetBuffer(MAX_PATH),
                                          MAX_PATH, lpszFileName) > 3);
  sURL.ReleaseBuffer();
...
BOOL CShareazaApp::OpenDownload(LPCTSTR lpszFileName, BOOL bDoIt) {
  ...
  GetLongPathName(lpszFileName, strFileName.GetBuffer(MAX_PATH * 2),
                  MAX_PATH * 2);
  ...
  if (Downloads.FindBySDName(strSDName) == NULL) {
    ...
    if (CDownload *pDownload = Downloads.Load(lpszFileName)) {
      ...
      pDownload->m_sPath =
          Settings.Downloads.IncompletePath + _T("\\") + strSDName;
      if (pDownload->Save(TRUE)) {
        // Rename old file
        ::MoveFileEx(_T("\\\\?\\") + strFileName,
                     _T("\\\\?\\") + strFileName + _T(".sav"),
                     MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH);

        theApp.Message(MSG_NOTICE,
                       _T("Download file \"%s\" has been successfully loaded ")
                       _T("and saved as \"%s\"."),
                       (LPCTSTR)strFileName, (LPCTSTR)pDownload->m_sPath);
      } else
        theApp.Message(MSG_ERROR,
                       _T("Failed to save download file \"%s\" as \"%s\"."),
                       (LPCTSTR)strFileName, (LPCTSTR)pDownload->m_sPath);
    } else
      theApp.Message(MSG_ERROR, _T("Failed to load download file \"%s\"."),
                     (LPCTSTR)strFileName);
  } else
    theApp.Message(MSG_WARNING, _T("Download file already loaded \"%s\"."),
                   (LPCTSTR)strFileName);
...
void CShareazaApp::InitResources() {
  ...
  if (dwSize) {
    if (BYTE *pBuffer = new BYTE[dwSize]) {
      if (GetFileVersionInfo(m_strBinaryPath, NULL, dwSize, pBuffer)) {
        ...
        if (VerQueryValue(pBuffer, _T("\\"), (VOID **)&pTable,
                          (UINT *)&dwSize)) {
  ...
  m_sVersion.Format(_T("%u.%u.%u.%u"), m_nVersion[0], m_nVersion[1],
  ...
  if (HMODULE hKernel32 = GetModuleHandle(_T("kernel32.dll"))) {
    (FARPROC &)m_pRegisterApplicationRestart =
        GetProcAddress(hKernel32, "RegisterApplicationRestart");
  }
  ...
  if ((m_hTheme = LoadLibrary(_T("UxTheme.dll"))) != NULL) {
    ...
    (FARPROC &)m_pfnDrawThemeBackground =
        GetProcAddress(m_hTheme, "DrawThemeBackground");
    (FARPROC &)m_pfnGetThemeSysFont =
        GetProcAddress(m_hTheme, "GetThemeSysFont");
  }

  // Get pointers to some functions that require Internet Explorer 6.01 or
  // greater
  if ((m_hShlWapi = LoadLibrary(_T("shlwapi.dll"))) != NULL) {
    (FARPROC &)m_pfnAssocIsDangerous =
        GetProcAddress(m_hShlWapi, "AssocIsDangerous");
  }

  if ((m_hShell32 = LoadLibrary(_T("shell32.dll"))) != NULL) {
    (FARPROC &)m_pfnSHGetFolderPathW =
        GetProcAddress(m_hShell32, "SHGetFolderPathW");
    (FARPROC &)m_pfnSHGetKnownFolderPath =
        GetProcAddress(m_hShell32, "SHGetKnownFolderPath");
    (FARPROC &)m_pfnSHCreateItemFromParsingName =
        GetProcAddress(m_hShell32, "SHCreateItemFromParsingName");
    (FARPROC &)m_pfnSHGetPropertyStoreFromParsingName =
        GetProcAddress(m_hShell32, "SHGetPropertyStoreFromParsingName");
    (FARPROC &)m_pfnSetCurrentProcessExplicitAppUserModelID =
        GetProcAddress(m_hShell32, "SetCurrentProcessExplicitAppUserModelID");
    (FARPROC &)m_pfnSHGetImageList =
        GetProcAddress(m_hShell32, MAKEINTRESOURCEA(727));
  }

  if ((m_hUser32 = LoadLibrary(_T("user32.dll"))) != NULL) {
    (FARPROC &)m_pfnChangeWindowMessageFilter =
        GetProcAddress(m_hUser32, "ChangeWindowMessageFilter");
    (FARPROC &)m_pfnShutdownBlockReasonCreate =
        GetProcAddress(m_hUser32, "ShutdownBlockReasonCreate");
    (FARPROC &)m_pfnShutdownBlockReasonDestroy =
        GetProcAddress(m_hUser32, "ShutdownBlockReasonDestroy");
  }

  // Windows Vista: Enable drag-n-drop and window control operations from
  // application with lower security level
  if (theApp.m_pfnChangeWindowMessageFilter) {
    ...
    VERIFY(
        theApp.m_pfnChangeWindowMessageFilter(WM_COPYGLOBALDATA, MSGFLT_ADD));
  ...
  // We load it in a custom way, so Shareaza plugins can use this library also
  // when it isn't in its search path but loaded by CustomLoadLibrary (very
  // useful when running Shareaza inside Visual Studio)
  m_hLibGFL = CustomLoadLibrary(_T("libgfl340.dll"));

  CryptAcquireContext(&m_hCryptProv, NULL, NULL, PROV_RSA_FULL,
                      CRYPT_VERIFYCONTEXT);

  srand(GetTickCount());

  m_hHookKbd = SetWindowsHookEx(WH_KEYBOARD, (HOOKPROC)KbdHook, NULL,
                                AfxGetThread()->m_nThreadID);
  m_hHookMouse = SetWindowsHookEx(WH_MOUSE, (HOOKPROC)MouseHook, NULL,
                                  AfxGetThread()->m_nThreadID);
...
void CShareazaApp::InitFonts() {
  ...
  if (SystemParametersInfo(SPI_GETFONTSMOOTHING, 0, &bFontSmoothing, 0) &&
    ...
    m_nFontQuality =
        (nSmoothingType == FE_FONTSMOOTHINGSTANDARD)
            ? ANTIALIASED_QUALITY
            : ((nSmoothingType == FE_FONTSMOOTHINGCLEARTYPE) ? CLEARTYPE_QUALITY
                                                             : DEFAULT_QUALITY);
  }
  ...
  m_gdiFont.CreateFont(-(int)Settings.Fonts.FontSize, 0, 0, 0, FW_NORMAL, FALSE,
                       FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                       CLIP_DEFAULT_PRECIS, m_nFontQuality,
                       DEFAULT_PITCH | FF_DONTCARE, Settings.Fonts.DefaultFont);

  m_gdiFontBold.CreateFont(
      -(int)Settings.Fonts.FontSize, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE,
      DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, m_nFontQuality,
      DEFAULT_PITCH | FF_DONTCARE, Settings.Fonts.DefaultFont);

  m_gdiFontLine.CreateFont(
      -(int)Settings.Fonts.FontSize, 0, 0, 0, FW_NORMAL, FALSE, TRUE, FALSE,
      DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, m_nFontQuality,
      DEFAULT_PITCH | FF_DONTCARE, Settings.Fonts.DefaultFont);
}
...
HINSTANCE CShareazaApp::CustomLoadLibrary(LPCTSTR pszFileName) {
  ...
  if ((hLibrary = LoadLibrary(pszFileName)) != NULL ||
      (hLibrary =
           LoadLibrary(Settings.General.Path + _T("\\") + pszFileName)) != NULL)
    ;
  else
...
CMainWnd *CShareazaApp::SafeMainWnd() const {
  ...
  return static_cast<CMainWnd *>(IsWindow(m_pSafeWnd->m_hWnd) ? m_pSafeWnd
                                                              : NULL);
}
...
bool CShareazaApp::IsLogDisabled(WORD nType) const {
  return
      // Severity filter
      (static_cast<DWORD>(nType & MSG_SEVERITY_MASK) >
       Settings.General.LogLevel) ||
      // Facility filter
      ((nType & MSG_FACILITY_MASK) == MSG_FACILITY_SEARCH &&
       !Settings.General.SearchLog);
}
...
void CShareazaApp::ShowStartupText() {
  ...
  strBody.Replace(_T("(version)"), (LPCTSTR)(theApp.m_sVersion + _T(" (") +
                                             theApp.m_sBuildDate + _T(")")));
  ...
  PrintMessage(MSG_INFO,
               IsRunAsAdmin()
                   ? _T("Running with administrative privileges.")
                   : _T("Running without administrative privileges."));
}
...
void CShareazaApp::PrintMessage(WORD nType, const CString &strLog) {
  ...
  CFile pFile;
  if (pFile.Open(Settings.General.UserPath + _T("\\Data\\") CLIENT_NAME_T
                                             _T(".log"),
                 CFile::modeReadWrite)) {
    ...
    if ((Settings.General.MaxDebugLogSize) && // If log rotation is on
        (pFile.GetLength() >
         Settings.General.MaxDebugLogSize)) // and file is too long...
    {
      // Close the file
      pFile.Close();

      // Rotate the logs
      MoveFileEx(CString(_T("\\\\?\\")) + Settings.General.UserPath +
                     _T("\\Data\\") CLIENT_NAME_T _T(".log"),
                 CString(_T("\\\\?\\")) + Settings.General.UserPath +
                     _T("\\Data\\") CLIENT_NAME_T _T(".old.log"),
                 MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH);
      // Start a new log
      if (!pFile.Open(Settings.General.UserPath + _T("\\Data\\") CLIENT_NAME_T
                                                  _T(".log"),
  ...
  if (Settings.General.ShowTimestamp) {
    ...
    CString strLine;
    strLine.Format(_T("[%.2i:%.2i:%.2i] "), pNow.GetHour(), pNow.GetMinute(),
                   pNow.GetSecond());
    pFile.Write((LPCTSTR)strLine, sizeof(TCHAR) * strLine.GetLength());
  }

  pFile.Write((LPCTSTR)strLog,
              static_cast<UINT>(sizeof(TCHAR) * strLog.GetLength()));
  pFile.Write(_T("\r\n"), sizeof(TCHAR) * 2);
...
CString GetErrorString(DWORD dwError) {
  ...
  static LPCTSTR const szModules[] = {_T("netapi32.dll"), _T("netmsg.dll"),
                                      _T("wininet.dll"),  _T("ntdll.dll"),
                                      _T("ntdsbmsg.dll"), NULL};
  for (int i = 0; szModules[i]; i++) {
    HMODULE hModule =
        LoadLibraryEx(szModules[i], NULL, LOAD_LIBRARY_AS_DATAFILE);
    if (hModule) {
...
CString CShareazaApp::GetCountryCode(IN_ADDR pAddress) const {
  if (m_pfnGeoIP_country_code_by_ipnum && m_pGeoIP)
    return CString(
        m_pfnGeoIP_country_code_by_ipnum(m_pGeoIP, htonl(pAddress.s_addr)));
  return _T("");
}
...
CString CShareazaApp::GetCountryName(IN_ADDR pAddress) const {
  if (m_pfnGeoIP_country_name_by_ipnum && m_pGeoIP)
    return CString(
        m_pfnGeoIP_country_name_by_ipnum(m_pGeoIP, htonl(pAddress.s_addr)));
  return _T("");
}
...
void CShareazaApp::LoadCountry() {
  if ((m_hGeoIP = CustomLoadLibrary(_T("geoip.dll"))) != NULL) {
    GeoIP_newFunc pfnGeoIP_new =
        (GeoIP_newFunc)GetProcAddress(m_hGeoIP, "GeoIP_new");
    m_pfnGeoIP_cleanup =
        (GeoIP_cleanupFunc)GetProcAddress(m_hGeoIP, "GeoIP_cleanup");
    m_pfnGeoIP_delete =
        (GeoIP_deleteFunc)GetProcAddress(m_hGeoIP, "GeoIP_delete");
    m_pfnGeoIP_country_code_by_ipnum =
        (GeoIP_country_code_by_ipnumFunc)GetProcAddress(
            m_hGeoIP, "GeoIP_country_code_by_ipnum");
    m_pfnGeoIP_country_name_by_ipnum =
        (GeoIP_country_name_by_ipnumFunc)GetProcAddress(
            m_hGeoIP, "GeoIP_country_name_by_ipnum");
    m_pfnGeoIP_country_code_by_ipnum_v6 =
        (GeoIP_country_code_by_ipnumFunc_v6)GetProcAddress(
            m_hGeoIP, "GeoIP_country_code_by_ipnum_v6");
    m_pfnGeoIP_country_name_by_ipnum_v6 =
        (GeoIP_country_name_by_ipnumFunc_v6)GetProcAddress(
            m_hGeoIP, "GeoIP_country_name_by_ipnum_v6");
    if (pfnGeoIP_new)
    ...
    if (CLibraryFile *pFile = Library.LookupFile(nIndex)) {
      if (pFile->IsAvailable()) {
        ...
        CFileExecutor::Execute(strPath);
             ...
             strURI.Find(_T("mailto:")) == 0 || strURI.Find(_T("aim:")) == 0 ||
             strURI.Find(_T("magnet:")) == 0 || strURI.Find(_T("foxy:")) == 0 ||
             strURI.Find(_T("gnutella:")) == 0 ||
             strURI.Find(_T("gnet:")) == 0 ||
             strURI.Find(_T("shareaza:")) == 0 ||
             strURI.Find(_T("gwc:")) == 0 || strURI.Find(_T("uhc:")) == 0 ||
             strURI.Find(_T("ukhl:")) == 0 ||
             strURI.Find(_T("gnutella1:")) == 0 ||
             strURI.Find(_T("gnutella2:")) == 0 ||
             strURI.Find(_T("mp2p:")) == 0 || strURI.Find(_T("ed2k:")) == 0 ||
             strURI.Find(_T("dchub://")) == 0 ||
             strURI.Find(_T("sig2dat:")) == 0) {
    ShellExecute(pMainWnd->GetSafeHwnd(), _T("open"), strURI, NULL, NULL,
                 SW_SHOWNORMAL);
...
BOOL LoadString(CString &str, UINT nID) { return Skin.LoadString(str, nID); }
...
DWORD TimeFromString(LPCTSTR pszTime) {
  ...
  if (_tcslen(pszTime) != 17)
    return 0;
  if (pszTime[4] != '-' || pszTime[7] != '-')
    return 0;
  if (pszTime[10] != 'T' || pszTime[13] != ':' || pszTime[16] != 'Z')
    return 0;
  ...
  if (_stscanf(pszTime, _T("%i"), &nTemp) != 1)
    return 0;
  pTime.tm_year = nTemp - 1900;
  for (psz = pszTime + 5; *psz == '0'; psz++)
    ;
  if (_stscanf(psz, _T("%i"), &nTemp) != 1)
    return 0;
  ...
  tm pGM = {};
  if (tGMT == -1 || gmtime_s(&pGM, &tGMT) != 0 || (tSub = mktime(&pGM)) == -1) {
    return 0;
  }
...
CString TimeToString(time_t tVal) {
  ...
  if (gmtime_s(&time, &tVal) == 0) {
    str.Format(_T("%.4i-%.2i-%.2iT%.2i:%.2iZ"), time.tm_year + 1900,
...
CString TimeToString(FILETIME *pTime) {
  ...
  str.Format(_T("%.4u-%.2u-%.2uT%.2u:%.2uZ"), pOut.wYear, pOut.wMonth,
             pOut.wDay, pOut.wHour, pOut.wMinute);
...
void RecalcDropWidth(CComboBox *pWnd, int nMargin) {
  ...
  nWidth += tm.tmAveCharWidth + ::GetSystemMetrics(SM_CXVSCROLL) +
            2 * ::GetSystemMetrics(SM_CXEDGE) + nMargin;

  pWnd->SetDroppedWidth(nWidth);
}

BOOL LoadIcon(LPCTSTR szFilename, HICON *phSmallIcon, HICON *phLargeIcon,
              HICON *phHugeIcon, int nIcon) {
...
HICON LoadCLSIDIcon(LPCTSTR szCLSID) {
  ...
  if (RegOpenKeyEx(HKEY_CLASSES_ROOT, strPath, 0, KEY_READ, &hKey) !=
      ERROR_SUCCESS) {
...
HICON CreateMirroredIcon(HICON hIconOrig, BOOL bDestroyOriginal) {
        ...
        if (GetIconInfo(hIconOrig, &ii) &&
          ...
          if (hbm != NULL) {
            ...
            if (hbmMask != NULL) {
              ...
              DrawIconEx(hdcBitmap, 0, 0, hIconOrig, bm.bmWidth, bm.bmHeight, 0,
                         NULL, DI_IMAGE);
              DrawIconEx(hdcMask, 0, 0, hIconOrig, bm.bmWidth, bm.bmHeight, 0,
                         NULL, DI_MASK);
  ...
  if (hdcMask)
    DeleteDC(hdcMask);
  if (hIcon && hIconOrig && bDestroyOriginal)
    VERIFY(DestroyIcon(hIconOrig));
  if (!hIcon)
    hIcon = hIconOrig;
  return hIcon;
}
...
CString CShareazaApp::GetWindowsFolder() const {
  ...
  if (m_pfnSHGetKnownFolderPath) {
    PWSTR pPath = NULL;
    hr = m_pfnSHGetKnownFolderPath(FOLDERID_Windows, KF_FLAG_DONT_VERIFY, NULL,
                                   &pPath);
    if (pPath) {
      sWindows = pPath;
      CoTaskMemFree(pPath);
...
CString CShareazaApp::GetProgramFilesFolder64() const {
  ...
  if (m_pfnSHGetKnownFolderPath) {
    PWSTR pPath = NULL;
    hr = m_pfnSHGetKnownFolderPath(FOLDERID_ProgramFilesX64,
  ...
  // 32-bit way
  ExpandEnvironmentStrings(_T("%ProgramW6432%"),
                           sProgramFiles.GetBuffer(MAX_PATH), MAX_PATH);
...
CString CShareazaApp::GetProgramFilesFolder() const {
  ...
  if (m_pfnSHGetKnownFolderPath) {
    PWSTR pPath = NULL;
    hr = m_pfnSHGetKnownFolderPath(FOLDERID_ProgramFilesX86,
                                   KF_FLAG_DONT_VERIFY, NULL, &pPath);
    if (pPath) {
      sProgramFiles = pPath;
      CoTaskMemFree(pPath);
...
CString CShareazaApp::GetDocumentsFolder() const {
  ...
  // Legacy way
  sDocuments = CRegistry::GetString(
      _T("Shell Folders"), _T("Personal"), _T(""),
...
CString CShareazaApp::GetAppDataFolder() const {
  ...
  // Legacy way
  sAppData = CRegistry::GetString(
      _T("Shell Folders"), _T("AppData"), _T(""),
      _T("Software\\Microsoft\\Windows\\CurrentVersion\\Explorer"));
...
CString CShareazaApp::GetLocalAppDataFolder() const {
  ...
  // Legacy way
  sLocalAppData = CRegistry::GetString(
      _T("Shell Folders"), _T("Local AppData"), _T(""),
      _T("Software\\Microsoft\\Windows\\CurrentVersion\\Explorer"));
...
void CShareazaApp::OnRename(LPCTSTR pszSource, LPCTSTR pszTarget) {
  ...
  if (pszTarget == NULL) {
    ...
    if (CMainWnd *pMainWnd = theApp.SafeMainWnd()) {
      if (CMediaWnd *pMediaWnd = (CMediaWnd *)pMainWnd->m_pWindows.Find(
...
CDatabase *CShareazaApp::GetDatabase() const {
  return new CDatabase(Settings.General.UserPath + _T("\\Data\\") CLIENT_NAME_T
                                                   _T(".db3"));
}

BOOL CShareazaApp::GetPropertyStoreFromParsingName(LPCWSTR pszPath,
                                                   IPropertyStore **ppv) {
  if (m_pfnSHGetPropertyStoreFromParsingName) {
    __try {
      return SUCCEEDED(m_pfnSHGetPropertyStoreFromParsingName(
          pszPath, NULL, GPS_BESTEFFORT, __uuidof(IPropertyStore),
          (void **)ppv));
    } __except (EXCEPTION_EXECUTE_HANDLER) {
    }
  }
...
void DeleteFiles(CStringList &pList) {
  ...
  while (!pList.IsEmpty()) {
    ...
    for (INT_PTR nProcess = dlg.m_bAll ? pList.GetCount() : 1;
      ...
      {
        CQuickLock pTransfersLock(
            Transfers.m_pSection); // Can clear uploads and downloads
        {
          CQuickLock pLibraryLock(Library.m_pSection);
...
BOOL DeleteFileEx(LPCTSTR szFileName, BOOL bShared, BOOL bToRecycleBin,
                  BOOL bEnableDelayed) {
...
void PurgeDeletes() {
  HKEY hKey = NULL;
  LSTATUS nResult = RegOpenKeyEx(HKEY_CURRENT_USER, REGISTRY_KEY _T("\\Delete"),
  ...
  if (ERROR_SUCCESS == nResult) {
    ...
    for (DWORD nIndex = 0;; ++nIndex) {
      ...
      DWORD nMode = 8;
      TCHAR szMode[8] = {};
      nResult = RegEnumValue(hKey, nIndex, szPath, &nPath, 0, &nType,
                             (LPBYTE)szMode, &nMode);
      if (ERROR_SUCCESS != nResult)
        break;
    ...
    if (hMemory) {
      ...
      if (pszInput) {
        if (bGZIP) {
          ...
          if (buf.Ungzip()) {
            int nWide = MultiByteToWideChar(0, 0, (LPCSTR)buf.m_pBuffer,
                                            buf.m_nLength, NULL, 0);
            LPTSTR pszOutput = strBody.GetBuffer(nWide + 1);
            MultiByteToWideChar(0, 0, (LPCSTR)buf.m_pBuffer, buf.m_nLength,
                                pszOutput, nWide);
...
CString LoadRichHTML(UINT nResourceID, CString &strResponse,
                     CShareazaFile *pFile) {
  ...
  for (;;) {
    ...
    if (strReplace.CompareNoCase(_T("Client")) == 0)
      ...
      strReplace = Neighbours.GetNeighbourList(
          strReplace.Right(strReplace.GetLength() - 11));
...
bool ResourceRequest(const CString &strPath, CBuffer &pResponse,
                     CString &sHeader) {
  bool ret = false;
  for (int i = 0; WebResources[i].szPath; i++) {
    if (strPath.Compare(WebResources[i].szPath) == 0) {
      ...
      if (HRSRC hRes =
              FindResource(hModule, MAKEINTRESOURCE(WebResources[i].nID),
                           WebResources[i].szType)) {
        ...
        if (hMemory) {
          ...
          if (pSource) {
            if (WebResources[i].szType == RT_GZIP ||
              ...
              DWORD dwTotalSize =
                  sizeof(ICONDIR) + sizeof(ICONDIRENTRY) * piDir->idCount;
              pResponse.EnsureBuffer(dwTotalSize);
              CopyMemory(pResponse.m_pBuffer, piDir, sizeof(ICONDIR));

              GRPICONDIRENTRY *piDirEntry =
                  (GRPICONDIRENTRY *)(pSource + sizeof(ICONDIR));

              // Find all subicons
              for (WORD j = 0; j < piDir->idCount; j++) {
                // pResponse.m_pBuffer may be changed
                ICONDIRENTRY *piEntry =
                    (ICONDIRENTRY *)(pResponse.m_pBuffer + sizeof(ICONDIR));

                // Load subicon
                HRSRC hResIcon = FindResource(
                    hModule, MAKEINTRESOURCE(piDirEntry[j].nID), RT_ICON);
                if (hResIcon) {
                  DWORD nSizeIcon = SizeofResource(hModule, hResIcon);
                  HGLOBAL hMemoryIcon = LoadResource(hModule, hResIcon);
                  if (hMemoryIcon) {
                    BITMAPINFOHEADER *piImage =
                        (BITMAPINFOHEADER *)LockResource(hMemoryIcon);
                    ...
                    CopyMemory(pResponse.m_pBuffer + dwTotalSize, piImage,
                               nSizeIcon);
...
BOOL SaveIcon(HICON hIcon, CBuffer &oBuffer, int colors) {
  ASSERT(hIcon);
  ASSERT(colors == -1 || colors == 1 || colors == 4 || colors == 8 ||
         colors == 16 || colors == 24 || colors == 32);
  ...
  int palette =
      (colors == 8) ? 256 : ((colors == 4) ? 16 : ((colors == 1) ? 2 : 0));

  CAutoVectorPtr<char> pHeader(
      new char[sizeof(BITMAPINFOHEADER) + sizeof(RGBQUAD) * 256]);
  if (!pHeader)
    // Out of memory
    return FALSE;
  ...
  // Get mask bits
  bih.bmiHeader.biBitCount = 1;
  if (GetDIBits(hDC, ii.hbmMask, 0, cx,
                (char *)pBuffer + bih.bmiHeader.biSizeImage, &bih,
                DIB_RGB_COLORS) != cx)
    return FALSE;

  // Get image bits
  bih.bmiHeader.biBitCount = (WORD)colors;
  if (GetDIBits(hDC, ii.hbmColor, 0, cx, (char *)pBuffer, &bih,
                DIB_RGB_COLORS) != cx)
    return FALSE;
  ...
  ICONDIR id = {0, 1, 1};
  ICONDIRENTRY ide = {(BYTE)((cx < 256) ? cx : 0),
                      (BYTE)((cx < 256) ? cx : 0),
                      (BYTE)((colors < 8) ? (1 << colors) : 0),
                      0,
                      1,
                      (WORD)colors,
                      sizeof(BITMAPINFOHEADER) + sizeof(RGBQUAD) * palette +
                          nImageSize,
...
bool MarkFileAsDownload(const CString &sFilename) {
  ...
  if (Settings.Library.MarkFileAsDownload) {
    // TODO: pFile->m_bVerify and IDS_LIBRARY_VERIFY_FIX warning features could
    // be merged with this function, because they resemble the security warning.
    // Should raza unblock files from the application without forcing user to do
    // that manually?
    ...
    if (dwOrigAttr != INVALID_FILE_ATTRIBUTES &&
        (dwOrigAttr & FILE_ATTRIBUTE_READONLY))
      bChanged = SetFileAttributes(CString(_T("\\\\?\\")) + sFilename,
                                   dwOrigAttr & ~FILE_ATTRIBUTE_READONLY);

    HANDLE hStream = CreateFile(
        CString(_T("\\\\?\\")) + sFilename + _T(":Zone.Identifier"),
        GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
        NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

    if (hStream == INVALID_HANDLE_VALUE) {
      HANDLE hFile =
          CreateFile(CString(_T("\\\\?\\")) + sFilename, GENERIC_WRITE,
...
bool LoadGUID(const CString &sFilename, Hashes::Guid &oGUID) {
  ...
  if (Settings.Library.UseFolderGUID) {
    HANDLE hFile = CreateFile(
        CString(_T("\\\\?\\")) + sFilename + _T(":") CLIENT_NAME_T _T(".GUID"),
...
bool SaveGUID(const CString &sFilename, const Hashes::Guid &oGUID) {
  ...
  if (Settings.Library.UseFolderGUID) {
    ...
    if (hStream == INVALID_HANDLE_VALUE) {
      ...
      if (hFile != INVALID_HANDLE_VALUE) {
        hStream =
            CreateFile(CString(_T("\\\\?\\")) + sFilename +
                           _T(":") CLIENT_NAME_T _T(".GUID"),
                       GENERIC_WRITE,
                       FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
  ...
  if (SUCCEEDED(pIShellLink.CoCreateInstance(CLSID_ShellLink))) {
    ...
    if (pIPersistFile &&
        ...
        SUCCEEDED(pIShellLink->Resolve(AfxGetMainWnd()->GetSafeHwnd(),
                                       SLR_NO_UI | SLR_NOUPDATE | SLR_NOSEARCH |
                                           SLR_NOTRACK | SLR_NOLINKINFO))) {
      CString sPath;
      BOOL bResult = SUCCEEDED(
          pIShellLink->GetPath(sPath.GetBuffer(MAX_PATH), MAX_PATH, NULL, 0));
      sPath.ReleaseBuffer();
      if (bResult)
        return sPath;
    }
...
CString BrowseForFolder(LPCTSTR szTitle, LPCTSTR szInitialPath, HWND hWnd) {
  ...
  if (!szInitialPath || !*szInitialPath) {
    if (!*szDefaultPath)
      _tcsncpy_s(szDefaultPath, MAX_PATH, (LPCTSTR)theApp.GetDocumentsFolder(),
                 MAX_PATH - 1);
...
template <> __int8 GetRandomNum<__int8>(const __int8 &min, const __int8 &max) {
...
BOOL AreServiceHealthy(LPCTSTR szService) {
  ...
  if (schSCManager) {
    SC_HANDLE schServiceRead =
        OpenService(schSCManager, szService, GENERIC_READ);
    if (schServiceRead) {
      SERVICE_STATUS_PROCESS ssStatus = {};
      DWORD nBytesNeeded = 0;
      if (QueryServiceStatusEx(schServiceRead, SC_STATUS_PROCESS_INFO,
                               (LPBYTE)&ssStatus,
                               sizeof(SERVICE_STATUS_PROCESS), &nBytesNeeded)) {
        bResult = (ssStatus.dwCurrentState == SERVICE_RUNNING);
      }
      CloseServiceHandle(schServiceRead);

      if (!bResult) {
        SC_HANDLE schServiceStart =
            OpenService(schSCManager, szService, SERVICE_START);
        if (schServiceStart) {
          // Power users have only right to start service, thus try to start it
          // here
...
IShellLink *CreateShellLink(LPCWSTR szTargetExecutablePath,
                            LPCWSTR szCommandLineArgs, LPCWSTR szTitle,
                            LPCWSTR szIconPath, int nIconIndex,
...
CProgressDialog::CProgressDialog(LPCTSTR szTitle, DWORD dwFlags) {
  if (SUCCEEDED(CoCreateInstance(CLSID_ProgressDialog))) {
    ...
    p->StartProgressDialog(theApp.SafeMainWnd()
                               ? theApp.SafeMainWnd()->GetSafeHwnd()
                               : GetDesktopWindow(),
                           NULL, dwFlags, NULL);
  }
}
