...
CLibraryFile::CLibraryFile(CLibraryFolder *pFolder, LPCTSTR pszName)
    : m_pNextSHA1(NULL), m_pNextTiger(NULL), m_pNextED2K(NULL),
      m_pNextBTH(NULL), m_pNextMD5(NULL), m_nScanCookie(0ul),
      m_nUpdateCookie(0ul), m_nSelectCookie(0ul), m_nListCookie(0ul),
      m_pFolder(pFolder), m_nIndex(0ul), m_bShared(TRI_UNKNOWN),
      m_nVirtualBase(0ull), m_nVirtualSize(0ull), m_bVerify(TRI_UNKNOWN),
      m_pSchema(NULL), m_pMetadata(NULL), m_bMetadataAuto(FALSE),
      m_bMetadataModified(FALSE), m_nRating(0), m_nUploadsToday(0ul),
      m_nUploadsTotal(0ul), m_bCachedPreview(FALSE), m_bBogus(FALSE),
      m_nHitsToday(0ul), m_nHitsTotal(0ul), m_nSearchCookie(0ul),
      m_nSearchWords(0ul), m_pNextHit(NULL), m_nCollIndex(0ul), m_nIcon16(-1),
      m_bNewFile(FALSE), m_tCreateTime(0) {
...
CString CLibraryFile::GetSearchName() const {
  ...
  if (m_pFolder && m_pFolder->m_pParent) {
    for (const CLibraryFolder *pFolder = m_pFolder;;
         pFolder = pFolder->m_pParent) {
...
CXMLElement *CLibraryFile::CreateXML(CXMLElement *pRoot, BOOL bSharedOnly,
                                     XmlType nType) const {
  ...
  default:
    ...
    if (pFile) {
      if (m_oSHA1 && m_oTiger) {
        pFile->AddElement(_T("id"))->SetValue(_T("urn:bitprint:") +
                                              m_oSHA1.toString() + _T('.') +
                                              m_oTiger.toString());
      } else if (m_oSHA1) {
...
void CLibraryFile::SetShared(bool bShared, bool bOverride) {
  TRISTATE bNewShare =
      bOverride ? (bShared ? TRI_TRUE : TRI_FALSE) : TRI_UNKNOWN;
  ...
  if (bNewShare == TRI_UNKNOWN) {
    ...
    bNewShare = bShared ? (bFolderShared ? TRI_UNKNOWN : TRI_TRUE)
...
bool CLibraryFile::IsPrivateTorrent() const {
  return (m_pSchema && m_pMetadata &&
          ...
          m_pMetadata->GetAttributeValue(_T("privateflag"), _T("true"))
...
DWORD CLibraryFile::GetCreationTime() {
  ...
  HANDLE hFile =
      CreateFile(CString(_T("\\\\?\\")) + GetPath(), FILE_READ_ATTRIBUTES,
...
BOOL CLibraryFile::SetCreationTime(DWORD tTime) {
  ...
  HANDLE hFile =
      CreateFile(CString(_T("\\\\?\\")) + GetPath(), FILE_WRITE_ATTRIBUTES,
                 FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL,
                 OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
  if (hFile == INVALID_HANDLE_VALUE)
    return FALSE;
...
BOOL CLibraryFile::CheckFileAttributes(QWORD nSize, BOOL bSharedOnly,
                                       BOOL bAvailableOnly) const {
  return (nSize == SIZE_UNKNOWN || nSize == 0 || nSize == m_nSize) &&
         (!bAvailableOnly || m_pFolder) && (!bSharedOnly || IsShared());
}
...
BOOL CLibraryFile::IsNewFile() const { return m_bNewFile; }
  ...
  /*	if ( m_pMetadata != NULL )
      {
              ...
              CString strMetaOld		= strMetaFolder + '\\' + m_sName +
     _T(".xml"); CString strMetaNew		= strMetaFolder + '\\' + pszName
     + _T(".xml");
...
BOOL CLibraryFile::AddMetadata(const CLibraryFile *pFile) {
  ...
  if (pFile->m_nHitsToday || pFile->m_nHitsTotal || pFile->m_nUploadsToday ||
...
void CLibraryFile::UpdateMetadata(const CDownload *pDownload) {
  ...
  if (pDownload->HasMetadata()) {
    if (m_pMetadata) {
      ...
      // Recreate metadata
      TRACE("Using download XML:%s",
            (LPCSTR)CT2A(pBody->ToString(FALSE, TRUE)));
...
BOOL CLibraryFile::SetMetadata(CXMLElement *&pXML, BOOL bMerge,
                               BOOL bOverwrite) {
  ASSUME_LOCK(Library.m_pSection);
  ...
  if (pXML != NULL) {
    // Try fully specified XML first, for example <videos ...><video ...
    // /></videos>
    pSchema =
        SchemaCache.Get(pXML->GetAttributeValue(CXMLAttribute::schemaName));
    if (pSchema == NULL) {
...
CString CLibraryFile::GetAlternateSources(CList<CString> *pState, int nMaximum,
  ...
  for (POSITION pos = m_pSources.GetHeadPosition(); pos;) {
    ...
    if (!pSource->IsExpired(ftNow) &&
      ...
      if ((nProtocol == PROTOCOL_HTTP) &&
          (_tcsncmp(pSource->m_sURL, _T("http://"), 7) != 0))
        continue;

      if ((nProtocol == PROTOCOL_SSL) &&
          (_tcsncmp(pSource->m_sURL, _T("https://"), 8) != 0))
        continue;
...
void CLibraryFile::Serialize(CArchive &ar, int nVersion) {
  if (ar.IsStoring()) {
    ...
    if (nVersion < 22 && m_pSchema != NULL &&
        m_pSchema->CheckURI(CSchema::uriAudio)) {
      m_oSHA1.clear();
      m_oTiger.clear();
      m_oMD5.clear();
      m_oED2K.clear();
...
BOOL CLibraryFile::ThreadScan(DWORD nScanCookie, QWORD nSize,
                              FILETIME *pTime /*, LPCTSTR pszMetaData*/) {
  ASSUME_LOCK(Library.m_pSection);
  ASSERT(m_pFolder);
  ...
  if (m_nSize != nSize || CompareFileTime(&m_pTime, pTime) != 0) {
    ...
    CFolderScanDlg::Update(
        m_sName, (m_nSize == SIZE_UNKNOWN) ? 0 : (DWORD)(m_nSize / 1024));
...
BOOL CLibraryFile::OnVerifyDownload(const CLibraryRecent *pRecent) {
  ...
  if (Settings.Downloads.VerifyFiles && m_bVerify == TRI_UNKNOWN &&
      m_nVirtualSize == 0) {
    if ((bool)m_oSHA1 && (bool)pRecent->m_oSHA1 &&
        pRecent->m_oSHA1.isTrusted()) {
      m_bVerify = (m_oSHA1 == pRecent->m_oSHA1) ? TRI_TRUE : TRI_FALSE;
    }
    if (m_bVerify != TRI_FALSE && (bool)m_oTiger && (bool)pRecent->m_oTiger &&
        pRecent->m_oTiger.isTrusted()) {
      m_bVerify = (m_oTiger == pRecent->m_oTiger) ? TRI_TRUE : TRI_FALSE;
    }
    if (m_bVerify != TRI_FALSE && (bool)m_oED2K && (bool)pRecent->m_oED2K &&
        pRecent->m_oED2K.isTrusted()) {
      m_bVerify = (m_oED2K == pRecent->m_oED2K) ? TRI_TRUE : TRI_FALSE;
    }
    if (m_bVerify != TRI_FALSE && (bool)m_oMD5 && (bool)pRecent->m_oMD5 &&
        pRecent->m_oMD5.isTrusted()) {
      m_bVerify = (m_oMD5 == pRecent->m_oMD5) ? TRI_TRUE : TRI_FALSE;
    }
    if (m_bVerify != TRI_FALSE && (bool)m_oBTH && (bool)pRecent->m_oBTH &&
        pRecent->m_oBTH.isTrusted()) {
...
BOOL CLibraryFile::PrepareDoc(LPCTSTR pszTemplate,
                              CArray<CString> &oDocs) const {
  ASSUME_LOCK(Library.m_pSection);
    ...
    for (POSITION pos = pMetadata->GetAttributeIterator(); pos;) {
      ...
      if (str == _T("seconds") || str == _T("minutes")) {
        double nTotalSecs = (str == _T("minutes"))
                                ? _tstof((LPCTSTR)strReplace) * 60
                                : _tstof((LPCTSTR)strReplace);
  ...
  if (m_oSHA1) {
    ...
    ReplaceNoCase(strDoc, _T("$meta:gnutella$"),
                  _T("gnutella://urn:sha1:") + m_oSHA1.toString() + _T('/') +
                      strNameURI + _T('/'));
  }
  ...
  if (m_oSHA1 && m_oTiger) {
    strMagnet = _T("xt=urn:bitprint:") + m_oSHA1.toString() + _T('.') +
                m_oTiger.toString();

    ReplaceNoCase(strDoc, _T("$meta:bitprint$"),
                  m_oSHA1.toString() + _T('.') + m_oTiger.toString());
  }

  if (m_oED2K) {
    if (strMagnet.GetLength())
      strMagnet += _T("&amp;");
    ...
    if (strSize.GetLength())
      ReplaceNoCase(strDoc, _T("$meta:ed2k$"),
                    _T("ed2k://|file|") + strNameURI + _T('|') + strSize +
                        _T('|') + m_oED2K.toString() + _T("|/"));
  }
...
STDMETHODIMP CLibraryFile::XLibraryFile::get_Hash(URN_TYPE nType,
                                                  ENCODING nEncoding,
                                                  BSTR FAR *psURN) {
...
STDMETHODIMP CLibraryFile::XLibraryFile::get_Application(
    IApplication FAR *FAR *ppApplication) {
  METHOD_PROLOGUE(CLibraryFile, LibraryFile)
  return CApplication::GetApp(ppApplication);
}

STDMETHODIMP
CLibraryFile::XLibraryFile::get_Library(ILibrary FAR *FAR *ppLibrary) {
  METHOD_PROLOGUE(CLibraryFile, LibraryFile)
...
STDMETHODIMP
CLibraryFile::XLibraryFile::get_Folder(ILibraryFolder FAR *FAR *ppFolder) {
  METHOD_PROLOGUE(CLibraryFile, LibraryFile)
  if (!pThis->m_pFolder)
    *ppFolder = NULL;
  else
    *ppFolder = (ILibraryFolder *)pThis->m_pFolder->GetInterface(
        IID_ILibraryFolder, TRUE);
...
STDMETHODIMP
CLibraryFile::XLibraryFile::get_EffectiveShared(VARIANT_BOOL FAR *pbValue) {
  METHOD_PROLOGUE(CLibraryFile, LibraryFile)
...
STDMETHODIMP
CLibraryFile::XLibraryFile::get_MetadataAuto(VARIANT_BOOL FAR *pbValue) {
  METHOD_PROLOGUE(CLibraryFile, LibraryFile)
...
STDMETHODIMP
CLibraryFile::XLibraryFile::get_Metadata(ISXMLElement FAR *FAR *ppXML) {
  METHOD_PROLOGUE(CLibraryFile, LibraryFile)
  *ppXML = NULL;
...
STDMETHODIMP CLibraryFile::XLibraryFile::MergeMetadata(ISXMLElement *pXML,
  ...
  if (CXMLElement *pReal = CXMLCOM::Unwrap(pXML)) {
    *pbValue =
        pThis->MergeMetadata(pReal, bOverwrite) ? VARIANT_TRUE : VARIANT_FALSE;
  } else
