...
CMainWnd::CMainWnd()
    : m_bTrayNotify(FALSE), m_bTrayHide(FALSE), m_bTrayIcon(FALSE),
      m_bTrayUpdate(TRUE), m_bTimer(FALSE), m_pSkin(NULL), m_nAlpha(255) {
  ZeroMemory(&m_pTray, sizeof(NOTIFYICONDATA));
  m_pTray.cbSize = theApp.m_bIsVistaOrNewer ? sizeof(NOTIFYICONDATA)
...
BOOL CMainWnd::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName,
  ...
  return CMDIFrameWnd::Create(
      lpszClassName, lpszWindowName, dwStyle, rect, pParentWnd, NULL,
...
CMDIChildWnd *CMainWnd::MDIGetActive(BOOL *pbMaximized) const {
  if (m_hWnd == NULL || m_pWindows.IsEmpty())
    return NULL;
...
BOOL CMainWnd::OnCreateClient(LPCREATESTRUCT lpcs,
                              CCreateContext * /* pContext */) {
  // Bypass menu creation
...
BOOL CMainWnd::PreCreateWindow(CREATESTRUCT &cs) {
  ...
  if (cs.hInstance) {
    ...
    wndcls.hIcon =
        CoolInterface.ExtractIcon(IDR_MAINFRAME, FALSE, LVSIL_NORMAL);
...
int CMainWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  if (!m_wndNavBar.Create(this, WS_CHILD | WS_VISIBLE | CBRS_TOP, IDW_NAV_BAR))
    return -1;
  ...
  if (!m_wndToolBar.Create(this, WS_CHILD | CBRS_TOP, IDW_TOOL_BAR))
    return -1;
  ...
  if (!m_wndTabBar.Create(this, WS_CHILD | WS_VISIBLE | CBRS_BOTTOM,
                          IDW_TAB_BAR))
    return -1;
  ...
  if (!m_wndMonitorBar.Create(this, WS_CHILD | WS_VISIBLE | CBRS_BOTTOM,
                              IDW_MONITOR_BAR))
    return -1;
  ...
  if ( ( theApp.m_dwWindowsVersion >= 5 ) && (GetSystemMetrics( SM_CMONITORS ) >
  1) ) {	// Multi Monitor iXSize = GetSystemMetrics( SM_CXVIRTUALSCREEN
  ...
  */
  ...
  SetWindowPos(NULL, iXSize * 1 / 10, iYSize * 1 / 10, iXSize * 8 / 10,
               iYSize * 8 / 10, 0);
  ...
  if (!m_wndNavBar.IsVisible() && !m_wndToolBar.IsVisible()) {
    ShowControlBar(&m_wndNavBar, Settings.General.GUIMode != GUI_WINDOWED,
                   TRUE);
    ShowControlBar(&m_wndToolBar, Settings.General.GUIMode == GUI_WINDOWED,
                   TRUE);
  }
...
void CMainWnd::SetGUIMode(int nMode, BOOL bSaveState) {
  ...
  if (bSaveState) {
    if (nMode < 0)
      nMode = Settings.General.GUIMode;
  ...
  if (bSaveState) {
    ...
    DockControlBar(&m_wndTabBar, nMode == GUI_WINDOWED ? AFX_IDW_DOCKBAR_BOTTOM
                                                       : AFX_IDW_DOCKBAR_TOP);
...
BOOL CMainWnd::OnCmdMsg(UINT nID, int nCode, void *pExtra,
...
BOOL CMainWnd::OnCommand(WPARAM wParam, LPARAM lParam) {
  if (Plugins.OnCommand(m_pWindows.GetActive(), LOWORD(wParam)))
    return TRUE;
...
void CMainWnd::OnUpdateShell(CCmdUI *pCmdUI) { pCmdUI->Enable(TRUE); }
...
void CMainWnd::OnWindowPosChanging(WINDOWPOS *lpwndpos) {
  ...
  HMONITOR hMonitor =
      MonitorFromWindow(GetSafeHwnd(), MONITOR_DEFAULTTOPRIMARY);
...
void CMainWnd::OnTimer(UINT_PTR nIDEvent) {
  ...
  // Switch tray icon
  if (m_bTrayHide || Settings.General.TrayMinimise ||
...
void CMainWnd::OnActivate(UINT nState, CWnd *pWndOther, BOOL bMinimized) {
  ...
  if (nState != WA_INACTIVE) {
    if (CChildWnd *pChildWnd = m_pWindows.GetActive()) {
      pChildWnd->SendMessage(WM_MDIACTIVATE, NULL,
                             (LPARAM)pChildWnd->GetSafeHwnd());
    }
  }
}
...
void CMainWnd::AddTray() {
  if (!m_bTrayIcon) {
    // Delete existing tray icon (if any), windows can't create a new icon with
    // same uID
...
void CMainWnd::OnTrayOpen() { OpenFromTray(); }
...
LRESULT CMainWnd::OnChangeAlpha(WPARAM wParam, LPARAM /*lParam*/) {
  ...
  if (m_nAlpha == 0) {
    ...
    pWndPopup->RedrawWindow(
        NULL, NULL, RDW_ERASE | RDW_INVALIDATE | RDW_FRAME | RDW_ALLCHILDREN);
  } else {
    pWndPopup->ModifyStyleEx(0, WS_EX_LAYERED);
    ::SetLayeredWindowAttributes(pWndPopup->GetSafeHwnd(), 0, (BYTE)m_nAlpha,
                                 LWA_ALPHA);
  }
...
void CMainWnd::OnSysCommand(UINT nID, LPARAM lParam) {
  ...
  if (m_bTrayHide) {
    ...
    case SC_CLOSE:
      ...
      break;
    case SC_MINIMIZE:
      if ((Settings.General.TrayMinimise && !bShift) ||
          (!Settings.General.TrayMinimise && bShift)) {
        CloseToTray();
        return;
      }
      break;
...
LRESULT CMainWnd::OnHandleImport(WPARAM wParam, LPARAM /*lParam*/) {
  ...
  if (CHostCacheWnd *pCacheWnd =
          (CHostCacheWnd *)m_pWindows.Open(RUNTIME_CLASS(CHostCacheWnd))) {
...
LRESULT CMainWnd::OnVersionCheck(WPARAM wParam, LPARAM /*lParam*/) {
  if (wParam == VC_MESSAGE_AND_CONFIRM &&
      VersionChecker.m_sMessage.GetLength()) {
...
// Used from Remote pages when the search is performed. Receives WM_OPENSEARCH
// message
...
LRESULT CMainWnd::OnMediaKey(WPARAM /*wParam*/, LPARAM lParam) {
  if (CMediaWnd *pWnd =
          (CMediaWnd *)m_pWindows.Find(RUNTIME_CLASS(CMediaWnd))) {
...
void CMainWnd::UpdateMessages() {
  ...
  if (IsForegroundWindow()) {
    ...
    if (Network.IsWellConnected()) {
      ...
      if (Settings.General.GUIMode == GUI_BASIC) {
        ...
        strStatusbar.Format(
            LoadString(IDS_STATUS_BAR_CONNECTED_SIMPLE),
            (LPCTSTR)Settings.SmartVolume(nLocalVolume, KiloBytes));
      } else {
        // Display node type and number of neighbours
        strStatusbar.Format(
            LoadString(Neighbours.IsG2Hub()
                           ? (Neighbours.IsG1Ultrapeer()
                                  ? IDS_STATUS_BAR_CONNECTED_HUB_UP
                                  : IDS_STATUS_BAR_CONNECTED_HUB)
                           : (Neighbours.IsG1Ultrapeer()
                                  ? IDS_STATUS_BAR_CONNECTED_UP
                                  : IDS_STATUS_BAR_CONNECTED)),
            Neighbours.GetStableCount(),
            (LPCTSTR)Settings.SmartVolume(nLocalVolume, KiloBytes));
      }
    } else if (Network.IsConnected()) {
      // If G1, G2, eDonkey are disabled and only BitTorrent is enabled say
      // connected
      if (!Settings.Gnutella1.EnableToday && !Settings.Gnutella2.EnableToday &&
          !Settings.eDonkey.EnableToday && !Settings.DC.EnableToday) {
        strStatusbar.Format(
            LoadString(IDS_STATUS_BAR_CONNECTED_SIMPLE),
            (LPCTSTR)Settings.SmartVolume(nLocalVolume, KiloBytes));
      } else
    ...
    // StatusBar pane 1
    strStatusbar.Format(
        LoadString(IDS_STATUS_BAR_BANDWIDTH),
        (LPCTSTR)Settings.SmartSpeed(
            CGraphItem::GetValue(GRC_TOTAL_BANDWIDTH_IN), bits),
        (LPCTSTR)Settings.SmartSpeed(
            CGraphItem::GetValue(GRC_TOTAL_BANDWIDTH_OUT), bits),
    ...
    // StatusBar pane 2
    strStatusbar = HostToString(&Network.m_pHost) + _T(", ") +
                   HostToString(&Network.m_pHostIPv6);
  ...
  (LPCTSTR)Settings.SmartAgent(), (LPCTSTR)LoadString( IDS_DLM_VOLUME_DOWNLOADED
  ), float( ( 10000 * nComplete ) / nTotal ) / 100.f, (LPCTSTR)LoadString(
  IDS_DLM_TOTAL_SPEED ), (LPCTSTR)Settings.SmartSpeed( CGraphItem::GetValue(
  GRC_TOTAL_BANDWIDTH_IN ), bits ) );
      }
      else
      {
...
// This function runs some basic checks that everything is okay- disks,
// directories, local network is up, etc.
void CMainWnd::LocalSystemChecks() {
  static DWORD nConnectionFailCount =
      0; // Counter for times a connection problem has been detected

  // Check disk space
  if (Settings.Live.DiskSpaceStop == FALSE) {
    if (!Downloads.IsSpaceAvailable((QWORD)Settings.General.DiskSpaceStop *
                                    1024 * 1024)) {
  ...
  if (Settings.Live.DiskSpaceWarning == FALSE) {
    if (!Downloads.IsSpaceAvailable((QWORD)Settings.General.DiskSpaceWarning *
                                    1024 * 1024)) {
  ...
  if (Settings.Live.DiskWriteWarning == FALSE) {
    ...
    nIncompleteAttributes =
        GetFileAttributes(Settings.Downloads.IncompletePath);

    if ((nCompleteAttributes == INVALID_FILE_ATTRIBUTES) ||
        (nIncompleteAttributes == INVALID_FILE_ATTRIBUTES) ||
        !(nCompleteAttributes & FILE_ATTRIBUTE_DIRECTORY) ||
        !(nIncompleteAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
      ...
      // Note: These checks fail on some machines. WinXP goofyness?
                          ...
                          if ( ( _taccess( Settings.Downloads.IncompletePath, 06
      ) != 0 ) || ( _taccess( Settings.Downloads.CompletePath, 06 ) != 0 ) )
                          {
                                  Settings.Live.DiskWriteWarning = TRUE;
                                  PostMessage( WM_COMMAND, ID_HELP_DISKWRITEFAIL
      );
  ...
  if (Settings.Connection.DetectConnectionLoss) {
    if (Network.IsConnected()) {
      if (Network.IsAvailable() || Network.IsWellConnected()) {
        ...
        if (nConnectionFailCount > 1) {
          if (Settings.Connection.DetectConnectionReset)
            theApp.Message(
                MSG_ERROR,
                _T("Internet disconnection detected- shutting down network"));
          else
      ...
      // We are not currently connected. Check if we disconnected because of
      // failure and want to re-connect.
      if ((nConnectionFailCount > 0) &&
          (Settings.Connection.DetectConnectionReset)) {
        // See if we can reconnect
        if (Network.IsAvailable()) {
          nConnectionFailCount = 0;
          PostMessage(WM_COMMAND, ID_NETWORK_CONNECT);
          theApp.Message(MSG_ERROR,
                         _T("Internet reconnect detected- restarting network"));
        }
      }
    }
  }
  ...
  if (!Settings.Live.DonkeyServerWarning && Settings.eDonkey.EnableToday) {
    ...
    if (!Settings.eDonkey.AutoDiscovery &&
        !HostCache.EnoughServers(PROTOCOL_ED2K))
...
void CMainWnd::OnUpdateNetworkConnect(CCmdUI *pCmdUI) {
  ...
  UINT nTextID = 0;
  bool bNetworksEnabled =
      (Settings.Gnutella1.EnableToday || Settings.Gnutella2.EnableToday ||
       Settings.eDonkey.EnableToday);

  if (Network.IsConnected() &&
      (Network.IsWellConnected() ||
       !bNetworksEnabled) // If Network.IsWellConnected() or G1, G2, eDonkey are
                          // disabled and only BitTorrent is enabled say
                          // connected
  ) {
...
void CMainWnd::OnNetworkConnect() { Network.Connect(TRUE); }

void CMainWnd::OnUpdateNetworkDisconnect(CCmdUI *pCmdUI) {
  pCmdUI->Enable(Network.IsConnected());
}

void CMainWnd::OnNetworkDisconnect() { Network.Disconnect(); }
...
void CMainWnd::OnNetworkConnectTo() {
  ...
  Network.ConnectTo(dlg.m_sHost, dlg.m_nPort, dlg.m_nProtocol,
                    dlg.m_bNoUltraPeer);
}
...
void CMainWnd::OnNetworkExit() { PostMessage(WM_CLOSE); }
...
void CMainWnd::OnUpdateTabConnect(CCmdUI * /*pCmdUI*/) {
  ...
  if (Network.IsConnected() &&
    ...
    if (pItem)
      pItem->SetTextColour(RGB(255, 0, 0));
    ...
    if (pItem)
      pItem->SetCheck(TRUE);
    if (pItem)
      pItem->SetTextColour(CoolInterface.m_crCmdText == 0
                               ? CoolInterface.m_crTextStatus
                               : CoolInterface.m_crCmdText);
    m_wndTabBar.SetMessage((UINT)0);
    nTextID = IDS_NETWORK_CONNECTING;
    nTipID = ID_NETWORK_DISCONNECT;
  } else {
...
void CMainWnd::OnTabConnect() {
  if (Network.IsConnected()) {
    ...
    if ((GetAsyncKeyState(VK_SHIFT) & 0x8000) || !Network.IsWellConnected() ||
...
void CMainWnd::OnUpdateTabMedia(CCmdUI *pCmdUI) {
  ...
  if (CCoolBarItem *pItem = m_wndToolBar.GetID(ID_TAB_MEDIA)) {
    if ((pChild = (CMediaWnd *)m_pWindows.Find(RUNTIME_CLASS(CMediaWnd))) !=
        NULL) {
      pItem->SetTextColour(pChild->IsPlaying() ? CoolInterface.m_crTextStatus
                                               : CoolInterface.m_crCmdText);
    } else {
...
void CMainWnd::OnUpdateTabTransfers(CCmdUI *pCmdUI) {
  ...
  pCmdUI->SetCheck(pChild != NULL &&
                   pChild->IsKindOf(RUNTIME_CLASS(CDownloadsWnd)));
}
...
void CMainWnd::OnUpdateTabNetwork(CCmdUI *pCmdUI) {
  ...
  pCmdUI->SetCheck(pChild != NULL &&
                   pChild->IsKindOf(RUNTIME_CLASS(CNeighboursWnd)));
}
...
void CMainWnd::OnToolsLanguage() {
  ...
  if (bRestart) {
    HINSTANCE hResult = ShellExecute(
        AfxGetMainWnd()->GetSafeHwnd(), NULL, theApp.m_strBinaryPath,
        _T("-nowarn -wait"), Settings.General.Path, SW_SHOWNORMAL);
...
void CMainWnd::OnToolsSeedTorrent() {
  CFileDialog dlgFile(TRUE, _T("torrent"),
                      (Settings.Downloads.TorrentPath + _T("\\.")),
                      OFN_HIDEREADONLY,
                      _T("Torrent Files|*.torrent|") +
                          SchemaCache.GetFilter(CSchema::uriAllFiles) + _T("|"),
                      this);
...
void CMainWnd::OnDiskSpace() { CHelpDlg::Show(_T("GeneralHelp.DiskSpace")); }
...
void CMainWnd::OnToolsReskin() { OnSkinChanged(0, 0); }
...
void CMainWnd::OnUpdateWindowCascade(CCmdUI *pCmdUI) { pCmdUI->Enable(TRUE); }
...
void CMainWnd::OnHelpVersionCheck() { VersionChecker.ForceCheck(); }

void CMainWnd::OnHelpHomepage() {
  const CString strWebSite(WEB_SITE_T);

  ShellExecute(GetSafeHwnd(), _T("open"),
               strWebSite + _T("?Version=") + theApp.m_sVersion, NULL, NULL,
               SW_SHOWNORMAL);
}
...
void CMainWnd::OnHelpWeb1() {
  ...
  ShellExecute(GetSafeHwnd(), _T("open"),
               strWebSite + _T("help/external/?link1"), NULL, NULL,
               SW_SHOWNORMAL);
}
...
void CMainWnd::OnHelpWeb2() {
  ...
  ShellExecute(GetSafeHwnd(), _T("open"),
               strWebSite + _T("help/external/?link2"), NULL, NULL,
               SW_SHOWNORMAL);
}
...
void CMainWnd::OnHelpWeb3() {
  ...
  ShellExecute(GetSafeHwnd(), _T("open"),
               strWebSite + _T("help/external/?link3"), NULL, NULL,
               SW_SHOWNORMAL);
}
...
void CMainWnd::OnHelpWeb4() {
  ...
  ShellExecute(GetSafeHwnd(), _T("open"),
               strWebSite + _T("help/external/?link4"), NULL, NULL,
               SW_SHOWNORMAL);
}
...
void CMainWnd::OnHelpWeb5() {
  ...
  ShellExecute(GetSafeHwnd(), _T("open"),
               strWebSite + _T("help/external/?link5"), NULL, NULL,
               SW_SHOWNORMAL);
}
...
void CMainWnd::OnHelpWeb6() {
  ...
  ShellExecute(GetSafeHwnd(), _T("open"),
               strWebSite + _T("help/external/?link6"), NULL, NULL,
               SW_SHOWNORMAL);
}
...
void CMainWnd::OnHelpFaq() {
  ...
  ShellExecute(GetSafeHwnd(), _T("open"), strWebSite + _T("help/?faq"), NULL,
               NULL, SW_SHOWNORMAL);
}

void CMainWnd::OnHelpConnectiontest() {
  CString strWebSite;
  strWebSite.Format(_T("%s?port=%u&lang=%s&Version=%s"), CONNECTION_SITE_T,
                    Settings.Connection.InPort,
                    (LPCTSTR)Settings.General.Language,
                    (LPCTSTR)theApp.m_sVersion);
  ShellExecute(GetSafeHwnd(), _T("open"), strWebSite, NULL, NULL,
               SW_SHOWNORMAL);
}
...
void CMainWnd::OnHelpGuide() {
  ...
  ShellExecute(GetSafeHwnd(), _T("open"), strWebSite + _T("help/?guide"), NULL,
               NULL, SW_SHOWNORMAL);
}
...
void CMainWnd::OnHelpForums() {
  ...
  ShellExecute(GetSafeHwnd(), _T("open"), strWebSite + _T("help/?forum"), NULL,
               NULL, SW_SHOWNORMAL);
}
...
void CMainWnd::OnHelpRouter() {
  ...
  ShellExecute(GetSafeHwnd(), _T("open"), strWebSite + _T("help/?router"), NULL,
               NULL, SW_SHOWNORMAL);
}
...
void CMainWnd::OnHelpCodec() {
  ...
  ShellExecute(GetSafeHwnd(), _T("open"), strWebSite + _T("help/?codec"), NULL,
               NULL, SW_SHOWNORMAL);
}
...
void CMainWnd::OnHelpTorrent() {
  ...
  ShellExecute(GetSafeHwnd(), _T("open"),
               strWebSite + _T("help/?torrentencoding"), NULL, NULL,
               SW_SHOWNORMAL);
}
...
BOOL CMainWnd::OnNcActivate(BOOL bActive) {
  if (m_pSkin) {
    m_pSkin->OnNcActivate(this, IsWindowEnabled() &&
                                    (bActive || (m_nFlags & WF_STAYACTIVE)));
    return TRUE;
  } else
...
void CMainWnd::OnNcLButtonDown(UINT nHitTest, CPoint point) {
  ...
  // Windows Vista skinning workaround (system caption buttons over skin
  // drawing)
...
BOOL CMainWnd::OnDrop(IDataObject *pDataObj, DWORD /* grfKeyState */,
                      POINT /* ptScreen */, DWORD *pdwEffect, BOOL bDrop) {
  if (!pDataObj || !pdwEffect)
    return FALSE;

  FORMATETC fmtcFiles = {CF_HDROP, NULL, DVASPECT_CONTENT, -1, TYMED_HGLOBAL};
  FORMATETC fmtcURL = {(CLIPFORMAT)RegisterClipboardFormat(CFSTR_SHELLURL),
...
int CMainWnd::SnarlCommand(LPCSTR szCommand) {
  ...
  if (HWND hWndSnarl = ::FindWindow(_T("w>Snarl"), _T("Snarl"))) {
    ...
    COPYDATASTRUCT cds = {uSnarlMagic, (DWORD)strlen(szCommand) + 1,
                          (LPSTR)szCommand};
    SendMessageTimeout(hWndSnarl, WM_COPYDATA, (WPARAM)GetCurrentProcessId(),
                       (LPARAM)&cds, SMTO_ABORTIFHUNG, 1000, &lResult);
  }
  return (int)lResult;
}

bool CMainWnd::SnarlRegister() {
  return Settings.Interface.Snarl &&
         (SnarlCommand("register?app-sig=app/" CLIENT_NAME
                       "&title=" CLIENT_NAME) >= 0);
}
...
bool CMainWnd::SnarlNotify(const CString &sText, const CString &sTitle,
  ...
  sCommand.Format("notify?app-sig=app/%s&timeout=%u&title=%s&text=%s",
                  CLIENT_NAME, uTimeout, (LPCSTR)UTF8Encode(sSafeTitle),
                  (LPCSTR)UTF8Encode(sSafeText));
...
void CMainWnd::ShowTrayPopup(const CString &sText, const CString &sTitle,
                             DWORD dwIcon, UINT uTimeout) {
  if (sText.IsEmpty())
    return;
  ...
  if ((dwIcon & NIIF_ICON_MASK) == NIIF_USER)
    m_pTray.hBalloonIcon = CoolInterface.ExtractIcon(
        IDI_USER, FALSE, theApp.m_bIsVistaOrNewer ? LVSIL_NORMAL : LVSIL_SMALL);
  else
    m_pTray.hBalloonIcon = NULL;

  m_pTray.dwInfoFlags =
      dwIcon | (theApp.m_bIsVistaOrNewer ? NIIF_LARGE_ICON : 0);
...
BOOL CMainWnd::OnCopyData(CWnd * /*pWnd*/, COPYDATASTRUCT *pCopyDataStruct) {
  if (pCopyDataStruct) {
    ...
    case COPYDATA_SCHEDULER:
      CScheduler::Execute(
          CString((LPCTSTR)pCopyDataStruct->lpData,
...
void CMainWnd::OnUpdatePathCopy(CCmdUI *pCmdUI) { pCmdUI->Enable(); }

void CMainWnd::OnUpdatePathExplore(CCmdUI *pCmdUI) { pCmdUI->Enable(); }
