...
DWORD GetCurrentTimeInSeconds() {
  ...
  // Convert 100-nanosecond intervals since January 1, 1601 (UTC) to seconds
  // since January 1, 1970 (UTC) Subtract the number of 100-nanosecond intervals
  // from 1601 to 1970
...
CDownload::CDownload()
    : m_nSerID(Downloads.GetFreeSID()),
      m_bExpanded(Settings.Downloads.AutoExpand), m_bSelected(FALSE),
      m_tCompleted(0), m_tAdded(GetCurrentTimeInSeconds()), m_nRunCookie(0),
      m_nGroupCookie(0)

      ,
      m_bTempPaused(FALSE), m_bPaused(FALSE), m_bBoosted(FALSE),
      m_bShared(Settings.Uploads.SharePartials), m_bComplete(false),
      m_tSaved(0), m_tBegan(0), m_bDownloading(false), m_nCompletedAtBegan(0),
      m_nStartFrom(0)
#pragma warning(                                                               \
        suppress : 4355) // 'this' : used in base member initializer list
      ,
      m_pTask(this), m_bStableName(false) {
}
...
void CDownload::SetStableName(bool bStable) { m_bStableName = bStable; }

float CDownload::GetProgress() const {
  return IsMoving() ? m_pTask.GetProgress()
                    : CDownloadWithExtras::GetProgress();
}
...
dtask CDownload::GetTaskType() const { return m_pTask.GetTaskType(); }

void CDownload::AbortTask() { m_pTask.Abort(); }

void CDownload::Allocate() { m_pTask.Allocate(); }

void CDownload::Copy() { m_pTask.Copy(); }
...
void CDownload::MergeFile(CList<CString> *pFiles, BOOL bValidation,
                          const Fragments::List *pGaps) {
...
void CDownload::Boost() {
  if (!IsFileOpen() || m_bBoosted)
    return;

  theApp.Message(MSG_NOTICE, IDS_DOWNLOAD_BOOST, (LPCTSTR)GetDisplayName());

  for (CDownloadTransfer *pTransfer = GetFirstTransfer(); pTransfer;
...
QWORD CDownload::GetNonRandomEnd(bool bForce) {
  ...
  if (Settings.Downloads.MediaBuffer && m_tStartFromSet > 0 &&
    ...
    return m_nStartFrom + (((GetTickCount() - m_tStartFromSet +
                             Settings.Downloads.MediaBuffer) /
                            1000) *
                           nByterate);
...
DWORD CDownload::GetStartTimer() const { return m_tBegan; }
...
bool CDownload::IsDownloading() const { return m_bDownloading; }

bool CDownload::IsCompleted() const { return m_bComplete; }

bool CDownload::IsBoosted() const { return (m_bBoosted != 0); }

// Is the download currently trying to download?
bool CDownload::IsTrying() const { return (m_tBegan != 0); }

bool CDownload::IsShared() const {
  return m_bShared || (Settings.Gnutella1.EnableToday && m_oSHA1) ||
         (Settings.Gnutella2.EnableToday && HasHash()) ||
         (Settings.eDonkey.EnableToday && m_oED2K) ||
         (Settings.DC.EnableToday && m_oTiger) ||
         (Settings.BitTorrent.EnableToday && IsTorrent() &&
          (IsSeeding() || IsStarted()));
}

DWORD CDownload::GetDateAddedInSeconds() const { return m_tAdded; }
...
CString CDownload::GetDateAdded() const {
  ...
  CString result;
  result.Format(_T("%02d/%02d/%04d %02d:%02d:%02d"), st.wDay, st.wMonth,
...
CString CDownload::GetDownloadStatus() const {
  ...
  if (IsCompleted()) {
    ...
    if (nTime == 0xFFFFFFFF)
      ...
      strText.Format(_T("%u:%.2u:%.2u:%.2u"), nTime / 86400,
                     (nTime / 3600) % 24, (nTime / 60) % 60, nTime % 60);
    else
      strText.Format(_T("%u:%.2u:%.2u"), nTime / 3600, (nTime / 60) % 60,
                     nTime % 60);
  } else if (!IsTrying())
...
CString CDownload::GetDownloadSources() const {
  ...
  if (IsCompleted()) {
    ...
    strText.Format(_T("(%i/%i %s)"), nSources, nTotalSources,
                   (LPCTSTR)strSource);
  }
  return strText;
}
...
void CDownload::OnRun() {
  ...
  if (!IsPaused()) {
    if (GetFileError() != ERROR_SUCCESS) {
      ...
      //'Dead download' check- if download appears dead, give up and allow
      //another to start.
      if (!IsCompleted() &&
          (tNow - GetStartTimer()) >
              (3 * 60 * 60 * 1000)) { // If it's not complete, and we've been
                                      // trying for at least 3 hours

        DWORD tHoursToTry = min((GetEffectiveSourceCount() + 49u) / 50u, 9lu) +
                            Settings.Downloads.StarveGiveUp;

        if ((tNow - m_tReceived) >
            (tHoursToTry * 60 * 60 *
             1000)) { // And have had no new data for 5-14 hours

          if (IsTorrent()) // If it's a torrent
          {
            if (Downloads.GetTryingCount(
                    true, Settings.BitTorrent.DownloadTorrents) >=
                Settings.BitTorrent
                    .DownloadTorrents) { // If there are other torrents that
                                         // could start
          ...
          {
            if (Downloads.GetTryingCount() >=
                (Settings.Downloads.MaxFiles +
                 Settings.Downloads
                     .MaxFileSearches)) { // If there are other downloads that
                                          // could try
              StopTrying(); // Give up for now, try again later
              return;
            }
      ...
      if (Network.IsConnected()) {
        ...
        if (Downloads.GetTryingCount() < (Settings.Downloads.MaxFiles +
                                          Settings.Downloads.MaxFileSearches)) {
          if (!IsTorrent() ||
              // Torrents only try when 'ready to go'. (Reduce tracker load)
              (Downloads.GetCount() < Settings.BitTorrent.DownloadTorrents ||
               Downloads.GetTryingCount(true,
                                        Settings.BitTorrent.DownloadTorrents) <
                   Settings.BitTorrent.DownloadTorrents)) {
            Resume();
          }
        }
  ...
  if (tNow - m_tSaved >= (GetCount() > 20 ? 5 * Settings.Downloads.SaveInterval
                                          : Settings.Downloads.SaveInterval)) {
    if (IsModified()) {
      FlushFile();
      if (Save())
        m_tSaved = tNow;
...
BOOL CDownload::OpenDownload() {
  ...
  if (m_nSize != SIZE_UNKNOWN &&
    ...
    CString sFileError;
    sFileError.Format(LoadString(IDS_DOWNLOAD_DISK_SPACE), (LPCTSTR)m_sName,
                      (LPCTSTR)Settings.SmartVolume(m_nSize));
...
BOOL CDownload::PrepareFile() { return OpenDownload() && IsRemaining(); }
...
BOOL CDownload::SeedTorrent() {
  ...
  if (!pFragmentedFile->Open(m_pTorrent, FALSE)) {
    SetFileError(pFragmentedFile->GetFileError(),
                 pFragmentedFile->GetFileErrorString());
    return FALSE;
  }
  ...
  if (IsSingleFileTorrent()) {
    ...
    if (CLibraryFile *pLibraryFile =
            LibraryMaps.LookupFileByPath(pBTFile->FindFile())) {
...
BOOL CDownload::Load(LPCTSTR pszName) {
  ...
  if (!bSuccess &&
      pFile.Open(_T("\\\\?\\") + m_sPath + _T(".sav"), CFile::modeRead)) {
    TRY {
      CArchive ar(&pFile, CArchive::load, 32768); // 32 KB buffer
      Serialize(ar, 0);
      bSuccess = TRUE;
    ...
    AND_CATCH_ALL(pException) {
      theApp.Message(MSG_ERROR, IDS_DOWNLOAD_FILE_OPEN_ERROR,
                     (LPCTSTR)(m_sPath + _T(".sav")));
    }
    END_CATCH_ALL
...
BOOL CDownload::Save(BOOL bFlush) {
  ...
  if (m_sPath.IsEmpty()) {
    ...
    m_sPath = Settings.Downloads.IncompletePath + _T("\\") + GetFilename() +
              _T(".sd");
  }
...
BOOL CDownload::OnVerify(const CLibraryFile *pFile, TRISTATE bVerified) {
  ...
  if (bVerified != TRI_FALSE && (!IsTorrent() || IsSingleFileTorrent())) {
    ...
    if (strExt.CompareNoCase(_T(".torrent")) == 0) {
      theApp.Message(MSG_DEBUG, _T("Auto-starting torrent file: %s"),
                     (LPCTSTR)pFile->GetPath());
...
bool CDownload::Resize(QWORD nNewSize) {
  ...
  if (!m_oSHA1 && !m_oTiger && !m_oED2K && !m_oMD5 && (m_oBTH || IsTorrent()) &&
      IsFileOpen()) {
    // Remove old file
    AbortTask();
    ClearVerification();
