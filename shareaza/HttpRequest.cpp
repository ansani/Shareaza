...
CHttpRequest::CHttpRequest()
    : m_hInternet(NULL), m_nLimit(0), m_nStatusCode(0),
      //	m_pPost( NULL ),
      m_pResponse(NULL), m_hNotifyWnd(NULL), m_nNotifyMsg(NULL),
      m_nNotifyParam(NULL), m_bUseCookie(true) {}

CHttpRequest::~CHttpRequest() { Clear(); }
...
BOOL CHttpRequest::SetURL(LPCTSTR pszURL) {
  if (IsPending())
    return FALSE;
  if (pszURL == NULL || !(_tcsncmp(pszURL, _T("http"), 4) == 0 ||
                          _tcsncmp(pszURL, _T("https"), 5) == 0))
    return FALSE;
  m_sURL = pszURL;
  return TRUE;
}

CString CHttpRequest::GetURL() const { return m_sURL; }
...
CString CHttpRequest::GetHeader(LPCTSTR pszName) const {
  CString strOut;
  return (!IsPending() && m_pResponseHeaders.Lookup(pszName, strOut)) ? strOut
                                                                      : _T("");
}

CString CHttpRequest::GetResponseString(UINT nCodePage) const {
  return (!IsPending() && m_pResponse)
             ? m_pResponse->ReadString(m_pResponse->m_nLength, nCodePage)
             : _T("");
}
...
BOOL CHttpRequest::IsPending() const { return IsThreadAlive(); }

BOOL CHttpRequest::IsFinished() const { return !IsPending() && m_nStatusCode; }
...
void CHttpRequest::OnRun() {
  ...
  if (m_hInternet) {
    HINTERNET hURL = CNetwork::InternetOpenUrl(
        m_hInternet, m_sURL, m_sRequestHeaders, m_sRequestHeaders.GetLength(),
    ...
    if (hURL) {
      ...
      BYTE nNull = 0;
      if (!IsThreadEnabled() ||
          !HttpQueryInfo(hURL, HTTP_QUERY_STATUS_TEXT,
                         m_sStatusString.GetBuffer(nLength), &nLength, 0))
        nLength = 0;
      ...
      if (m_sStatusString.GetLength()) {
        ...
        for (; IsThreadEnabled() &&
               ...
               nRemaining > 0 && m_pResponse->EnsureBuffer(nRemaining);) {
          if (!InternetReadFile(hURL,
                                m_pResponse->m_pBuffer + m_pResponse->m_nLength,
        ...
        if (IsThreadEnabled() && nRemaining == 0) {
          ...
          if (nLength) {
            ...
            HttpQueryInfo(hURL, HTTP_QUERY_RAW_HEADERS, pszHeaders, &nLength,
                          0);
...
void CHttpRequest::EnableCookie(bool bEnable) { m_bUseCookie = bEnable; }
