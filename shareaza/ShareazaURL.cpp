...
#define REG_STRING(x)                                                          \
  REG_SZ, ((LPBYTE)(LPCTSTR)(x)),                                              \
      ((DWORD)(sizeof(TCHAR) * (_tcslen((LPCTSTR)(x)) + 1)))

//////////////////////////////////////////////////////////////////////
// CShareazaURL construction

CShareazaURL::CShareazaURL(LPCTSTR pszURL)
    : m_nProtocol(PROTOCOL_NULL), m_nAction(uriNull), m_pTorrent(),
      m_pAddress(), m_pAddressIPv6(), m_nPort(0), m_pServerAddress(),
      m_nServerPort(0) {
  if (pszURL != NULL)
    Parse(pszURL);
}
...
CShareazaURL::CShareazaURL(CBTInfo *pTorrent)
    : CShareazaFile(static_cast<const CShareazaFile &>(*pTorrent)),
      m_nProtocol(PROTOCOL_NULL), m_nAction(uriDownload), m_pTorrent(pTorrent),
      m_pAddress(), m_pAddressIPv6(), m_nPort(0), m_pServerAddress(),
      m_nServerPort(0) {}

CShareazaURL::CShareazaURL(const CShareazaURL &pURL)
    : CShareazaFile(static_cast<const CShareazaFile &>(pURL)),
      m_nProtocol(pURL.m_nProtocol), m_nAction(pURL.m_nAction),
      m_pTorrent(pURL.m_pTorrent), m_sAddress(pURL.m_sAddress),
      m_pAddress(pURL.m_pAddress), m_pAddressIPv6(pURL.m_pAddressIPv6),
      m_nPort(pURL.m_nPort), m_pServerAddress(pURL.m_pServerAddress),
      m_nServerPort(pURL.m_nServerPort), m_sLogin(pURL.m_sLogin),
      m_sPassword(pURL.m_sPassword), m_oBTC(pURL.m_oBTC) {}
...
BOOL CShareazaURL::Parse(const CString &sText, CList<CString> &pURLs,
                         BOOL bResolve) {
  pURLs.RemoveAll();
...
BOOL CShareazaURL::ParseHTTP(LPCTSTR pszURL, BOOL bResolve) {
  ...
  if (StartsWith(m_sPath, _PT("/uri-res/N2R?"))) {
    ...
    CString sName(URLDecode(
        m_sPath.Mid(m_sPath.ReverseFind('/') + 1).SpanExcluding(_T("?"))));
    if (sName.GetLength()) {
      m_sName = sName;
    }
  }
  ...
  if (bResult = IPv6FromString(m_sAddress, &pHost)) {
    ...
    bResult = Network.Resolve(m_sAddress, INTERNET_DEFAULT_HTTP_PORT, &saHost,
                              bResolve);
...
BOOL CShareazaURL::ParseHTTPS(LPCTSTR pszURL, BOOL bResolve) {
  ...
  if (bResult = IPv6FromString(m_sAddress, &pHost)) {
    ...
    bResult = Network.Resolve(m_sAddress, INTERNET_DEFAULT_HTTPS_PORT, &saHost,
                              bResolve);
...
BOOL CShareazaURL::ParseFTP(LPCTSTR pszURL, BOOL bResolve) {
  ...
  if (!m_sName.GetLength() && nPos >= 0) {
    CString sName(URLDecode(m_sPath.Mid(nPos + 1).SpanExcluding(_T("?"))));
    if (sName.GetLength()) {
      m_sName = sName;
    }
  }
  ...
  BOOL bResult =
      Network.Resolve(m_sAddress, INTERNET_DEFAULT_FTP_PORT, &saHost, bResolve);
...
BOOL CShareazaURL::ParseED2KFTP(LPCTSTR pszURL, BOOL bResolve) {
  ...
  if (nSlash > 0) {
    ...
    if (_stscanf(strHash, _T("%lu"), &m_pAddress.S_un.S_addr) != 1)
      return FALSE;
    bPush = TRUE;
  }

  SOCKADDR_IN saHost;
  BOOL bResult = Network.Resolve(m_sAddress, protocolPorts[PROTOCOL_ED2K],
                                 &saHost, bResolve);
...
BOOL CShareazaURL::ParseDCHub(LPCTSTR pszURL, BOOL bResolve) {
  ...
  SOCKADDR_IN saHost = {};
  BOOL bResult = Network.Resolve(m_sAddress, protocolPorts[PROTOCOL_DC],
                                 &saHost, bResolve);
...
BOOL CShareazaURL::ParseBTC(LPCTSTR pszURL, BOOL bResolve) {
  ...
  if (bResult = IPv6FromString(m_sAddress, &pHost)) {
    ...
    SOCKADDR_IN saHost = {};
    bResult = Network.Resolve(m_sAddress, protocolPorts[PROTOCOL_BT], &saHost,
                              bResolve);
...
BOOL CShareazaURL::ParseMagnet(LPCTSTR pszURL) {
  ...
  for (strURL += _T('&'); strURL.GetLength();) {
    ...
    if (strKey == _T("xt") || // "eXact Topic" (URN containing file hash)
        ...
        strKey ==
            _T("ws") || // "Web Seed" HTTP/FTP Seeding (GetRight style), BEP 19
        strKey ==
            _T("as") ||     // "Acceptable Source" (web link to the online file)
        strKey == _T("tr")) // "TRacker address" (tracker URL for BitTorrent
                            // downloads)
    {
      if (StartsWith(strValue, _PT("urn:")) ||
        ...
        if (nMyNumber == nNumber) {
          if (!m_oSHA1)
            m_oSHA1.fromUrn(strValue);
          if (!m_oTiger)
            m_oTiger.fromUrn(strValue);
          if (!m_oMD5)
            m_oMD5.fromUrn(strValue);
          if (!m_oED2K)
            m_oED2K.fromUrn(strValue);
          if (!m_oBTH)
            m_oBTH.fromUrn(strValue);
          if (!m_oBTH)
            m_oBTH.fromUrn<Hashes::base16Encoding>(strValue);
        }
        ...
        } else if (strKey == _T("tr")) // "TRacker address" (tracker URL for
               ...
               strKey == _T("fs"))   // used by Foxy
    {
      if (nMyNumber == nNumber) {
        ...
        if (m_nSize == SIZE_UNKNOWN &&
            _stscanf(strValue, _T("%I64u"), &nSize) == 1 && nSize != 0) {
          m_nSize = nSize;
        }
      }
    } else if (strKey == _T("bn")) // "Bittorrent Node" (BitTorrent node address
                                   // for DHT bootstrapping)
    {
      // Nodes are common
...
BOOL CShareazaURL::ParseShareaza(LPCTSTR pszURL) {
  ...
  if (_stscanf(pszURL, _T("%i.%i.%i.%i"), &nIP[0], &nIP[1], &nIP[2], &nIP[3]) ==
      4) {
    ...
    return ParseDiscovery(SkipSlashes(pszURL, 4),
                          CDiscoveryService::dsWebCache);
  } else if (_tcsnicmp(pszURL, _T("meturl:"), 7) == 0) {
    return ParseDiscovery(SkipSlashes(pszURL, 7),
                          CDiscoveryService::dsServerMet);
...
BOOL CShareazaURL::ParseShareazaFile(LPCTSTR pszURL) {
  ...
  for (strURL += '/'; strURL.GetLength();) {
    ...
    if (StartsWith(strPart, _PT("urn:")) || StartsWith(strPart, _PT("sha1:")) ||
        StartsWith(strPart, _PT("bitprint:")) ||
        StartsWith(strPart, _PT("tree:tiger:")) ||
        StartsWith(strPart, _PT("md5:")) || StartsWith(strPart, _PT("btih:")) ||
        StartsWith(strPart, _PT("ed2k:"))) {
      if (!m_oSHA1)
        m_oSHA1.fromUrn(strPart);
      if (!m_oTiger)
        m_oTiger.fromUrn(strPart);
      if (!m_oMD5)
        m_oMD5.fromUrn(strPart);
      if (!m_oED2K)
        m_oED2K.fromUrn(strPart);
      if (!m_oBTH)
        m_oBTH.fromUrn(strPart);
      if (!m_oBTH)
        m_oBTH.fromUrn<Hashes::base16Encoding>(strPart);
...
BOOL CShareazaURL::ParseDonkeyFile(LPCTSTR pszURL) {
  ...
  QWORD nSize = 0;
  if (_stscanf(strPart, _T("%I64u"), &nSize) != 1 || nSize == 0)
    return FALSE;
  m_nSize = nSize;
  ...
  while (strURL.GetLength() > 8) {
    ...
    if (_tcsncmp(strPart, _T("sources"), 7) != 0)
      return TRUE;
    ...
    CString strEDFTP;
    strEDFTP.Format(_T("ed2kftp://%s/%s/%I64u/"), (LPCTSTR)strPart,
...
BOOL CShareazaURL::ParseDonkeyServer(LPCTSTR pszURL) {
  ...
  if (_stscanf(pszPort + 1, _T("%hu"), &m_nPort) != 1)
    return FALSE;
...
BOOL CShareazaURL::ParseDiscovery(LPCTSTR pszURL, int nType) {
  if (_tcsncmp(pszURL, _T("http://"), 7) != 0 &&
      ...
      _tcsncmp(pszURL, _PT(DSGnutella2TCP)) != 0)
    return FALSE;
...
void CShareazaURL::Register(BOOL bRegister, BOOL bOnStartup) {
  if (bRegister) {
    RegisterShellType(NULL, _T("shareaza"),
                      _T("URL:") CLIENT_NAME_T _T(" Protocol"), NULL,
                      CLIENT_NAME_T, _T("URL"), IDR_MAINFRAME);
    RegisterMagnetHandler(CLIENT_NAME_T, CLIENT_NAME_T _T(" Peer to Peer"),
                          CLIENT_NAME_T
                          _T(" can automatically search for and download the ")
                          _T("selected content its peer-to-peer networks."),
                          CLIENT_NAME_T, IDR_MAINFRAME);
  } else {
  ...
  if (Settings.Web.Magnet && bRegister) {
    RegisterShellType(NULL, _T("magnet"), _T("URL:Magnet Protocol"), NULL,
                      CLIENT_NAME_T, _T("URL"), IDR_MAINFRAME);
  } else {
  ...
  if (Settings.Web.DC && bRegister) {
    RegisterShellType(NULL, _T("dchub"), _T("URL:DirectConnect Protocol"), NULL,
                      CLIENT_NAME_T, _T("URL"), IDR_MAINFRAME);
    RegisterShellType(NULL, _T("dcfile"), _T("URL:DirectConnect Protocol"),
                      NULL, CLIENT_NAME_T, _T("URL"), IDR_MAINFRAME);
  } else {
  ...
  if (Settings.Web.Foxy && bRegister) {
    RegisterShellType(NULL, _T("foxy"), _T("URL:Foxy Protocol"), NULL,
                      CLIENT_NAME_T, _T("URL"), IDR_MAINFRAME);
  } else {
  ...
  if (Settings.Web.Gnutella && bRegister) {
    RegisterShellType(NULL, _T("gnutella"), _T("URL:Gnutella Protocol"), NULL,
                      CLIENT_NAME_T, _T("URL"), IDR_MAINFRAME);
    RegisterShellType(NULL, _T("gnet"), _T("URL:Gnutella Protocol"), NULL,
                      CLIENT_NAME_T, _T("URL"), IDR_MAINFRAME);
    RegisterShellType(NULL, _T("uhc"), _T("URL:Gnutella1 UDP Host Cache"), NULL,
                      CLIENT_NAME_T, _T("URL"), IDR_MAINFRAME);
    RegisterShellType(NULL, _T("ukhl"), _T("URL:Gnutella2 UDP known Hub Cache"),
                      NULL, CLIENT_NAME_T, _T("URL"), IDR_MAINFRAME);
    RegisterShellType(NULL, _T("gnutella1"), _T("URL:Gnutella1 Bootstrap"),
                      NULL, CLIENT_NAME_T, _T("URL"), IDR_MAINFRAME);
    RegisterShellType(NULL, _T("gnutella2"), _T("URL:Gnutella2 Bootstrap"),
                      NULL, CLIENT_NAME_T, _T("URL"), IDR_MAINFRAME);
    RegisterShellType(NULL, _T("gwc"), _T("URL:GWC Protocol"), NULL,
                      CLIENT_NAME_T, _T("URL"), IDR_MAINFRAME);
  } else {
  ...
  if (Settings.Web.ED2K && bRegister) {
    RegisterShellType(NULL, _T("ed2k"), _T("URL:eDonkey2000 Protocol"), NULL,
                      CLIENT_NAME_T, _T("URL"), IDR_MAINFRAME);
  } else {
  ...
  if (Settings.Web.Piolet && bRegister) {
    RegisterShellType(NULL, _T("mp2p"), _T("URL:Piolet Protocol"), NULL,
                      CLIENT_NAME_T, _T("URL"), IDR_MAINFRAME);
  } else {
  ...
  if (Settings.Web.Torrent && bRegister) {
    RegisterShellType(NULL, CLIENT_NAME_T _T(".Torrent"),
                      CLIENT_NAME_T _T(" Torrent File"), _T(".torrent"),
                      CLIENT_NAME_T, _T("RAZAFORMAT"), IDR_MAINFRAME);
    RegisterShellType(_T("Applications\\") CLIENT_NAME_T _T(".exe"), NULL,
                      CLIENT_NAME_T _T(" Torrent File"), _T(".torrent"),
                      CLIENT_NAME_T, _T("RAZAFORMAT"), IDR_MAINFRAME);
  } else {
  ...
  if (bRegister) {
    RegisterShellType(NULL, CLIENT_NAME_T _T(".Download"),
                      CLIENT_NAME_T _T(" Download File"), _T(".sd"),
                      CLIENT_NAME_T, _T("RAZAFORMAT"), IDR_MAINFRAME);
    RegisterShellType(_T("Applications\\") CLIENT_NAME_T _T(".exe"), NULL,
                      CLIENT_NAME_T _T(" Download File"), _T(".sd"),
                      CLIENT_NAME_T, _T("RAZAFORMAT"), IDR_MAINFRAME);
  ...
  if (bRegister) {
    RegisterShellType(NULL, CLIENT_NAME_T _T(".Collection"),
                      CLIENT_NAME_T _T(" Collection File"), _T(".co"),
                      CLIENT_NAME_T, _T("RAZAFORMAT"), IDI_COLLECTION);
    RegisterShellType(_T("Applications\\") CLIENT_NAME_T _T(".exe"), NULL,
                      CLIENT_NAME_T _T(" Collection File"), _T(".co"),
                      CLIENT_NAME_T, _T("RAZAFORMAT"), IDI_COLLECTION);

    RegisterShellType(NULL, CLIENT_NAME_T _T(".Collection"),
                      CLIENT_NAME_T _T(" Collection File"), _T(".collection"),
                      CLIENT_NAME_T, _T("RAZAFORMAT"), IDI_COLLECTION);
    RegisterShellType(_T("Applications\\") CLIENT_NAME_T _T(".exe"), NULL,
                      CLIENT_NAME_T _T(" Collection File"), _T(".collection"),
                      CLIENT_NAME_T, _T("RAZAFORMAT"), IDI_COLLECTION);
    ...
    RegisterShellType(NULL, CLIENT_NAME_T _T(".Collection"),
                      CLIENT_NAME_T _T(" Collection File"),
                      _T(".emulecollection"), CLIENT_NAME_T, _T("RAZAFORMAT"),
                      IDI_COLLECTION);
    RegisterShellType(_T("Applications\\") CLIENT_NAME_T _T(".exe"), NULL,
                      CLIENT_NAME_T _T(" Collection File"),
                      _T(".emulecollection"), CLIENT_NAME_T, _T("RAZAFORMAT"),
...
BOOL CShareazaURL::RegisterShellType(LPCTSTR pszRoot, LPCTSTR pszProtocol,
                                     LPCTSTR pszName, LPCTSTR pszType,
                                     LPCTSTR pszApplication, LPCTSTR pszTopic,
                                     UINT nIDIcon, BOOL bOverwrite) {
  HKEY hRootKey =
      AfxGetPerUserRegistration() ? HKEY_CURRENT_USER : HKEY_LOCAL_MACHINE;
  ...
  if (RegCreateKeyEx(hRootKey, (LPCTSTR)strSubKey, 0, NULL, 0, KEY_ALL_ACCESS,
                     NULL, &hKey, &nDisposition) != ERROR_SUCCESS)
    return FALSE;
  ...
  BOOL bApplication =
      pszRoot &&
      _tcsicmp(pszRoot, _T("Applications\\") CLIENT_NAME_T _T(".exe")) == 0;

  // Register protocol to "Default Programs"
  if (bProtocol) {
    CString strUrlAssociations = _T("Software\\Microsoft\\Windows\\Shell\\Asso")
                                 _T("ciations\\UrlAssociations\\");
    strUrlAssociations += pszProtocol;
    SHSetValue(hRootKey, strUrlAssociations + _T("\\UserChoice"), _T("Progid"),
               REG_STRING(CLIENT_NAME_T));
  }
  ...
  if (RegCreateKey(hKey, _T("shell"), &hSub1) == ERROR_SUCCESS) {
    if (RegCreateKey(hSub1, _T("open"), &hSub2) == ERROR_SUCCESS) {
      if (RegCreateKey(hSub2, _T("command"), &hSub3) == ERROR_SUCCESS) {
        strValue.Format(_T("\"%s\" \"%%%c\""), (LPCTSTR)theApp.m_strBinaryPath,
                        bProtocol ? _T('L') : _T('1'));
...
BOOL CShareazaURL::UnregisterShellType(LPCTSTR pszRoot) {
  ...
  if (pszRoot && *pszRoot != _T('.')) {
    ...
    LRESULT ret =
        SHGetValue(hRootKey, strUrlAssociations + _T("\\UserChoice"),
                   _T("Progid"), &nType, strProgID.GetBuffer(MAX_PATH), &nSize);
  ...
  if (pszRoot && *pszRoot == _T('.')) {
    ...
    if (RegOpenKeyEx(hRootKey, strSubKey, 0, KEY_QUERY_VALUE, &hKey) ==
        ERROR_SUCCESS) {
      CString strPath;
      DWORD dwType;
  ...
  if (!bRegisteredUser) {
    if (RegOpenKeyEx(hRootKey,
                     strSubKey + _T("\\shell\\open\\ddeexec\\Application"), 0,
                     KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS) {
      CString strPath;
      DWORD dwType;
      DWORD dwSize = MAX_PATH * sizeof(TCHAR);
...
BOOL CShareazaURL::RegisterMagnetHandler(LPCTSTR pszID, LPCTSTR pszName,
                                         LPCTSTR pszDescription,
                                         LPCTSTR pszApplication, UINT nIDIcon) {
  ...
  lResult = RegCreateKeyEx(hMagnetRoot, _T("Handlers"), 0, NULL, 0,
                           KEY_ALL_ACCESS, NULL, &hHandlers, &dwDisposition);
  ...
  lResult = RegCreateKeyEx(hHandlers, pszID, 0, NULL, 0, KEY_ALL_ACCESS, NULL,
                           &hHandler, &dwDisposition);
