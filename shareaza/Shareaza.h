...
public:
  CLogMessage(WORD nType, const CString &strLog)
      : m_strLog(strLog), m_nType(nType), m_Time(CTime::GetCurrentTime()) {}
  CString m_strLog;
  WORD m_nType;
  CTime m_Time;
};
...
public:
  ...
  // Kernel functions
  HRESULT(WINAPI *m_pRegisterApplicationRestart)
  (__in_opt PCWSTR pwzCommandline, __in DWORD dwFlags);
  ...
  (HANDLE, HDC, int, int, const RECT *, const RECT *);
  ...
  HRESULT(WINAPI *m_pfnSHGetFolderPathW)
  (__reserved HWND hwnd, __in int csidl, __in_opt HANDLE hToken,
   __in DWORD dwFlags, __out_ecount(MAX_PATH) LPWSTR pszPath);
  HRESULT(WINAPI *m_pfnSHGetKnownFolderPath)
  (__in REFKNOWNFOLDERID rfid, __in DWORD /* KNOWN_FOLDER_FLAG */ dwFlags,
   __in_opt HANDLE hToken, __deref_out PWSTR *ppszPath);
  HRESULT(WINAPI *m_pfnSHCreateItemFromParsingName)
  (__in PCWSTR pszPath, __in_opt IBindCtx *pbc, __in REFIID riid,
   __deref_out void **ppv);
  HRESULT(WINAPI *m_pfnSHGetPropertyStoreFromParsingName)
  (__in PCWSTR pszPath, __in_opt IBindCtx *pbc,
   __in GETPROPERTYSTOREFLAGS flags, __in REFIID riid, __deref_out void **ppv);
  HRESULT(WINAPI *m_pfnSetCurrentProcessExplicitAppUserModelID)
  (__in PCWSTR pszAppID);
  HRESULT(WINAPI *m_pfnSHGetImageList)
  (__in int iImageList, __in REFIID riid, __out void **ppv);

  HINSTANCE m_hUser32;
  BOOL(WINAPI *m_pfnChangeWindowMessageFilter)(UINT message, DWORD dwFlag);
  BOOL(WINAPI *m_pfnShutdownBlockReasonCreate)
  (_In_ HWND hWnd, _In_ LPCWSTR pwszReason);
  ...
  typedef const char *(*GeoIP_country_code_by_ipnumFunc)(GeoIP *gi,
                                                         unsigned long ipnum);
  typedef const char *(*GeoIP_country_name_by_ipnumFunc)(GeoIP *gi,
                                                         unsigned long ipnum);
  typedef const char *(*GeoIP_country_code_by_ipnumFunc_v6)(GeoIP *gi,
                                                            geoipv6_t ipnum);
  typedef const char *(*GeoIP_country_name_by_ipnumFunc_v6)(GeoIP *gi,
                                                            geoipv6_t ipnum);
  ...
  void SplashStep(LPCTSTR pszMessage = NULL, int nMax = 0,
                  bool bClosing = false);
  void SplashAbort();
...
protected:
  ...
  void
  InitResources();  // Initialize Shareaza version, system info, load DLLs, etc.
...
public:
  CProgressDialog(LPCTSTR szTitle,
                  DWORD dwFlags = PROGDLG_NOCANCEL | PROGDLG_AUTOTIME);
...
// mirrored icon (original destroyed if needed) if succeed or original icon
// otherwise
...
// Generate safe file name for file system (bPath == true - allow path i.e. "\"
// symbol)
CString SafeFilename(CString strName, bool bPath = false);

// Create directory. If one or more of the intermediate folders do not exist,
...
CString BrowseForFolder(UINT nTitle, LPCTSTR szInitialPath = NULL,
                        HWND hWnd = NULL);
CString BrowseForFolder(LPCTSTR szTitle, LPCTSTR szInitialPath = NULL,
                        HWND hWnd = NULL);
...
struct CompareNums {
  bool operator()(WORD lhs, WORD rhs) const { return lhs > rhs; }
};

// Use with whole numbers only
template <typename T> inline T GetRandomNum(const T &min, const T &max) {
  if (theApp.m_hCryptProv != 0) {
    T nRandom = 0;
    if (CryptGenRandom(theApp.m_hCryptProv, sizeof(T), (BYTE *)&nRandom))
      return static_cast<T>((double)nRandom * ((double)max - (double)min + 1) /
                                ((double)static_cast<T>(-1) + 1) +
                            min);
  }

  // Fallback to non-secure method
  return static_cast<T>(
      (double)rand() * (max - min + 1) / ((double)RAND_MAX + 1) + min);
}
...
#define WM_WINSOCK                                                             \
  (WM_APP + 101) // Winsock messages proxy to Network object ( used by
                 // WSAAsyncGetHostByName() function )
#define WM_VERSIONCHECK                                                        \
  (WM_APP +                                                                    \
   102) // Version check ( WAPARM: VERSION_CHECK nCode, LPARAM: unused )
#define WM_OPENCHAT                                                            \
  (WM_APP +                                                                    \
   103) // Open chat window ( WAPARM: CChatSession* pChat, LPARAM: unused )
#define WM_TRAY                                                                \
  (WM_APP +                                                                    \
   104) // Tray icon notification ( WPARAM: unused, LPARAM: uMouseMessage )
#define WM_URL                                                                 \
  (WM_APP + 105) // Open URL ( WPARAM: CShareazaURL* pURL, LPARAM: unused )
#define WM_SKINCHANGED                                                         \
  (WM_APP + 106) // Skin change ( WPARAM: unused, LPARAM: unused )
#define WM_COLLECTION                                                          \
  (WM_APP +                                                                    \
   107) // Open collection file ( WPARAM: LPTSTR szFilename, LPARAM: unused )
#define WM_OPENSEARCH                                                          \
  (WM_APP +                                                                    \
   108) // Open new search ( WPARAM: CQuerySearch* pSearch, LPARAM: unused )
#define WM_LIBRARYSEARCH                                                       \
  (WM_APP + 110) // Start file library search ( WPARAM: LPTSTR pszSearch,
                 // LPARAM: unused )
#define WM_PLAYFILE                                                            \
  (WM_APP +                                                                    \
   111) // Play file by media system ( WPARAM: TRISTATE bForcePlay - TRI_TRUE -
        // force play, TRI_FALSE - force enqueue, LPARAM: CString* pFilename )
#define WM_SETALPHA                                                            \
  (WM_APP + 113) // Increase/decrease main window transparency ( WPARAM: 0 - to
                 // decrease or 1 - to increase, LPARAM: unused )
#define WM_METADATA                                                            \
  (WM_APP +                                                                    \
   114) // Set/clear library meatapanel data and status message ( WPARAM:
        // CMetaPanel* pPanelData, LPARAM: LPCTSTR pszMessage )
#define WM_SANITY_CHECK                                                        \
  (WM_APP + 115) // Run allsystem check against banned hosts ( WPARAM: unused,
                 // LPARAM: unused )
#define WM_NOWUPLOADING                                                        \
  (WM_APP + 117) // New upload notification ( WPARAM: unused, LPARAM: CString*
                 // pFilename )
#define WM_TORRENT                                                             \
  (WM_APP +                                                                    \
   118) // Open torrent file ( WPARAM: LPTSTR szFilename, LPARAM: unused )
#define WM_IMPORT                                                              \
  (WM_APP +                                                                    \
   119) // Import hub list file ( WPARAM: LPTSTR szFilename, LPARAM: unused )

// WM_COPYDATA types
#define COPYDATA_SCHEDULER                                                     \
  0 // Scheduler task ( lpData: LPCTSTR szTaskData - encoded string )
#define COPYDATA_OPEN                                                          \
  1 // Open file ( lpData: LPCTSTR szFilename - file name or URL )
...
#define PANEL_WIDTH                                                            \
  200 // Left panel default size in pixels (Home, Search, IRC tabs)
#define THUMB_STORE_SIZE 128 // Thumbnail dimensions (128x128 px)

#define HTTP_HEADER_MAX_LINE                                                   \
  (256 * 1024) // Maximum allowed size of single HTTP-header line (256 Kb)
...
// 0 = eMule, 1 = cDonkey, 4 = old Shareaza alpha/beta/mod/fork, 0x28 (40) =
// Shareaza, 0xcb (203) = ShareazaPlus with RazaCB core, etc
#define ED2K_CLIENT_ID 41

// 2 Character BT peer-id code
// SZ = Shareaza, S~ = old Shareaza alpha/beta , CB = ShareazaPlus with RazaCB
// core, AZ = Azureus, etc
#define BT_ID1 'i'
#define BT_ID2 'S'
...
#define URI_UPDATE                                                             \
  _T("https://api.github.com/repos/ansani/Shareaza/releases/latest")
...
// ID_HELP_WEB_1	help/external/?link1
// unused ID_HELP_WEB_2	help/external/?link1
// unused ID_HELP_WEB_3	help/external/?link1
// unused ID_HELP_WEB_4	help/external/?link1
// unused ID_HELP_WEB_5	help/external/?link1
// unused ID_HELP_WEB_6	help/external/?link1 Skin Guide
//
// ID_HELP_TORRENT	help/?torrentencoding					Torrent
// Encoding Help -				?id=support
// BugTrap -				version/?Version=x.x.x.x&&Platform=x
// CVersionChecker
