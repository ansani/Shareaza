...
// A socket connection to a remote computer on the Internet running peer-to-peer
// software
class CConnection {
  // Construction
protected:
...
public:
  SOCKADDR_IN
      m_pHost; // The remote computer's IP address in Windows Sockets format
  SOCKADDR_IN6 m_pHostIPv6; // The remote computer's IPv6 address in Windows
                            // Sockets format
  ...
  BOOL m_bInitiated; // True if we initiated the connection, false if the remote
                     // computer connected to us
  BOOL m_bConnected; // True when the socket is connected
  DWORD m_tConnected; // The tick count when the socket connection was made
  SOCKET m_hSocket; // The actual Windows socket for the Internet connection to
                    // the remote computer
  CString
      m_sUserAgent; // The name of the program the remote computer is running
  BOOL
      m_bClientExtended; // Does the remote computer support extended functions
                         // i.e. running under Shareaza or compatible mod? In
                         // practice, this means can we use chat, browse, etc...
  CString
      m_sLastHeader; // The handshake header that ReadHeaders most recently read
...
private:
  ...
  CBuffer *m_pInput; // Data from the remote computer, will be compressed if the
                     // remote computer is sending compressed data
  CCriticalSectionPtr m_pOutputSection;
  CBuffer *m_pOutput; // Data to send to the remote computer, will be compressed
                      // if we are sending the remote computer compressed data
...
public:
  ...
  // Operations
public:
  // Exchange data with the other computer, measure bandwidth, and work with
  // headers
  virtual BOOL DoRun(); // Communicate with the other computer, reading and
                        // writing everything we can right now
  void QueueRun();      // (do) may no longer be in use
  void Measure();    // Measure the bandwidth, setting nMeasure in the bandwidth
                     // meters for each direction
  void MeasureIn();  // Measure the incoming bandwidth, setting nMeasure in the
                     // bandwidth meter
  void MeasureOut(); // Measure the outgoing bandwidth, setting nMeasure in the
                     // bandwidth meter
  BOOL ReadHeaders();   // Read text headers sitting in the input buffer
  BOOL SendMyAddress(); // If we are listening on a port, tell the other
                        // computer our IP address and port number
  ...
  inline BOOL IsValid() const throw() { return (m_hSocket != INVALID_SOCKET); }

  inline bool IsIPv6Host() const throw() {
    if (m_pHost.sin_addr.s_addr == INADDR_ANY ||
        m_pHost.sin_addr.s_addr == INADDR_NONE) {
      int i = 0;
  ...
  inline void Write(const CString &strData,
                    const UINT nCodePage = CP_ACP) throw() {
    CQuickLock oOutputLock(*m_pOutputSection);
  ...
  template <typename Descriptor, template <typename> class StoragePolicy,
            template <typename> class CheckingPolicy,
            template <typename> class ValidationPolicy>
  inline void Write(Hashes::Hash<Descriptor, StoragePolicy, CheckingPolicy,
                                 ValidationPolicy> &oHash) throw() {
    CQuickLock oOutputLock(*m_pOutputSection);
  ...
  template <typename Descriptor, template <typename> class StoragePolicy,
            template <typename> class CheckingPolicy,
            template <typename> class ValidationPolicy>
  inline void Read(Hashes::Hash<Descriptor, StoragePolicy, CheckingPolicy,
                                ValidationPolicy> &oHash) throw() {
  ...
  // Overrides
public:
  // Make a connection, accept a connection, copy a connection, and close a
  // connection
  virtual BOOL ConnectTo(
      const SOCKADDR_IN *pHost); // Connect to an IP address and port number
  ...
  virtual void
  AcceptFrom(SOCKET hSocket,
             SOCKADDR_IN *pHost); // Accept a connection from a remote computer
  virtual void
  AcceptFrom(SOCKET hSocket,
             SOCKADDR_IN6 *pHost); // Accept a connection from a remote computer
  ...
  virtual void DelayClose(UINT nError); // Send the buffer then close the
                                        // socket, record the error given
  ...
  virtual BOOL
  OnRead(); // Read data waiting in the socket into the input buffer
  virtual BOOL
  OnSSLRead(); // Read data waiting in the socket into the input buffer
  virtual BOOL
  OnWrite(); // Move the contents of the output buffer into the socket
  virtual BOOL
  OnSSLWrite(); // Move the contents of the output buffer into the socket
  virtual void OnDropped(); // (do) empty
  virtual BOOL
  OnHeaderLine(CString &strHeader,
...
public:
  ...
  static const DWORD METER_MINIMUM =
      METER_SECOND / 10ul; // Granuality of bandwidth meter, 1/10th of a second
  static const DWORD METER_LENGTH =
      60ul; // Number of slots in the bandwidth meter
  static const DWORD METER_PERIOD =
      METER_MINIMUM *
      METER_LENGTH; // The time that the bandwidth meter keeps information for

  // Keep track of how fast we are reading or writing bytes to a socket
  typedef struct {
    // Options to limit bandwidth
    DWORD *pLimit; // Points to a DWORD that holds the limit for this bandwidth
                   // meter

    // Transfer statistics
    DWORD nTotal;   // The total number of bytes read or written
    DWORD tLast;    // The time the last read or write happened
    DWORD nMeasure; // The average speed in bytes per second over the last
                    // METER_PERIOD
    ...
    void Add(const DWORD nBytes,
             const DWORD tNow); // Add to History and Time arrays
    DWORD CalculateLimit(DWORD tNow,
                         bool bMaxMode = true) const; // Work out the limit
    DWORD CalculateUsage(
        DWORD tTime) const; // Work out the meter usage from a given time
    // ( optimal for time periods more than METER_LENGTH / 2 )
    DWORD CalculateUsage(DWORD tTime, bool bShortPeriod)
        const; // Work out the meter usage from a given time
