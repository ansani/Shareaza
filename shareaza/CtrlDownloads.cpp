...
CDownloadsCtrl::CDownloadsCtrl()
    : m_nGroupCookie(0), m_nFocus(0), m_bCreateDragImage(FALSE),
      m_pDragDrop(NULL), m_bDrag(FALSE), m_pDeselect1(NULL), m_pDeselect2(NULL),
      m_pbSortAscending(NULL), m_bShowSearching(TRUE), m_tSwitchTimer(0) {
  // Try to get the number of lines to scroll when the mouse wheel is rotated
  if (!SystemParametersInfo(SPI_GETWHEELSCROLLLINES, 0, &m_nScrollWheelLines,
                            0)) {
...
BOOL CDownloadsCtrl::Create(CWnd *pParentWnd, UINT nID) {
  ...
  return CWnd::CreateEx(0, NULL, _T("CDownloadsCtrl"),
                        WS_CHILD | WS_CLIPSIBLINGS | WS_TABSTOP, rc, pParentWnd,
                        nID);
}
...
int CDownloadsCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  if (Settings.Downloads.SortColumns)
    m_wndHeader.Create(WS_CHILD | HDS_BUTTONS | HDS_DRAGDROP | HDS_HOTTRACK |
                           HDS_FULLDRAG,
                       rect, this, AFX_IDW_PANE_FIRST);
  else
    m_wndHeader.Create(WS_CHILD | HDS_DRAGDROP | HDS_HOTTRACK | HDS_FULLDRAG,
                       rect, this, AFX_IDW_PANE_FIRST);
...
void CDownloadsCtrl::InsertColumn(int nColumn, LPCTSTR pszCaption, int nFormat,
                                  int nWidth) {
  HDITEM pColumn = {};
...
void CDownloadsCtrl::SaveColumnState() {
  ...
  theApp.WriteProfileString(_T("ListStates"), _T("CDownloadCtrl.Ordering"),
                            strOrdering);
  theApp.WriteProfileString(_T("ListStates"), _T("CDownloadCtrl.Widths"),
                            strWidths);
}
...
BOOL CDownloadsCtrl::LoadColumnState() {
  ...
  strOrdering = theApp.GetProfileString(_T("ListStates"),
                                        _T("CDownloadCtrl.Ordering"), _T(""));
  strWidths = theApp.GetProfileString(_T("ListStates"),
                                      _T("CDownloadCtrl.Widths"), _T(""));
...
void CDownloadsCtrl::SelectTo(int nIndex) {
  ...
  if (bShift) {
    ...
    if (bControl) {
      if (pDownload != NULL)
        pDownload->m_bSelected = !pDownload->m_bSelected;
      if (pSource != NULL)
        pSource->m_bSelected = !pSource->m_bSelected;
    } else {
      if (pDownload != NULL)
        pDownload->m_bSelected = TRUE;
      if (pSource != NULL)
...
void CDownloadsCtrl::SelectAll(CDownload * /*pDownload*/,
                               CDownloadSource * /*pSource*/) {
  ASSUME_LOCK(Transfers.m_pSection);
...
void CDownloadsCtrl::DeselectAll(CDownload *pExcept1,
                                 CDownloadSource *pExcept2) {
...
BOOL CDownloadsCtrl::HitTest(const CPoint &point, CDownload **ppDownload,
                             CDownloadSource **ppSource, int *pnIndex,
                             RECT *prcItem) {
  ASSUME_LOCK(Transfers.m_pSection);
  ...
  for (POSITION posDownload = Downloads.GetIterator();
       posDownload && rcItem.top < rcClient.bottom;) {
    ...
    if (!pDownload->m_bExpanded ||
        (pDownload->IsSeeding() && !Settings.General.DebugBTSources))
      continue;
...
BOOL CDownloadsCtrl::GetAt(int nSelect, CDownload **ppDownload,
...
BOOL CDownloadsCtrl::DropShowTarget(CList<CDownload *> * /*pSel*/,
...
BOOL CDownloadsCtrl::DropObjects(CList<CDownload *> *pSel,
                                 const CPoint &ptScreen) {
  CPoint ptLocal(ptScreen);
  CRect rcClient;
  ...
  if (pSel == NULL || !rcClient.PtInRect(ptLocal))
    return FALSE;
...
void CDownloadsCtrl::OnSize(UINT nType, int cx, int cy) {
  ...
  m_wndHeader.SetWindowPos(NULL, -nScroll, 0, rcClient.right + nScroll,
                           HEADER_HEIGHT, SWP_SHOWWINDOW);
...
void CDownloadsCtrl::OnPaint() {
  ...
  for (POSITION posDownload = Downloads.GetIterator(); posDownload;) {
    ...
    if (nScroll > 0)
      ...
      PaintDownload(dc, rcItem, pDownload, bFocus && (m_nFocus == nIndex),
                    m_pDragDrop == pDownload);
      rcItem.OffsetRect(0, ITEM_HEIGHT);
    }
    ...
    for (POSITION posSource = pDownload->GetIterator(); posSource;) {
      ...
      if (Settings.Downloads.ShowSources || pSource->IsOnline()) {
        if (nScroll > 0)
          ...
          PaintSource(dc, rcItem, pDownload, pSource,
                      bFocus && (m_nFocus == nIndex));
          rcItem.OffsetRect(0, ITEM_HEIGHT);
        }
...
void CDownloadsCtrl::PaintDownload(CDC &dc, const CRect &rcRow,
                                   CDownload *pDownload, BOOL bFocus,
                                   BOOL bDrop) {
  COLORREF crNatural =
      m_bCreateDragImage ? DRAG_COLOR_KEY : CoolInterface.m_crWindow;
  COLORREF crBack =
      pDownload->m_bSelected ? CoolInterface.m_crHighlight : crNatural;
  COLORREF crText = CoolInterface.m_crText;
  COLORREF crLeftAligned = crBack;
  COLORREF crBorder = pDownload->m_bSelected
                          ? CoolInterface.m_crFragmentBorderSelected
                          : CoolInterface.m_crFragmentBorder;
  COLORREF crBorderSimple =
      pDownload->m_bSelected ? CoolInterface.m_crFragmentBorderSimpleBarSelected
                             : CoolInterface.m_crFragmentBorderSimpleBar;
    ...
    if (pDownload->m_bSelected) {
      if (pDownload->m_bVerify == TRI_FALSE)
        crText = CoolInterface.m_crTransferVerifyFailSelected;
               ...
               (pDownload->m_nSize > pDownload->m_nTorrentUploaded))
  ...
  for (int nColumn = 0; m_wndHeader.GetItem(nColumn, &pColumn); nColumn++) {
    ...
    POINT ptHover;
    RECT rcTick = {rcCell.left + 2, rcCell.top + 2, rcCell.left + 14,
                   rcCell.bottom - 2};
    ...
    if (client_rect.right < rcCell.left || client_rect.left > rcCell.right ||
        client_rect.top > rcCell.bottom || client_rect.bottom < rcCell.top)
      continue;
    ...
    case DOWNLOAD_COLUMN_TITLE:
      ...
      if (IsExpandable(pDownload)) {
        if (pDownload->m_bExpanded) {
          CoolInterface.Draw(
              &dc, (PtInRect(&rcTick, ptHover) ? IDI_MINUS_HOVER : IDI_MINUS),
              16, rcCell.left, rcCell.top, crLeftAligned);
        } else {
          CoolInterface.Draw(
              &dc, (PtInRect(&rcTick, ptHover) ? IDI_PLUS_HOVER : IDI_PLUS), 16,
      ...
      case 1: // Ratings suggest fake file
        CoolInterface.Draw(&dc, IDI_RATING_FAKE, 16, rcCell.left, rcCell.top,
                           CLR_NONE, pDownload->m_bSelected);
        break;
      case 2:
      case 3:
      case 4: // Ratings suggest average file
        CoolInterface.Draw(&dc, IDI_RATING_AVERAGE, 16, rcCell.left, rcCell.top,
                           CLR_NONE, pDownload->m_bSelected);
        break;
      default: // Ratings suggest good file
        CoolInterface.Draw(&dc, IDI_RATING_GOOD, 16, rcCell.left, rcCell.top,
                           CLR_NONE, pDownload->m_bSelected);
        break;
      }

      rcCell.left += 16;
      dc.FillSolidRect(rcCell.left, rcCell.top, 1, rcCell.Height(),
                       crLeftAligned);
      rcCell.left += 1;
      ...
      if (rcCell.Width() > 75) {
        ...
        if (NonRandomEnd > 0 && pDownload->m_nSize > 0) {
          ...
          if (NonRandomEnd > pDownload->m_nSize)
            ...
            rect.left +=
                ((rect.Width() - 3) * NonRandomEnd) / pDownload->m_nSize;
        ...
        if (pDownload->m_nStartFrom > 0 && pDownload->m_nSize > 0) {
          ...
          if (pDownload->m_nStartFrom > pDownload->m_nSize)
            ...
            rect.left += ((rect.Width() - 3) * pDownload->m_nStartFrom) /
                         pDownload->m_nSize;
    ...
    case DOWNLOAD_COLUMN_SPEED:
      if (pDownload->IsTrying()) {
        ...
        if (nSpeed && nRealSpeed)
          strText.Format(_T("%s (%s)"), Settings.SmartSpeed(nSpeed),
    ...
    if (dc.GetTextExtent(strText).cx > rcCell.Width() - 8) {
      while (dc.GetTextExtent(strText + _T('\x2026')).cx >
                 (rcCell.Width() - 8) &&
             strText.GetLength() > 0) {
    ...
    dc.SetBkColor(crBack);
    dc.ExtTextOut(nPosition, rcCell.top + 2, ETO_CLIPPED | ETO_OPAQUE, &rcCell,
                  strText, NULL);
  }
  ...
  if (bFocus) {
    CRect rcFocus(nTextLeft, rcRow.top, max(int(rcRow.right), nTextRight),
                  rcRow.bottom);
    dc.Draw3dRect(&rcFocus, CoolInterface.m_crHiBorder,
...
void CDownloadsCtrl::PaintSource(CDC &dc, const CRect &rcRow,
                                 CDownload *pDownload, CDownloadSource *pSource,
                                 BOOL bFocus) {
  int nFlagImage = Flags.GetFlagIndex(pSource->m_sCountry);
  COLORREF crNatural =
      m_bCreateDragImage ? DRAG_COLOR_KEY : CoolInterface.m_crWindow;
  COLORREF crBack =
      pSource->m_bSelected ? CoolInterface.m_crHighlight : crNatural;
  COLORREF crLeftAligned = crBack;
  COLORREF crBorder = pSource->m_bSelected
                          ? CoolInterface.m_crFragmentBorderSelected
                          : CoolInterface.m_crFragmentBorder;
  ...
  for (int nColumn = 0; m_wndHeader.GetItem(nColumn, &pColumn); nColumn++) {
    ...
    switch (pColumn.lParam) {
    case DOWNLOAD_COLUMN_TITLE:
      dc.FillSolidRect(rcCell.left, rcCell.top, 24, rcCell.Height(),
                       crLeftAligned);
      rcCell.left += 24;
      ...
      ImageList_DrawEx(m_pProtocols, pSource->m_nProtocol, dc.GetSafeHdc(),
                       rcCell.left, rcCell.top, 16, 16, crLeftAligned,
                       CLR_DEFAULT,
                       pSource->m_bSelected ? ILD_SELECTED : ILD_NORMAL);
      rcCell.left += 16;
      dc.FillSolidRect(rcCell.left, rcCell.top, 1, rcCell.Height(),
                       crLeftAligned);
      rcCell.left += 1;
      ...
      // Or an active transfer
        ...
        strText.Format(_T("%s:%u"), (LPCTSTR)pSource->GetAddress(),
                       ntohs(pSource->GetPort()));
      }
      // Or just queued
      else {
        if (pSource->IsIPv6Source())
          strText.Format(_T("%s:%u"), IPv6ToString(&pSource->m_pAddressIPv6),
                         pSource->m_nPort);
        else
          strText.Format(_T("%s:%u"),
      ...
      if (rcCell.Width() > 75) {
        ...
        if (pSource->GetState() > dtsHeaders && pSource->m_oAvailable.empty())
          ...
          strText.Format(_T("%.2f%%"),
                         float(pSource->m_oAvailable.length_sum() * 10000 /
                               pSource->m_pDownload->m_nSize) /
                             100);
        else
          strText.Format(_T("%i%%"), int(pSource->m_oAvailable.length_sum() *
                                         100 / pSource->m_pDownload->m_nSize));
      break;
    ...
    case DOWNLOAD_COLUMN_PERCENTAGE:
      if (!pSource->IsIdle() && pSource->GetDownloaded() > 0 &&
        ...
        if (rcCell.Width() > 50)
          strText.Format(_T("%.2f%%"), float(pSource->GetDownloaded() * 10000 /
                                             pSource->m_pDownload->m_nSize) /
                                           100.0f);
        else
          strText.Format(_T("%i%%"), int(pSource->GetDownloaded() * 100 /
                                         pSource->m_pDownload->m_nSize));
      }
      break;
    case DOWNLOAD_COLUMN_COUNTRY:
      ...
      rcCell.left += 2;
      if (nFlagImage >= 0)
        Flags.Draw(nFlagImage, dc.GetSafeHdc(), rcCell.left, rcCell.top + 2,
...
void CDownloadsCtrl::OnVScroll(UINT nSBCode, UINT nPos,
...
void CDownloadsCtrl::OnHScroll(UINT nSBCode, UINT nPos,
                               CScrollBar * /*pScrollBar*/) {
  SCROLLINFO pInfo;
  ...
  m_wndHeader.SetWindowPos(NULL, -pInfo.nPos, 0, rcClient.right + pInfo.nPos,
                           HEADER_HEIGHT, SWP_NOZORDER);

  RedrawWindow(NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW);
}

BOOL CDownloadsCtrl::OnMouseWheel(UINT /*nFlags*/, short zDelta,
                                  CPoint /*pt*/) {
  OnVScroll(
      SB_THUMBPOSITION,
      (int)(GetScrollPos(SB_VERT) - zDelta / WHEEL_DELTA * m_nScrollWheelLines),
      NULL);
  return TRUE;
}
...
void CDownloadsCtrl::BubbleSortDownloads(
    int nColumn) // BinaryInsertionSortDownloads(int nColumn)
{

  m_pbSortAscending[nColumn] = !m_pbSortAscending[nColumn];

  if (Downloads.GetCount() < 2)
    return;
    ...
    while (bRlBk && (pos_y != NULL)) {
      ...
      if (m_pbSortAscending[nColumn] == FALSE) {
        ...
        case DOWNLOAD_COLUMN_DOWNLOADED:
          ...
          break;
        case DOWNLOAD_COLUMN_PERCENTAGE:
          if (((double)(x->GetVolumeComplete()) / (double)(x->m_nSize)) <
              ((double)(y->GetVolumeComplete()) / (double)(y->m_nSize)))
            bOK = TRUE;
          else
            bRlBk = FALSE;
          break;
...
void CDownloadsCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
  ...
  case VK_RIGHT:
  case VK_ADD:
    if (GetAt(m_nFocus, &pDownload, NULL) && pDownload != NULL &&
        pDownload->m_bExpanded == FALSE) {
  ...
  case 'E':
    if (bControl) {
      ...
      GetOwner()->PostMessage(
          WM_COMMAND, ID_DOWNLOADS_ENQUEUE); // Add the current file to playlist
    }
    return;
  case 'R':
    if (bControl)
  ...
  case VK_DELETE:
    ...
    return;
  case VK_RETURN: // If the enter key is pressed activate the function relevant
                  // to the current focus
    OnEnterKey(); // Run the function that does the actions on the download
                  // window when enter key is pressed
    return;
  }
...
void CDownloadsCtrl::OnEnterKey() {
  ...
  if (pDownload != NULL) // If the selected object is a download...
  {
    if (GetAsyncKeyState(VK_SHIFT) &
        0x8000) // And the control key is pressed...
    {
      GetOwner()->PostMessage(WM_TIMER, 5);
      GetOwner()->PostMessage(
          WM_COMMAND,
          pDownload->IsCompleted()
              ? ID_DOWNLOADS_LAUNCH_COMPLETE
              : ID_DOWNLOADS_LAUNCH_COPY); // Launch the current file
    }
  } else if (pSource != NULL) // If the selected object is a download source...
  {
    GetOwner()->PostMessage(WM_TIMER, 5);
    GetOwner()->PostMessage(WM_COMMAND,
                            ID_TRANSFERS_CONNECT); // Connect to the source
  }
}
...
void CDownloadsCtrl::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  if (HitTest(point, &pDownload, &pSource, &nIndex, &rcItem)) {
    ...
    if (point.x > nTitleStarts && point.x <= nTitleStarts + rcItem.left + 16) {
      ...
      if (pDownload != NULL &&
          (nFlags & (MK_SHIFT | MK_CONTROL | MK_RBUTTON)) == 0) {
        CRect rcCell;

        m_wndHeader.GetItemRect(DOWNLOAD_COLUMN_PROGRESS, &rcCell);
        if (point.x >= rcCell.left + rcItem.left &&
            point.x <= rcCell.right + rcItem.left - 3)
          pDownload->SetStartFrom(pDownload->m_nSize *
                                  (point.x - (rcCell.left + rcItem.left)) /
                                  (rcCell.Width() - 3));
      }
...
void CDownloadsCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) {
  ...
  if (HitTest(point, &pDownload, &pSource, NULL, &rcItem)) {
    int nTitleStarts = GetExpandableColumnX();
    if (pDownload != NULL && point.x > nTitleStarts &&
        point.x <= nTitleStarts + rcItem.left + 16) {
      if (IsExpandable(pDownload)) {
        pDownload->m_bExpanded = !pDownload->m_bExpanded;
...
CImageList *CDownloadsCtrl::CreateDragImage(CList<CDownload *> *pSel,
  ...
  pAll->Create(rcAll.Width(), rcAll.Height(), ILC_COLOR32 | ILC_MASK, 1, 1) ||
      pAll->Create(rcAll.Width(), rcAll.Height(), ILC_COLOR24 | ILC_MASK, 1,
                   1) ||
...
int CDownloadsCtrl::GetExpandableColumnX() const {
  ...
  for (int nColumn = 0;
       m_wndHeader.GetItem(m_wndHeader.OrderToIndex(nColumn), &pColumn);
       nColumn++) {
...
UINT CDownloadsCtrl::OnGetDlgCode() { return DLGC_WANTARROWS; }
