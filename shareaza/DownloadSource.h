...
public:
  ...
  CDownloadSource(const CDownload *pDownload, DWORD nClientID, WORD nClientPort,
                  DWORD nServerIP, WORD nServerPort, const Hashes::Guid &oGUID);
  CDownloadSource(const CDownload *pDownload, const Hashes::BtGuid &oGUID,
                  const IN_ADDR *pAddress, WORD nPort);
  CDownloadSource(const CDownload *pDownload, const Hashes::BtGuid &oGUID,
                  const IN6_ADDR *pAddress, WORD nPort);
  CDownloadSource(const CDownload *pDownload, LPCTSTR pszURL,
                  BOOL bHashAuth = FALSE, FILETIME *pLastSeen = NULL,
                  int nRedirectionCount = 0, BOOL bPartialSame = FALSE);
  ~CDownloadSource();
...
public:
  ...
  // 2 - Pure G2
  // 3 - Mixed G1/G2
  BOOL m_bClientExtended; // Does the user support extended (G2) functions? (In
                          // practice, this means can we use G2 chat, browse,
                          // etc...)
  ...
  CString m_sPreview; // if empty it has the default
                      // /gnutella/preview/v1?urn:xyz format
  BOOL m_bPreviewRequestSent;
  BOOL m_bMetaIgnore; // Ignore metadata from this source (for example already
                      // got)
...
public:
  ...
  // Remove source from download, add it to failed sources if bBan == TRUE, and
  // destroy source itself
  ...
  inline bool Equals(const CDownloadSource *pSource) const {
    ...
    if (m_nServerPort != pSource->m_nServerPort) {
      ...
      // Push
      if (m_pServerAddress.S_un.S_addr != pSource->m_pServerAddress.S_un.S_addr)
        return FALSE;
      if (!IN6_ADDR_EQUAL(&m_pIPv6ServerAddress,
                          &pSource->m_pIPv6ServerAddress))
        return FALSE;
      if (m_pAddress.S_un.S_addr != pSource->m_pAddress.S_un.S_addr)
        return FALSE;
      if (!IN6_ADDR_EQUAL(&m_pAddressIPv6, &pSource->m_pAddressIPv6))
        return FALSE;
    } else {
      // Direct
      if (m_pAddress.S_un.S_addr != pSource->m_pAddress.S_un.S_addr)
  ...
  inline bool IsOnline() const { return m_nBusyCount || IsConnected(); }

  inline bool IsHTTPSource() const {
    return (m_nProtocol == PROTOCOL_HTTP || m_nProtocol == PROTOCOL_G2);
  }

  inline bool IsSSLSource() const { return (m_nProtocol == PROTOCOL_SSL); }
...
struct std::less<CDownloadSource *>
    : public std::binary_function<CDownloadSource *, CDownloadSource *, bool> {
  inline bool operator()(const CDownloadSource *_Left,
                         const CDownloadSource *_Right) const throw() {
