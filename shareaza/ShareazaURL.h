...
public:
  ...
  // Parse URL list
  BOOL Parse(const CString &sText, CList<CString> &pURLs,
             BOOL bResolve = FALSE);
  // Parse single URL
...
protected:
  ...
  // http://[user[:password]@]host[:port]/[filepath], where {filepath} is a
  // regular path or "/uri-res/N2R?hash"
  BOOL ParseHTTP(LPCTSTR pszURL, BOOL bResolve);
  // https://[user[:password]@]host[:port]/[filepath], where {filepath} is a
  // regular path or "/uri-res/N2R?hash"
  ...
  // dchub://[login@]address:port/[filepath], where {filepath} can be a regular
  // path or "files.xml.bz2" or "TTH:tiger_hash/size/"
  ...
  // Host
  //	shareaza:[//]{verb}{[user@]address[:port]}, where {verb} is "" (empty),
  //"host:", "hub:", "server:", "browse:" or "btnode:"
  // WebCache
  //	shareaza:[//]gwc:{url}[?nets={net_list}], where {net_list} is "gnutella"
  //or "gnutella2"
  // ServerMet
  //	shareaza:[//]meturl:{url}
  // Discovery
  //	shareaza:[//]{verb}{url}, where {verb} is "uhc:", "ukhl:",
  //"gnutella1:host:" or "gnutella2:host:"
  // URL
  //	shareaza:[//]url:{nested_url}
  BOOL ParseShareaza(LPCTSTR pszURL);
  BOOL ParseShareazaHost(LPCTSTR pszURL, BOOL bBrowse = FALSE,
                         PROTOCOLID nProtocol = PROTOCOL_G2);
...
protected:
  static BOOL RegisterShellType(LPCTSTR pszRoot, LPCTSTR pszProtocol,
                                ...
                                UINT nIDIcon, BOOL bOverwrite = TRUE);
  static BOOL RegisterMagnetHandler(LPCTSTR pszID, LPCTSTR pszName,
