...
CVersionChecker::CVersionChecker() : m_bVerbose(false) {}

CVersionChecker::~CVersionChecker() { Stop(); }
...
BOOL CVersionChecker::IsVersionNewer() {
  WORD nVersion[4];
  return (_stscanf_s(Settings.VersionCheck.UpgradeVersion,
                     _T("%hu.%hu.%hu.%hu"), &nVersion[0], &nVersion[1],
                     &nVersion[2], &nVersion[3]) == 4) &&
...
BOOL CVersionChecker::NeedToCheck() {
  ...
  if (!Settings.VersionCheck.UpdateCheck)
    return FALSE;
  if (!Settings.VersionCheck.NextCheck)
    return TRUE;
  if (Settings.VersionCheck.NextCheck == 0xFFFFFFFF)
    return FALSE;

  return (DWORD)CTime::GetCurrentTime().GetTime() >=
         Settings.VersionCheck.NextCheck;
}
...
void JsonToCStringIMap(const boost::json::value &jVal, CStringIMap &map,
  ...
  if (jVal.is_object()) {
    ...
    CString valueStr(jVal.as_string().data(),
                     static_cast<int>(jVal.as_string().size()));
...
BOOL CVersionChecker::ExecuteRequest() {
  ...
  theApp.Message(MSG_DEBUG | MSG_FACILITY_OUTGOING,
                 _T("[VersionChecker] Request: %s"), (LPCTSTR)strURL);
  ...
  theApp.Message(MSG_DEBUG | MSG_FACILITY_INCOMING,
                 _T("[VersionChecker] Response: %s"), (LPCTSTR)strOutput);
...
// Find URI Asset
CString FindAssetURI(const CStringIMap &map, const CString platform,
                     const CString buildtype) {
  POSITION pos = map.GetStartPosition();
  while (pos != NULL) {
    CString key, value;
    map.GetNextAssoc(pos, key, value);

    if (key.Find(_T("asset")) == 0 &&
        key.Find(_T("browser_download_url")) != -1 &&
        value.Find(platform) != -1 && value.Find(buildtype) != -1) {
      return value;
    }
  }
...
void CVersionChecker::ProcessResponse() {
  ...
  if (m_pResponse.Lookup(_T("body"), strValue)) {
    for (strValue += '\n'; strValue.GetLength();) {
      ...
      if (strLine.GetLength())
        theApp.Message(MSG_NOTICE, strLine);
    }
  }

  // UpgradePrompt
  if (m_pResponse.Lookup(_T("name"), strValue)) {
    Settings.VersionCheck.UpgradePrompt =
        _T("New version available: ") + strValue;
    ...
    //		m_pResponse.Lookup( _T("UpgradeFile"),
    //Settings.VersionCheck.UpgradeFile );
    ...
    //		m_pResponse.Lookup( _T("UpgradeSources"),
    //Settings.VersionCheck.UpgradeSources );
  ...
  if (m_pResponse.Lookup(_T("AddDiscovery"), strValue)) {
    strValue.Trim();
    theApp.Message(MSG_DEBUG, _T("[VersionChecker] %s = %s"),
                   _T("AddDiscovery"), (LPCTSTR)strValue);
  ...
  if (m_pResponse.Lookup(_T("AddDiscoveryUHC"), strValue)) {
    strValue.Trim();
    theApp.Message(MSG_DEBUG, _T("[VersionChecker] %s = %s"),
                   _T("AddDiscoveryUHC"), (LPCTSTR)strValue);
...
BOOL CVersionChecker::CheckUpgradeHash(const CLibraryFile *pFile) {
  if (IsUpgradeAvailable()) {
    ...
    if (oFilter.m_oSHA1.fromString(Settings.VersionCheck.UpgradeSHA1)) {
      ...
      if (pFile && validAndEqual(pFile->m_oSHA1, oFilter.m_oSHA1) &&
